...
- **history-rollback-removes-complete-turns** — A rollback (round 081 / ADR 0053; its durability clause **witnessed and calibrated** by round 084 / **ADR 0056**) removes the last N **complete** `history.Entry` lines — the operator prompt, the provider answer, and **every** recorded tool `step` of each removed turn, atomically (a turn is never split) — and MUST leave the surviving entries byte-unchanged; the survivors are written to a **same-directory temporary file**, which is `fsync`ed **before** it is atomically renamed over the active file (so an in-place truncate is impossible and the prior history survives a crash before the rename; the store-level durability seam that performs the `fsync`-before-rename is the clause's witness, and the best-effort directory `fsync` is an **accepted-unwitnessed** limit — ADR 0056 §Forward), **clamps** N to the available turns, and MUST NOT touch the archive (`history.archive.jsonl`). The `history.Entry` / `history.Step` JSON shapes are unchanged.
