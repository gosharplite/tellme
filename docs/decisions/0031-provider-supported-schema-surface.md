...
**D6b — The floor is structure-aware and the postcondition is re-asserted (fold of review B-061-1).** `stripVendorExtensions` descends **only** through schema-node positions (`items`, `additionalProperties`, `not`, `contains`, `if`/`then`/`else`, `propertyNames`, `unevaluated*`; the `anyOf`/`allOf`/`oneOf`/`prefixItems` lists; and the `properties`/`patternProperties`/`dependentSchemas` maps, whose keys are NAMES and therefore opaque). It never walks a data value (`default`/`enum`/`const`/`examples`). `NormalizeMCPSchema` re-asserts `required ⊆ properties` on the **post-floor** object, so the round-031/#64 invariant holds by construction rather than by argument: a property *named* `x-…` survives, and the floor cannot re-create the 400 it exists to prevent.
