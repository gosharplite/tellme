...
| a non-string `type` array member (`[5]`/`[true]`/`[{}]`/`[["string"]]`) | **rejected** — the emitted `type: 5`/`true`/… is the same measured-reject as `type: 5` above (fold of W-061-1) |
...
| **wrong-KIND values** — `description: 123`, `title: true`, `type: 5`, `required: "x"`, `required: [5]`, `properties: "nope"`, `enum: [{"x":1}]` | **rejected** (TYPE_STRING / message / enum decode errors) |
| `minItems: "3"`, `nullable: "yes"`, `oneOf: {…}` (wrong kinds the proto tolerates) | accepted — still dropped for uniformity (an absent keyword is always safe) |
...
**D8 — The guarantee is key AND value kind: a data-driven kind table (fold of review V-061-1).** The surface's single owner is now `supportedSchemaValueKinds` — one table mapping each supported key to the JSON kind its proto field requires (`supportedSchemaKeys` is derived from it). The projection applies the kind per key: a kind-matched value passes through; the **measured** coercions apply (`type` array → lone member + `nullable`; `enum` scalars → strings, non-scalars dropped; a non-object schema node → `{}`); **every other mismatch is DROPPED** (an absent keyword is always accepted, so dropping is always safe — and never coerces into an unmeasured shape). The rule applies to a **substituted value** as well: a `type` array's lone member must itself be a string (a `[5]`/`[true]`/`[{}]`/`[["string"]]` member emits **no** `type`) — the fold of review W-061-1, closing the one path where the table's own coercion could emit a wrong-kind value. This closes the last position/kind chase (R-2/R-3/R-5/W-061-1 were each one instance of it) and moves the adapter toward the reference's typed per-family schema model (A1) while keeping the raw-JSON relay. The regression gate reads the table on **both** axes: key membership (`assertContainment`) **and** value kind (`SchemaValueKindOK`), plus the golden set and per-key coverage.
...
- **RF-061-12** *(V-061-1)* — the kind table is keyed by *kind*, not by per-field proto type (e.g. `minLength`/`minItems` share `kindInt`; `description`/`title`/`format`/`pattern` share `kindString`); a future proto-type change is one table entry per key.
