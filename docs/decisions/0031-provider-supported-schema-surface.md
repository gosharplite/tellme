...
**D6c — A non-object subschema degrades to `{}` (fold of review R-3).** At `properties` values, `items`, and `additionalProperties` (a `bool` there is legal and is kept), a subschema that is not an object — a draft-04/2019 boolean schema, a bare scalar — is unrepresentable as a `Schema` message and would be a proto type error (the same whole-request 400 class). It degrades to the accepted empty `{}` (`{}` was probed precisely so an empty subschema may be emitted).

**D6b — The floor is structure-aware and the postcondition is re-asserted (fold of review B-061-1).**
