...
- **RF-063-10** *(PR #130 review TD-063-4)* — the **comment-only "protection" Rule** is now a four-round recurrence (R-056-c / R-058-c / R-059-c / R-062-c, and this round's acceptance + interface features each carry one). Four assertions of the same debt with no convergence is the signal: **stop authoring meta-Rules** — delete the empty Rule (or demote it to a header comment), since the closing comment already cites the real carriers (the Examples + the unit pin + ADR Verification), so every Rule in a feature is executable. **PM-owned**; carried to round `064-*`.
