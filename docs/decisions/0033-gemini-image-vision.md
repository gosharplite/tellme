...
- **RF-063-1** — the Gemini ceiling figure (14 MiB) is **derived, not measured**; confirm the documented bound and its unit at the closeout live check, then move the single constant if it differs. (The plan-side acceptance wording is *relative* — "larger than the Gemini image size limit" — but the **E2E runner's fixture is a fixed `32<<20+1`**, so a ceiling move does *not* propagate to it: see **RF-063-8**.)
