...
- The failure surface is **byte-identical** to today: `the provider request failed` + exit 6 (or `the tool request failed` + exit 7), `stdout` empty, plus one non-class informational line.
...
- **RF-080-6** — hole #2's full behavioural E2E (a `Ctrl+C` inside a real 1–3 s retry wait) is unit-witnessed at the predicate; the timing-sensitive E2E is declined.
