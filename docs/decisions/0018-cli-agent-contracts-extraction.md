...
- **Witness**: the gate (RULE-E reports the baseline **2**, 0 new / 0 stale; RULE-A/B/C **0**; 0 cycles) + the **identifier-count check** (`internal/cli` production `→ agent` identifiers == 1) + the existing turn unit pins, plus falsifiability witnesses (a) an incomplete extraction ⇒ count **2+**; (b) a flipped domain contract ⇒ the CLI seam **fails to compile**; (c) a partial extraction leaving a second coupling ⇒ rejected — reproduced then reverted (ADR 0010). The E2E suite is regression, not the carrier (#92 AC5).

## Forward

- **Sub-slice 2** (the baseline-moving round): invert the surviving `AgentLoop` construction/execution into an injected domain port. It can now be **RULE-C-pure** in `internal/domain/agent` (the result/error types are domain-owned), with a tier-≥4 adapter (or the exempt `cmd/tellme`); the ratchet moves **2 → 1**.
