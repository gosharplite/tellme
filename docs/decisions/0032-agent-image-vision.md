...
- **Status:** Accepted (**D4/D8 narrowed and §Forward RF-062-1 superseded by [ADR 0033](0033-gemini-image-vision.md)** — the Gemini family's `inlineData` image path; **§Forward RF-062-10 FULLY DELIVERED** — its `ToolSetSpec` seam half by [ADR 0039](0039-toolset-spec-capability-seam.md) (round 069) and its media-channel half by [ADR 0040](0040-media-channel-in-band.md) (round 070); **`D7a` (the media-channel mechanism) is SUPERSEDED by [ADR 0040](0040-media-channel-in-band.md)**; **D7 CLARIFIED by [ADR 0055](0055-round-scoped-media-placement.md) (round 083)** — D7's **body** already reads round-scoped ("**one** `user` message … after the round's `tool`-result message(s)"), but its **heading** ("a `user` message after the tool result") was read per-call in the pre-083 **implementation** (`agentloop.go`, one media message inside the per-call loop), which broke any round with ≥2 media calls; ADR 0055 makes the implementation match D7's body (one media message after **all** the round's results) and adds the missing tripwire; the rest of this ADR stands)
- **Date:** 2026-09-19
