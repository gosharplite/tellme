...
1. **`TestFileStore_Rollback_SyncsTempFileBeforeRename`** — the fake logs an ordered event list (`sync` on `*.tmp` → `rename` `*.tmp → history.jsonl` → `syncdir`); asserts the `sync` precedes the `rename`. **Discriminating mutation:** remove the `s.fs.Sync(f)` call ⇒ the pin reddens.
...
4. **`TestFileStore_Rollback_PreservesSurvivorBytesOnToolWrittenPath`** — three entries appended **through the store**, one rolled back; the surviving bytes are **byte-identical** to the first two original lines (the tool-written path, not a hand-filled file), closing the grill-flagged "MUST leave the surviving entries byte-unchanged" sub-clause.
