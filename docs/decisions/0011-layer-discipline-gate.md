...
**D5 — Host-independence is evaluated as the union over `CROSS_TARGETS`.** `go list {{.Imports}}` resolves in the **host** build context: files excluded by `GOOS`/`GOARCH` land in `.IgnoredGoFiles` and their imports vanish, so a single-context walk is host-relative. The guard evaluates the **union** over the project's supported targets — `linux/amd64`, `linux/arm64`, `darwin/amd64`, `darwin/arm64` (round 020's `CROSS_TARGETS`) — so an OS-gated illegal import cannot hide and the single committed baseline is genuinely host-free.

**D6 — Open (`//go:build` tag-gated) files are out of scope, recorded.** Passing no `-tags` to the child means tag-gated files are invisible to `go list`; and a `go test -tags=…` invocation does **not** propagate its tag to a child `go list` (only `GOFLAGS`/env does). tellme's only custom tag today is `arch` (the guard's own file, in the exempt `tools/**` tree); the project's other conditional compilation is GOOS-based, which D5 covers. So **custom-tag-gated imports are a recorded out-of-scope residual** (none exist today) — the earlier "respects build tags" claim is **withdrawn**.
...
- **New:** a `-tags=arch` Go guard under `tools/arch/` + a committed `tools/arch/baseline.txt` (8 entries), invoked by a new `Makefile` target `verify-architecture` that is a member of `make verify` (and named in `make help` + `.PHONY`). `techstack.md` (Build & Tooling) records it; `specs/truth/techstack.md`'s Task runner row gains the member.
