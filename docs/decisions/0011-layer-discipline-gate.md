...
**D5 — Host-independence is evaluated as the union over `CROSS_TARGETS`.** `go list {{.Imports}}` resolves in the **host** build context: files excluded by `GOOS`/`GOARCH` land in `.IgnoredGoFiles` and their imports vanish, so a single-context walk is host-relative. The guard evaluates the **union** over the project's supported targets — `linux/amd64`, `linux/arm64`, `darwin/amd64`, `darwin/arm64` (round 020's `CROSS_TARGETS`) — each child `go list` run with an **explicit, sanitised environment** (`GOOS`/`GOARCH` per target, `CGO_ENABLED=0`, `GOFLAGS` cleared) so an ambient build export (e.g. a CI image's `GOFLAGS=-mod=vendor`) cannot fail the gate spuriously — so an OS-gated illegal import cannot hide and the single committed baseline is genuinely host-free.
