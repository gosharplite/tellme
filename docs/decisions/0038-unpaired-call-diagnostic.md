...
> **⚠ Trigger-gated — not open work.** A `§Forward` entry is a decision *deferred to a trigger*, not tasking: **do not re-raise, re-litigate, or propose it as a round theme until its trigger has fired.** An entry marked **TRIGGER-GATED** is inert until then. *(Recorded 2026-09-20, session 51, alongside the `STATUS.md` open-items curation rule — `RF-068-1` had been re-litigated at every bootstrap until annotated.)*

- **RF-068-1** — the diagnostic has no live producer / E2E carrier today (`agentloop.go` is complete); a real carrier would arrive with out-of-order/concurrent dispatch ([#36](https://github.com/gosharplite/tellme/issues/36) item 3). **⛔ RETIRED 2026-09-20 (session 52)** — the operator **re-affirmed sequential tool execution** (truth: *Tool-call concurrency — NOT PLANNED*, [#47](https://github.com/gosharplite/tellme/issues/47)), so the trigger is **settled-off** and `M < N` will have **no** live producer. The diagnostic stays **defensive-only**; **no** E2E carrier is required or expected. Closed, not deferred — do not re-raise.
...
- **RF-068-5** — the `[Tool Warning]` line is named in the **`Chrome` domain entity** (`tellme.modelith.yaml`); naming it on the *Turn chrome (operator)* truth row too is a deferred completeness edit (R-068-F2).
- **RF-068-6** *(fold-verification residual R-068-F1)* — the adapter's **round-closing ORDER** rule (a plain-text turn closes a round; `roundBuilder.textTurn`'s `flush`) has **no wire carrier**: deleting it leaves the gemini pins **and** the tie pin green (the tie pin's `total − unpaired == parts` invariant holds for grouping changes that do not alter the unpaired *set*; a discriminating fixture needs a stray result, whose `extraResults` emission breaks the count invariant). A dedicated wire-order pin is deferred — **a test-fixture-design item, not trigger-gated** (it does not depend on concurrency).
- **RF-068-7** *(fold-verification residual R-068-F3)* — `oneLine`'s fold is unobservable in `TestFormatUnpairedCalls_HostileAndCapped` (sanitize already strips CR/LF); the fold is carried by the sibling pins.
- **RF-068-1 (exit condition) — VOID 2026-09-20 (session 52).** The former condition ("when out-of-order/concurrent dispatch lands, an E2E carrier becomes mandatory") is **void**: the operator re-affirmed **sequential** tool execution ([#47](https://github.com/gosharplite/tellme/issues/47)), so the trigger is settled-off and the diagnostic remains **defensive-only with no E2E carrier**.
