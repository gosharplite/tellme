...
- **RF-068-1** — the diagnostic has no live producer / E2E carrier today (`agentloop.go` is complete); a real carrier arrives with out-of-order/concurrent dispatch ([#36](https://github.com/gosharplite/tellme/issues/36) item 3). **⚠ TRIGGER-GATED — not open work; do not re-raise absent the trigger.**
