...
- The account has **one owner** (`llm.UnpairedToolCalls`), so the adapter's wire behaviour and the CLI's diagnostic cannot drift.
...
- **RF-068-4** — the account is computed eagerly per `Complete` (unneeded memoisation at these sizes).
