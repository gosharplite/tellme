...
**D1 — CPU source = machine-wide, reference-style per-build split.** On macOS under **cgo**, `Sample()` reports machine-wide CPU from the Mach `host_statistics64` `HOST_CPU_LOAD_INFO` tick deltas (`(1 − Δidle/Δtotal) × 100`; `0.0` on the first call — no prior delta). Under **!cgo**, it reports the **process's own** CPU from `getrusage(RUSAGE_SELF)` (`(Δutime + Δstime) ÷ Δwall ÷ NumCPU`, a **real** non-zero figure under load) — matching the reference's cgo-less build's intent.

**D1a — the cgo-less source is `getrusage`, not `runtime/metrics`.** Go's `/cpu/classes/total:cpu-seconds` is documented (`$GOROOT/src/runtime/metrics/description.go`) as the *available* CPU budget (`GOMAXPROCS` integrated over wall time) — an availability figure, not consumed CPU — and on the round's darwin cgo-less host it is **never updated at all** (measured `0` under a saturated core for 3 s; `getrusage` read 3.003 s over the same window). Using it would have re-shipped the operator symptom (`CPU: 0.0%`) on exactly the build the `CGO_ENABLED=0` gate produces, while a truth claim said "non-zero". `getrusage` is cgo-free and `golang.org/x/sys/unix` is already this round's dependency (D2), so the fix costs no new module. *(The `÷ NumCPU` normalisation — R-059-2 — keeps the figure on the same machine-fraction scale as the machine-wide legs; it is not the same quantity as a machine-wide CPU.)*
...
**D7 — The cgo-less macOS CPU is a recorded narrowing of "machine-wide".** On the `CGO_ENABLED=0` build the CPU figure is the **process's own** CPU (`getrusage`, ÷ NumCPU). The `make verify-cross-compile` gate pins `CGO_ENABLED=0`, so it exercises the cgo-less leg only; the accurate machine-wide CPU runs on a normal `CGO_ENABLED=1` darwin build (a darwin release build MUST be `CGO_ENABLED=1` — see §Forward TD-059-1).
...
- **Forward** (RF-059-x): (1) the Mach tick counters are 32-bit `natural_t` and wrap — no rollover handling (mirrors the reference); (2) the cgo-less leg reports process CPU, not the machine's (D7); (3) the sample cache is spinner-local and not re-armed at `OnToolsStart` — a second tool phase starting < 1 s after the previous sample renders the prior phase's figures (bounded ≤ 1 s staleness, by design; R-059-1), and a future non-spinner consumer of the port would re-introduce 5 Hz sampling; (4) `x/sys` is now a direct dependency (keep it in the vendored/module budget); (5) a Windows sampler remains a locked exclusion; (6) **TD-059-1** — nothing compiles the `darwin && cgo` leg on a non-darwin host (`make verify` has no `go test` member and `verify-cross-compile` pins `CGO_ENABLED=0` for all four targets), so the Mach/C file is only compiled by `go test`/`go vet` on a cgo-enabled darwin host; a darwin **release** build MUST be `CGO_ENABLED=1`; (7) **TD-059-2** — the macOS machine-wide CPU is a **darwin-only Mach API with no cgo-free equivalent**; this acceptance has no `INTENTIONAL_NON_FIXES`-style catalog in tellme, so it is recorded here (the round-059 source marker that named that foreign convention is removed); (8) **R-059-3** — the strengthened E2E resource `Then` (≥ 1 frame with non-zero MEM) is host-dependent by construction (a memory-state/container surprise should be read against the DSL-row note, not treated as a code defect).
...
- **`runtime/metrics` for the cgo-less darwin CPU** (the first shape): rejected after review `B-059-1` — the metric is the *available* CPU budget, not consumed CPU, and on the darwin cgo-less host it is never updated (`0` under load), so it re-shipped `CPU: 0.0%` on the gate's own build; `getrusage` (D1a) is the cgo-free, real measurement.
