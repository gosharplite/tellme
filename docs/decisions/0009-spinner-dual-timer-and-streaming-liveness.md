...
- **Harness (round 040 review TD-1)**: `tests/e2e/suite_test.go` gains `Strict: true` **in the implementation half, together with the new stepdefs** — the plan half leaves the harness unchanged so `dev` never goes red (`make test` = `go test ./...`, which **includes** `tests/e2e`). Undefined steps then **fail** the suite rather than being reported-and-ignored, witnessed by the FAIL→PASS transition.
