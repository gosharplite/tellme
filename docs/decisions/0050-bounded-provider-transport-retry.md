...
- **RF-078-5** — **resolved at implementation**: the retry line yields/restores the indicator (no spinner residue; pinned by a **unit** pin on the `[yield, write, restore]` order via the narrow `retryYield` seam, and carried in the E2E by the round-019 *no-residue* Example) and deliberately carries **no** `tellme: ` class prefix (pinned by the unit test + the round-017 *exactly-one-`tellme:`-line* carrier, which the 078 E2E Exercises now exercise on the retry path).
