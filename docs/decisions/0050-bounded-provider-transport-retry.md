...
3. **The schedule and bound are fixed constants, not config (D3).** `retryDelays = {1 s, 3 s}` (an array) → at most **2 retries / 3 attempts**; then the frozen phrase + exit **6**. Pinned by a **literal** unit test (the *order* 1 s → 3 s is a constant-pin, not an observation). A hermetic seam `TELL_ME_FORCE_RETRY_DELAY_MS` (ms; unset/invalid = the real delays) mirrors `TELL_ME_FORCE_TOOLOUTPUT_IDLE_MS`, **scoped to the 078 E2E scenarios** so a retry asserts the request **count** (never wall-clock) without silently retry-enabling any other scenario. *Rejected:* config keys (out of scope for "simple"); exponential backoff + jitter (the reference's policy — not adopted).
...
5. **The retry is announced once per retry, on `stderr` only, and never with the class phrase (D5).** `[HH:MM:SS] retrying the provider request in <delay> (attempt <n> of <max>): <folded detail>` — chrome-styled, control-free, best-effort, emitted through the CLI's terminal-safe line/yield seam (ADR 0014/0015) so it cannot tear the spinner frame; never on `stdout`, never in `turns.log`. It carries **neither** the `tellme: ` class prefix **nor** the frozen phrase — so `tellme: the provider request failed` appears exactly once, on final failure, and the round-017 *exactly-one-`tellme:`-line* contract for a failed request holds. *Rejected:* reusing the class phrase; a `tellme:`-prefixed line (would break the same contract); silence.

6. **Cancellation aborts immediately (D6).** The decorator checks `ctx.Err()` before each attempt, **again before announcing a retry**, and through an interruptible timer sleep; a parent cancellation (SIGINT/SIGTERM) stops at once — and never announces a retry that will not run (a SIGINT mid-request must not print "retrying …"). A **client-side** 300 s timeout (`*url.Error` with `ctx` **not** cancelled) is retryable; a **parent** cancellation (`ctx.Err() != nil`) is not.
...
- **RF-078-5** — **resolved at implementation**: the retry line yields/restores the indicator — the yield/restore witness is the **unit** pin on the `[yield, write, restore]` order via the narrow `retryYield` seam (the round-019 *no-residue* E2E Example is the **non-retry** path, so it carries no-residue only on the sibling path; `plan.md` §6 N-2 states this identically). The line deliberately carries **no** `tellme: ` class prefix — pinned by the unit test + the round-017 *exactly-one-`tellme:`-line* carrier, which the 078 E2E `drop once`/`always drops` Examples now exercise (via `tellme announces on stderr that it is retrying the provider request`).
- **RF-078-6** — the MCP path is unchanged.
- **RF-078-7** — the offline reader paths are untouched.
- **RF-078-8** — **resolved at implementation**: `the retry re-sent the same request` asserts attempt 0's body is byte-identical to attempt 1's (the fake records bodies).
- **RF-078-9** — the retry **policy** is split three ways (class predicate in `domain/llm`, schedule + counter + line in `cli`); consistent with the round-068 decorator precedent and deliberate — a future consolidation into a domain `RetryPolicy` is a consideration, not a defect (review RF-078-4).
- **RF-078-10** — `DropFirst`'s hijack fallback (`panic(http.ErrAbortHandler)`) is unreachable on `httptest` (its `ResponseWriter` is a `Hijacker`); kept defensive with a comment (review N-078-1).
