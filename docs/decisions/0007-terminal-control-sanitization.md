...
- **The ESC consumption is ASCII-gated**, so ESC followed by a multi-byte rune drops only the ESC and leaves the rune intact; the sanitizer therefore **never emits invalid UTF-8** (`"\x1b日本"` → `"日本"`). This was review BLOCKER **B1** (the first cut consumed one arbitrary byte and decapitated the rune).
- **Consumption is bounded** (`escScanLimit = 64`): an *unterminated* CSI/OSC drops only the ESC and continues, so a mangled/binary fragment cannot swallow a whole line's visible text (review **RF-2**).
...
- **Recorded residual risk:** the scanner is deliberately conservative; an exotic multi-byte escape it does not model is consumed by the bounded scanners (or dropped as a stray ESC), and the D4 restore bounds the terminal-state risk.
