...
- **Recorded residual risk (review N-6):** the scanner is deliberately conservative and **window-bounded** — a sequence it does not model, or whose terminator lies **beyond its kind's window** (CSI 128 / OSC 1024), drops only the ESC and passes the remainder through as visible text, so this residual is *bounded by the window* (≤ 1024 bytes) rather than removed; the D4 restore bounds the terminal-state risk.
