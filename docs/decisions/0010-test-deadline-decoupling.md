...
**Provenance lesson (the most transferable part).** The phrase *"the production fast-fail constant (2 s)"* **entered via issue [#87](https://github.com/gosharplite/tellme/issues/87)'s root-cause refinement comment** (`5713960582`, 11:58) — *"it uses a production fast-fail constant (2 s) as its own budget"* — and was then **ratified unverified** into the round's premise, never questioned against the code (production's bound is 3 s and was **never** 2 s; the 2 s literal mirrors the sibling `ghWaitDelay`). The round's own method (*"measured, not hypothesised"*) is what ultimately caught it, in review. The lesson is therefore one level *up*: **a reviewer's claim is not evidence either** — a handed-down constant must be verified against the call site **before** it is ratified into a round's premise. A constant is a **claim**, and a claim about production belongs verified against the call site, not inherited from a sibling literal or a reviewer's summary.
