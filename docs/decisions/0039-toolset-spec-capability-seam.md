...
- **RF-069-5** — the ceiling **wiring** (`assembleAgentTools` → `NewReadImageTool`) is carried only by the E2E `reading-a-local-image` journey; the unit pins cover `ImageCeilingForFamily`'s owner table, not its consumption. A registry-tier unit pin on the wiring is a deferred strengthening (pre-existing; surfaced by this round's witness (b)).
