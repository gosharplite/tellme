...
**D5 — Scope: unconditional for *ambient* input — all targets.** The block applies to `build`, `fmt`, `vet`, `tidy`, `test`, `lint`, `staticcheck`, `vulncheck`, every `verify-*`, and `verify`; there is no per-target opt-out. It is **unconditional for ambient *Go-env* input** (one exception — make's `-e` mode, which resurrects the two `export`ed names: **R6**); the deliberate **command-line** form (`make GOENV=<file> …`) is the escape hatch (D8/R3) — that is the opt-out, so no separate `HERMETIC=0` switch is introduced. The two **mutating** targets (`fmt`, `tidy`) are covered and verified harmless (D7).
