...
| [0027](0027-tool-chrome-colour-and-payload-delta.md) | Tool-chrome colour accents (grey `[Tool Output]` frame + yellow `[Tool Action]`, round-054 gate) + `argValueCap` **189 → 500** + a signed pre-flight payload increment (in-memory, session-scoped) | Accepted |
