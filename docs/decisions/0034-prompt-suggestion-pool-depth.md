...
- **RF-064-5** — **Depth constant review**: if the reference (or the operator) moves `LoadTopN`'s 50, tellme's single `promptPoolDepth` follows; a periodic parity check is a candidate. *(Review R-4 recorded that the pair is guarded qualitatively — witness (a) catches `depth ≤ 10` and `depth > cap`, but nothing caught a silent `depth = 51`. The attestation is now a **literal pin** `promptPoolDepth == 50` in the unit pin — the round-057 `argValueCap = 500` pattern — so the "follows the reference" claim names its binding.)*
