...
- **banner**: the target prints `SUBSET — NOT THE GATE` naming the selected paths and the gate command; the **harness** also prints that banner whenever a selection is supplied (visible under `-v`, and always on a refusal — `go test` suppresses a package's output on success).
- **guard (on RESOLVED paths, in the HARNESS)**: `tests/e2e/suite_test.go` `guardSelection` resolves a supplied selection to the **set of `.feature` files it contains** and **refuses** one that **covers** the whole contract (**equality or superset**) — so a traversal spelling (`…/cli/../cli`), an all-modules list, a root-plus-extras selection, and the parent `…/features` dir are all caught, and **both** entry points (`make test-fast` *and* a hand-typed `go test -args -godog.paths=…`) obey the invariant (round-055 reviews **B-055-1** + **R-055-1**). The `Makefile` assembles the selection, refuses an empty list or a missing module, and pre-refuses a module that **resolves to the root** (so the refusal precedes the banner — review **R-055-4**); the previous string-equality clauses were removed as dead/mislabelled (review **TD-055-1**). *(Recorded residual **R-055-4b**: a union/parent spelling such as `E2E_FAST_MODULES=".."` still prints the banner **before** the harness refuses, because the Makefile's per-module pre-check cannot see a whole-set union without duplicating the Go guard in shell — which would create two authorities for one invariant; the run still **fails**, so the invariant holds, and the single authority stays the harness.)*
...
**D5 — the measurable bar (operator-locked).** The full gate MUST be **≤ 60 %** of the paired serial baseline on the same host/session (observed **33–47 %** across sessions — the `tests/e2e` ratio is stable at ~33 %, the paired full gate is load-sensitive (38 % here, 47 % under concurrent review load); `research.md` D5 owns the numbers); a loose absolute **ceiling of ≈25 s** is recorded here as a **sanity backstop only** (an absolute bar goes born-stale on a slower machine — ADR 0010); stability = **N = 5** green runs.
