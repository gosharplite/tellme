...
- **RF-52-3** — the caller-trust cursor is **total, not validated**: `set(items, cursor)` trusts its caller, but `selected()` is total (`cursor < 0 || cursor >= len(items)` ⇒ `""`) and `view()` highlights nothing — **no panic**. The **upper** bound (`cursor >= len(items)`) is **pinned** at the `suggester` tier (where `set` lives): `suggester_set_test.go` sets a **non-empty** list with an out-of-range cursor (`set([]string{"alpha"}, 3)`) and asserts no selection + **no cursor row** in `view()` (round-052 fold **F-52-3** — the original `set(nil, 3)` input was **vacuous**, short-circuiting on `len(items) == 0` before the cursor was consulted). The lower bound (`cursor < 0`) is already carried by every `noChoice` case. Deleting the upper bound reds it with `index out of range [3] with length 1`.
