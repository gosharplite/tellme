...
So today `make verify` is silent about, e.g., a new `internal/cli → internal/telemetry`, `→ internal/agent`, or `→ internal/ui` import — none violates RULE-A (not upward), RULE-B (not `infrastructure`), RULE-C (not domain), or RULE-D (all ranked). Only a human reviewer would notice. [#101](https://github.com/gosharplite/tellme/issues/101) records this as the R5 ambition and names its precondition: a **new rule** plus a **new ADR**, shipped **with its enablers**.
...
**D6 — What the rule is *not*.** RULE-E constrains only a governed application tier's imports of **`internal/**`** packages. It does **not** constrain: (i) **stdlib** imports; (ii) **third-party module** imports (out of scope — a **recorded residual**; no application-tier package imports a third-party module today); (iii) imports made by **non-application** tiers (`internal/agent`, `internal/ui`, `internal/infrastructure/**`, `internal/config`/`home` — governed by RULE-A/B/C); or (iv) `cmd/**`, `tests/**`, `tools/**` (exempt trees). RULE-E is **not** a blanket "nothing may import anything but the domain".
...
`internal/app/**` has **no** RULE-E violation (it imports only `domain` + the sanctioned `config`); `internal/cli → internal/config|home|app/**` and `internal/app/suggestions → internal/config` are **sanctioned**. ⇒ **RULE-E baseline = 3**, re-measured from the gate at implementation.
...
- **Recorded residuals:** third-party application-tier imports (D6); custom build-tag-gated imports (ADR 0011 **D6**); the 3 edges are the R5 de-coupling workstream; F-4/F-6/F-7/F-8 ([#101](https://github.com/gosharplite/tellme/issues/101)).
- **Forward item:** the sanctioned set may be **re-ruled** by a later round (e.g. `config`/`home`/`suggestions` become injected plain data) — a sanctioned-set edit + an ADR 0016 amendment, not this round's.
