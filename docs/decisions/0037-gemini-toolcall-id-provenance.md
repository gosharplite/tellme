...
**D3 — Cross-family scope: Gemini-local by construction; the OpenAI-compatible wire is untouched.** The id value flows, at the loop, into `llm.Message.ToolCallID` — which the OpenAI-compatible adapter serializes as `tool_call_id`. The concern that a shared convention would change that wire resolves **without any loop/port change**, because **one session drives exactly one family**: the adapter whose `parseResponse` produced the id is fixed for the session, so the Gemini adapter's change can only feed the Gemini adapter's own next request. An OpenAI-compatible session's ids come from the **OpenAI adapter's** `parseResponse` (which already prefers the provider's id). Therefore **no OpenAI-compatible wire changes** and **I-1 holds by construction**. The unification alternative (a single id convention across families) is **not** taken — it would couple the families for no gain.

**D4 — The unpaired calls of an `M < N` round are made observable (a single-owned accessor).** `roundBuilder` gains `unpaired() []string` — the ids, in call order, of the round's calls that received no result — computed at the **one** place the drop happens (`flush()`), so the accounting cannot drift between call sites. It is surfaced as a **returned value** (`UnpairedCallIDs(prior []llm.Message) []string`, a documented package-level accessor), **not** a `stderr`/`[Tool …]` diagnostic (the adapter owns no logging seam; a diagnostic is a forward item). The emitted batched turn is **unchanged** — it still carries the `M` parts the round produced (ADR 0035's recorded shape). Round 067 makes the drop **observable**; it does not change the wire.

**D5 — RF-066-8 is retired.** With exact-identity accounting, a pin asserts the **identity** of the unpaired call (`UnpairedCallIDs` for a 2-call/1-result round returns exactly the unpaired call's id, call order) — a mutant that drops the wrong call changes the returned set and reds the pin. The `M == N/2` arithmetic-equivalence gap no longer applies to the accounting claim.
...
- **The boundary drop is no longer silent**: the unpaired call ids are observable through a single-owned accessor; the round-066 residual (**RF-066-8**) is retired by an exact-identity pin.
- The **OpenAI-compatible wire is untouched** by construction (single-family sessions); the round-066 batched shape and the media-free text path are preserved.
...
- **RF-067-1** — the unpaired-call observability is a **returned-value accessor**; a user-visible `[Tool …]`/`stderr` diagnostic is deferred (no adapter logging seam).
