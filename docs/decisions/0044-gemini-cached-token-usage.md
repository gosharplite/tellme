...
- **Live (2026-09-20, session 59 cont. — PERFORMED AND PASSED)**: a real Vertex/Gemini turn through the `coder` peer on the `dev` provider (`gemini-3.8-flash`, project `websc-dev-433809`), using the `dev`-built binary (`go version -m` → `vcs.revision=c76ec06`, `vcs.modified=false`). A two-call session sharing a ~31.7k-token prefix: turn 1 `M: 31695 H: 0 C: 3 Th: 234` ($0.02466); turn 2 `M: 4160 H: 27661 C: 10 Th: 5131` → **$0.024473325** — a **non-zero `H`** (27,661 cached tokens; 43.5 % session hit ratio) with the reused prefix billed at the HIT rate (0.075) instead of MISS (0.75). Counterfactual (pre-fix: cached ignored ⇒ the whole prompt at MISS) = **$0.043144500**, so the witnessed turn is **~43 % lower**. `tokens.log` records the same `cached_tokens` (0 then 27661); `tokens.summary.json` = `{miss:35855, hit:27661, out:5378, cost:0.049133325}`. Evidence (transient, `/tmp/tellme-072-live/`): `a.err`/`b.err` (the metrics lines), `turns.log`, `a.out`/`b.out`.
