...
- **Witness**: the gate (`make verify`) + the identifier-count check (`internal/cli` production `→ agent` refs = **0**) + the `internal/cli` turn unit pins. RULE-E baseline **1**, 0 new / 0 stale; RULE-A/B/C **0**; 0 cycles; RULE-F `→ ui` **19** identifiers (0 new / 0 stale), no `→ agent` key; cross-compile 4/4.
  - **The real-loop rendering/ordering claim now has a NEW owner** (round-050 fold TD-2(iii); corrected by fold **F-1**): after Q4 → A the `internal/cli` turn tests drive an **in-package fake `agentport.Loop`**, so they no longer integrate the real loop. The real loop's **rendering/ordering** behaviour is owned by, in layers: the **loop schedule / hook ordering** → `internal/agent` unit tests (`agentloop_*_test.go`, `newloop_test.go`); **the bytes** (`[Tool Reason]`/`[Tool Result]`/`[Tool Action]`, caps, sanitization) → `internal/ui` (`toolrenderer_test.go`, `toolcall_reason_test.go`, `toolcall_sanitize_test.go`); **the CLI call renderer's tail/blank rules in isolation** → **`internal/cli/call_renderer_reason_test.go`** (it drives the renderer **directly**, not through the fake loop); and **end-to-end** → the godog E2E. `internal/cli`'s turn tests own its **orchestration** (Result/error handling, persistence, stream order). A future round MUST NOT read "the existing turn unit pins" as still covering the real loop integration.

## Forward

- **Standing rule — `LoopSpec` stays a capability contract, not a settings file (round-050 fold RF-1).** Every `LoopSpec` field MUST name a **domain-typed capability** the loop needs (a gateway, a registry, a sink, a renderer, an observer, a clock, a writer, two ints). A future **implementation-only** knob (retry policy, concurrency cap, debug flag) MUST NOT be added to `LoopSpec` for an implementation reason; it travels by another mechanism, or this ADR is amended first — otherwise the domain tier accretes a 20-field config bag every caller must populate.
- **Standing invariant — the loop's `Registry` identity, and `BindToolOutput` before `Run` (round-050 fold TD-3).** The CLI builds the loop with `Registry: reg` and then calls `dp.BindToolOutput(reg, …)`, i.e. it **mutates the registry after the spec was consumed**; the loop sees the mutation only because the factory shares the same object. The invariant is therefore: **the `LoopSpec.Registry` MUST be the same object the composition site binds, and `BindToolOutput` MUST run before `Run`.** A future adapter that derives, wraps, or copies the registry would silently drop the `[Tool Output]` sink. Upheld by review discipline only (like `cmd/tellme`'s single-assembly-site).
