...
- **Witness**: the gate (`make verify`) + the identifier-count check (`internal/cli` production `→ agent` refs = **0**) + the `internal/cli` turn unit pins. RULE-E baseline **1**, 0 new / 0 stale; RULE-A/B/C **0**; 0 cycles; RULE-F `→ ui` **19** identifiers (0 new / 0 stale), no `→ agent` key; cross-compile 4/4.
  - **The real-loop rendering/ordering claim now has a NEW owner** (round-050 fold TD-2(iii)): after Q4 → A the `internal/cli` turn tests drive an **in-package fake `agentport.Loop`**, so they no longer integrate the real loop. The real loop's tool-line **rendering/ordering** behaviour is owned by `internal/agent`'s unit tests + `internal/ui`'s `call_renderer_reason_test.go` + the godog E2E, while `internal/cli`'s tests own its **orchestration** (Result/error handling, persistence, stream order). A future round MUST NOT read "the existing turn unit pins" as still covering the real loop integration.
