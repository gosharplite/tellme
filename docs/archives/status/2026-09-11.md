# tellme — Status archive: 2026-09-11

> **Archived history — split out of `STATUS.md` on 2026-09-11 (session 13).** When the live-state file
> was split into a lean current-status file plus an archive, this file received the historical record
> verbatim: the delivered round-001 section, every closed grill round (#1–#6), the upstream resolution
> records (`aixbdd-tmg` CLI-seat, R1/R2, grill-#5 methodology), the clarify round, the feasibility pass,
> the accumulated *decisions locked this session* log, the closed PM follow-ups, the branch-model
> propagation history, and the past environment notes.
>
> The **current** live state lives in [`STATUS.md`](../../../STATUS.md).

## Branch-model propagation history
...
- [x] `docs/decisions/0001-project-language.md` + `docs/decisions/README.md` — project-language declaration (**named home**, English; upstream PR #8 / R2)
...
- **Session-continuity tooling**: `SESSION-BOOTSTRAP.md` gains **Step 8** (read the last 5 days of
  `docs/session-summary/<YYYY>/<MM>/<DD>/session-summary.md`); new **`SESSION-CLOSEOUT.md`** defines the end-of-day procedure
...
- **Project-language declaration home** (session 8, R2): `docs/decisions/0001-project-language.md` declares **English** as tellme's artifact language — the **named home** the upstream Project Language clause requires (previously recorded only in `STATUS.md` / the daily log). The fixed §4/§5 DSL meta-schema tokens are unchanged.
...
## Environment notes (archived history)
