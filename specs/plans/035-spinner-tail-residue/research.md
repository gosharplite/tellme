...
- **Literal line-count coverage (recorded candidate, PR #73 review F3).** SC-002 is carried by the whole-stream residue row only, and that narrowing is **permanent, not deferred**. Consequence stated for the record: a *future* regression where a `\r` inside a reason (or a lost newline) collapses two grouped reasons onto one visible row would **not** be caught by the residue row. If that property ever matters, its natural carrier is a literal count row in the owning `chat` module (a feature change ⇒ an audit re-run) — recorded here, not parked in a new home.
