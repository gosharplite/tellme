...
- The failed-turn display-only `Ready` overstatement (**G2**) and the per-call numbering skew (round-034 recorded divergences). **The G2 item also couples to round 035** (PR #73 review TD-2): `final` — which already gates the deferred tail (ADR 0005 G5) — is now load-bearing for the yield too, and the loop fires the call-end hook with `final=false` on **two** distinct paths (the continuing call, `agentloop.go:192`, and the terminal bound-reached failure, `:150`). The coupling is benign today (both want the same clear, and the bound-reached path gains a clean tail), but a round that defers a failed-turn tail must **re-check the round-035 phase-boundary yield** for that path. Recorded so G2's round owns the re-check; the numbering skew touches the same keying.
...
- **SC-002**: The grouped `[Tool Reason]` lines in a call's tail begin on their own line, witnessed **through** the whole-stream residue row (SC-001): the reported defect glued the tail's **first** grouped reason to the live spinner frame, so a finished capture with no braille frame + phase status is exactly what guarantees it. A *separate literal* `occurrences == lines` count assertion is **deliberately not carried — a permanent narrowing, not a deferral**: the residue row *is* the carrier (a flat capture reads the collision as the shared row), so no new row is minted for a display-only defect (US1/FR-002; PR #73 review M-1/F3).
