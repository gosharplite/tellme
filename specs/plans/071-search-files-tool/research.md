...
| **D4** | **Bound = the round-024 byte budget (primary) + a degenerate 100-match cap + a 500-char per-line trim** (clarify Q3 → A). Matches sorted **path asc, then line asc** (the one deliberate divergence from the reference's worker-dependent order). | Reuses the `read_files` "cap *and* budget" precedent; determinism is the testability axis. `spec.md` I-2/I-4. |
...
| **D7** | **Recorded in a new ADR (0043)** with the three divergences (no security gate · no workspace policy · deterministic order) and the design-intent justification. | Durable decision record; ADR governance (guard `verify-adr-index`). |
