...
- **Decision**: The concept name "current choice" stays. The at-rest `Then` inverts to *the interactive prompt **marks no** suggestion as the current choice* (exactly **zero** `>` cursor rows **in the at-rest frame** — the first painted frame, before any key is delivered). The after-`Tab` selection is **not** given a second acceptance rule — `prompting-with-suggestions.feature`'s existing accept journey (`… accepts the current suggestion` → *the interactive prompt holds the accepted suggestion "{text}"*) continues to carry *insertion* of the selected text.
