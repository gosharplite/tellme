...
> **Review fold (PR #94 architect, B-1/B-2 · TD-1…TD-4 · RF-1…RF-3; then the fold-review's R-1…R-6/R-8):** the first draft of D1 stated a **one-part** rule ("any governed package's import of a higher tier") that **cannot reproduce the 7 `cli → infrastructure` entries** it claimed as its baseline (they are *downward* imports) — so the baseline was not derivable from the rule. D1 is now a **two-part predicate** (see D1; ADR 0011 **D2**). D2's "respects build tags" claim was **false** and is **withdrawn** (see D6). Host-independence is the **`CROSS_TARGETS` union** (see D5). **D7** adds **ADR 0011**. The enumeration anchor is **D4**; **D8** default-deny; **D9** one normative source; **D10** baseline format; **D11** the acyclicity assertion; **D12** the BDD-techstack stance. *(This note previously mis-numbered these — the fold-review's R-3.)*
