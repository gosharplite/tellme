...
- **Q2 → resolved by measurement (RD-recordable).** Whether the gate governs `_test.go` imports and the `tests/**` harness **does not change the baseline**: measured 2026-09-17, **no** `internal/**` test file imports anything upward, and the only outward harness importer is `tests/e2e` (+ `tests/e2e/steps`), which is test-support. **Scope is therefore pinned as**: the gate governs **production `internal/**`**; `_test.go` imports in `internal/**` are also governed (they add no entries); **`cmd/tellme` (composition top)**, **`tests/**` (test-support) and **`tools/**`** (the guard's own tree) are **unranked/exempt**. Pinned in `research.md`.
...
> **Ratchet refinement (from Q1)**: [#92](https://github.com/gosharplite/tellme/issues/92) AC1 says the count "decreases as **R2** lands (→ 0)". Under the **two-part predicate** the 8th entry (`agent → ui`, the RULE-A violation) is **R3/R4's** workstream, so the ratchet reaches **0 across R2–R4**, not R2 alone. R2 removes the 7 RULE-B `cli → infrastructure` entries; R3/R4 removes the RULE-A `agent → ui` entry. Recorded so AC1 is read correctly.
...
- **FR-004**: The gate MUST be **hermetic and host-independent**: it MUST evaluate the import graph as the **union over the supported targets** (`linux/amd64`, `linux/arm64`, `darwin/amd64`, `darwin/arm64`) so an OS-gated illegal import cannot hide, and MUST run each child `go list` with the **inherited environment filtered** — a **drop/neutralise set** (`GOOS`/`GOARCH`/`GOARM`/`CGO_ENABLED` set or cleared per target; `GOFLAGS`/`GO111MODULE`/`GOEXPERIMENT`/`GOWORK` neutralised) so an ambient build export (e.g. `GOFLAGS=-mod=vendor`, a CI image's `GOWORK`, or `GO111MODULE=off`) cannot make the gate fail spuriously, **and a preserve set** (`PATH`/`HOME`/`GOPATH`/`GOMODCACHE`/`GOCACHE`) so the warm module cache the aggregate relies on is still found (an `env -i` child fails with *module cache not found* — the `verify-cross-compile` **PR #46 TD1** precedent, generalised). It MUST NOT use the network with a **warm module cache** (the `verify` aggregate runs it after `vet`/`verify-cross-compile`, which warm the cache). It MUST add no dependency or tool beyond the Go toolchain and `make`.
...
- **`_test.go` imports and the `tests/**` harness** → `internal/**` test files are governed (they add no entries); `tests/**`, `cmd/tellme` and `tools/**` are exempt (scope pinned in `research.md`).
- **OS/build-tag-gated files** → the gate MUST evaluate the **union over the supported targets** (`CROSS_TARGETS`) so an OS-gated illegal import (`//go:build linux`/`darwin`) cannot hide; **custom build-tag**-gated files (e.g. `//go:build arch`) are a **recorded out-of-scope residual** — the only custom-tagged file is the **guard itself** (`//go:build arch`), in the exempt `tools/**` tree (`research.md` D6) — not a claim that "build tags are respected".
