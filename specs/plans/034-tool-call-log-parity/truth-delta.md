...
| MODIFY | `specs/truth/techstack.md` — **Turn chrome (operator)** row | The chrome **cadence** becomes a status **frame per AI-endpoint call** (rule + `╭─⠿ Turn N - <mode>` + pre-flight payload), with the frame number advancing within a prompt (`prior-calls + k` — G6); the round-018/027 line **formats** are unchanged (Q4). **Discovery-ordering reversal (round 032 F9):** the first frame is emitted by the call-begin hook, whose pre-flight estimate must count the MCP-discovered tool set, so MCP discovery now runs **BEFORE** the first frame — the `╭─⠿ Turn N` header **follows** discovery, and during a slow/unreachable server's bounded fast-fail the pre-discovery signal is the `Input captured. Processing...` line, not the turn header (a recorded divergence from round 032's "discover after the turn chrome"). | FR-008, FR-009; G5/G6. |
