...
**Recorded ADR**: `docs/decisions/0005-tool-call-log-parity.md` (observer-port extension, the CLI-computed per-call estimate, the per-call cadence + final-call deferral + the display/persistence divergence, and the sorted-keys / `json.Number` / rune-cap divergences).
...
- **Recorded divergences** (all documented above): the decomposed `[Tool Engine]`-family shape (round 022's reversal); the rune-cap vs byte-cut + U+2026 vs ASCII dots; sorted-keys + `json.Number` vs map-iteration + `%v`; the bounded-and-stopped stream vs the reference's un-terminated `warnWriter`; the per-call cadence + final-call deferral + the display/persistence divergence; `output_file` → no block; trailing partial line dropped; the `Step i/M` executed-round unit vs the reference's call index; and the round-032 F9 **discovery ordering** — with the per-call cadence the `╭─⠿ Turn N` header now **follows** MCP tool discovery (the first frame's pre-flight estimate must count the discovered tool set, so the header cannot precede discovery), so during a slow/unreachable server's bounded fast-fail the pre-discovery signal is the `Input captured. Processing...` line, **not** the turn header (round 032's "discovery after the chrome" guarantee is superseded — see `truth-delta.md`).
