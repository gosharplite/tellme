...
## Round-039 fold review (PR #81) — TD-4 + process + hardening

- **TD-4** [pre-freeze] — corrected the artifacts that **caused** B1 (the **requirement**/decision record, not just the downstream ones): added the tool-round condition to `spec.md` **FR-008** and **US2 scenario 3**, qualified **SC-003**, and recorded the refinement on operator-locked **Q8** (*"tool-using turns only … refined by review B1"* — recorded, not silently rewritten). `research.md` D5 row 3's Rule + Mechanism now describe the shipped code (`renderedToolRound` marker, set on a non-final `OnCallEnd`), removing the FR-008 ∥ FR-009 contradiction before the package freezes.
