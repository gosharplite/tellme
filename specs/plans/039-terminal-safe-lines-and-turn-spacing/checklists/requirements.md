...
**Note**: two `stderr`-presentation workstreams. `/axb-spec-by-example` is **NOT** a NOOP (both are user-visible). `/axb-ui-plan` is **skipped** (no chrome change). The load-bearing downstream steps are `/axb-technical-research` (a `techstack.md` MODIFY + an **ADR 0007** amendment + the reference divergence check) → `/axb-system-analysis` (1 CLI end → `/axb-dsl-refine`; api/data NOOP) → `/axb-dsl-refine` (a sanitized-siblings Rule + a blank-line Rule + `chat/dsl.md` rows) → `/axb-tasks` → `/axb-implement` (unit + E2E pins).
