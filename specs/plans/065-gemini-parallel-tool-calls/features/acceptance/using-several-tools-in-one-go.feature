Feature: Using several tools in one go with a Gemini provider
  ...
  Rule: A Gemini turn that asks for several tools in one go still finishes
    ...
    Example: The model reads two files in one go and answers about both
      ...
      And the workspace holds a file "first.txt" whose content is "the first note"
      And the workspace holds a file "second.txt" whose content is "the second note"
    ...
    Example: The model reads three files in one go and the turn still finishes
      Given the operator has a runnable tellme installation
      And the runtime home is "ait-tmg"
      And the workspace holds a file "a.txt" whose content is "note a"
      And the workspace holds a file "b.txt" whose content is "note b"
      And the workspace holds a file "c.txt" whose content is "note c"
  ...
  Rule: A Gemini turn that asks for several tools in one go still shows the model every picture
    ...
    Example: The model reads two pictures in one go and the answer reflects both
      Given the operator has a runnable tellme installation
      And the runtime home is "ait-tmg"
      And the workspace holds an image file "left.png" that is a small PNG picture
      And the workspace holds an image file "right.png" that is a small PNG picture
  ...
  Rule: A Gemini turn that asks for a single tool is unchanged
    ...
    Example: A single file read still finishes as before
      Given the operator has a runnable tellme installation
      And the runtime home is "ait-tmg"
      And the workspace holds a file "only.txt" whose content is "the only note"
  ...
  Rule: A Gemini turn that reads several pictures in one go does not lose any of them
    ...
    Example: Two pictures whose content is the same kind are both carried
      Given the operator has a runnable tellme installation
      And the runtime home is "ait-tmg"
      And the workspace holds an image file "one.png" that is a small PNG picture
      And the workspace holds an image file "two.png" that is a small PNG picture
