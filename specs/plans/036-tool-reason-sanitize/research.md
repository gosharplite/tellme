...
- **Decision**: A `reason` that is empty **or whitespace-only after folding+trimming** emits **no** `[Tool Reason]` line. The suppression is evaluated at the two call sites (`logAction`, `renderCallTail`) with a `strings.TrimSpace(reason) != ""`-style guard, replacing today's raw `reason != ""` guard. `FormatToolReason` is **not** asked to return an empty-string sentinel.
...
- **Permanent E2E narrowing (D4)** — the collision stays outside the E2E surface; the unit hostile-fixture pin is the deterministic carrier. Recorded so a future reader does not read the coverage as complete.
