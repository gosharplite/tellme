...
- **`/axb-spec-by-example` scope.** This is an implementation-vs-truth fix: the one-line promise is a contract and the violated guarantee is a code behaviour, not a PM journey — `/axb-spec-by-example` is expected **NOOP** (recorded as such).
