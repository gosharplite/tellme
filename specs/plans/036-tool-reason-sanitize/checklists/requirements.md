...
**Note**: a presentation-contract bug fix — `/axb-spec-by-example` is **NOOP (executed 2026-09-17)** (the one-line promise is a contract; the violated guarantee is a code behaviour, not a new PM journey; a new plan-side acceptance rule would have no `InterfaceFeature` carrier under the Q1 narrowing and break `acceptance-coverage`). The load-bearing downstream steps are `/axb-technical-research` (cap-value ratification + the ADR-0005 divergence record + the `oneLine` home) and `/axb-dsl-refine` (the owned `chat/dsl.md` reason-row prose + cap-constant note), then `/axb-tasks` → `/axb-implement` (hostile-fixture Red → Green → Refactor).
