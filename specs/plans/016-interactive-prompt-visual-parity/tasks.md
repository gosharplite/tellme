...
- Phase 3 `Test Alignment & Implementation` 在寫產品碼之前，先把本輪所有受影響的自動化測試對齊最新版 truth（本輪 10 句為全新句 `ADD`；2 句為退役 `DELETE`）。
...
## Round-016 locked decision (implementation constraint — MUST)

> 來自 issue [#39](https://github.com/gosharplite/tellme/issues/39)（operator 拍板）。

- **[STRICT PARITY] 提示面與 `tell-me-go` 一致** — `-i` 提示只呈現**有框線的多行編輯器**（`lipgloss.NormalBorder`、fg `240`、固定高度 10、寬度隨終端、reference placeholder、無行號）與在其下方的**有樣式建議清單**（`Suggestions:` 標頭；選中列 bold fg `205` / bg `235`、未選中 fg `245`）；**移除** dashboard 標頭、**移除**底部狀態／keybinding 列、**無** `?` help overlay（提示語在 placeholder 內）。見 T003、T009、T021、T022、T023。
- **[INTERACTION PARITY] 建議刷新與接受** — 建議刷新**去抖動（debounce）且可取消**（stale fetch 不得覆寫較新結果），>3 行的建議**不得提供**；`Tab`/`Shift+Tab` **插入**選中建議（多詞且建議為單一 token 時只換最後一個 token）。見 T003、T011、T013、T014、T015、T017、T019、T020。
- **[RESPONSIVE] 終端寬度** — 處理 `tea.WindowSizeMsg`，編輯器寬度 = `msg.Width - 4`；極窄寬度不得 panic。見 T003、T012、T016、T021、T022。
- **[NO REGRESSION] 單一來源** — `stdout` 維持 byte-exact（TUI 只寫注入的 `stderr`）；keybindings 不變；shared log、建議來源、非 TTY fallback、round-012 plain reader 不變；class-phrase 詞彙維持 **11**。見 T022、T023、T025。
...
- [ ] T001 建立 12 個 stepdef 獨立檔骨架（10 新句 + 2 退役句；Zero Shared Edits）
  - Read:
    - `specs/truth/features/cli/chat/dsl.md`（本輪 10 個新句 + 2 個退役句）
    - `tests/e2e/steps/register.go`、`tests/e2e/harness/`（`TELL_ME_FORCE_STDIN_TTY` seam + scripted stdin）
  - 只做：建立 12 個獨立 stepdef 檔（各自 `init()` 自我註冊空白 registrar），檔名對應 Phase 3 task id：
    ...
    - `tests/e2e/steps/step_t015_chat_when_accept_suggestion.go`
...
- [ ] T003 落點 `internal/ui/tui/prompt/` 的 reference chrome tokens 與 debounced refresh seam 骨架
  - Read:
    ...
    - `specs/plans/016-interactive-prompt-visual-parity/ui/ui-plan.md`（terminal 視覺方向；`entry` / `10-suggestion` / `20-composed` / `30-narrow` frames）
  ...
  - 不做：不寫 chrome 渲染結果、不寫 debounce／insert 行為、不寫 resize 邏輯、不移除 dashboard（該動作為 T023）。

---

## Phase 3: Test Alignment & Implementation

**Goal**: 把本輪 Feature 用到的 DSL 自動化測試對齊最新版 truth；本輪 10 句為全新句（`ADD`），2 句為退役句（`DELETE`）。不寫產品行為。
...
- 本輪 chrome Then（`framed`／`lists suggestions beneath`／`marks one suggestion …`／`no session metrics header`／`advertises … keys`／`keeps the typed text`／`frames the editor within the terminal width`）依 `dsl.md` 是對「captured output 之 **presence／absence**」斷言（非精確 ANSI bytes）。

**Markers**:
- `[BDD-ALIGN]`：**無**（本輪未 reword 既有句）。
- `[BDD-REMOVE]`：本輪 **2 句**（退役的 dashboard Thens）。
- `[BDD-RED]`：本輪 **10 句**（`ADD` 的 DSL rows）。
...
**Parallel Hint**:
- T004–T017 各派一個獨立 subagent（依 `ParallelHint平行Subagent與同檔調度判準.md` 調度；目標檔案互斥，符合 Zero Shared Edits）；T018 等全部回來再啟動 subagent 執行 review。
...
- [ ] T009 [P] [BDD-RED] `Then: the interactive prompt shows no session metrics header`
  ...
  - 語意：captured output **不**帶 session metrics 標頭（無 `provider … | tokens … | turns …` 列）——strict parity。
...
- [ ] T011 [P] [BDD-RED] `Then: the interactive prompt keeps the typed text "{text}"`
  ...
  - 語意：captured output 於編輯器內帶所打字串 `{text}`。
...
- [ ] T015 [P] [BDD-RED] `When: the operator opens the interactive prompt, types "{query}", and accepts the current suggestion`
  ...
  - Landing: `tests/e2e/steps/step_t015_chat_when_accept_suggestion.go`
  - 語意：以 `TELL_ME_FORCE_STDIN_TTY=1` 與 scripted key（open → type `{query}` → `Tab` accept → abort）跑 `tellme -i`，captured 結果。

### UNIT（非 DSL 的單元斷言）

- [ ] T016 [P] [UNIT] TUI 模型：chrome 存在／無 dashboard／resize 寬度／`Tab` 插入（含 last-token）
  - Read:
  ...
  - 撰寫：以注入 I/O 直接驅動模型，斷言——渲染含 editor 框線／placeholder／`Suggestions:` 標頭／`>` cursor；渲染**不含** dashboard 列；`tea.WindowSizeMsg` 設編輯器寬度為 `msg.Width - 4`；`Tab`/`Shift+Tab` **插入**選中建議，且多詞輸入 + 單一 token 建議只換最後一個 token。
  - 落點：`internal/ui/tui/prompt/model_chrome_test.go`。

- [ ] T017 [P] [UNIT] 建議刷新：debounce + cancel（stale 丟棄）+ >3 行 drop
  - Read:
  ...
  - 撰寫：查詢變更後刷新經 debounce（不每次按鍵即刷新）；被較新按鍵取代的 fetch 結果被丟棄（不覆寫較新清單）；>3 行的建議被過濾。
...
- [ ] T018 subagent review (phase quality gate)
  ...
  - 檢驗：無 undefined step；10 個新句 stepdef + 2 個 `[UNIT]` 皆已註冊；退役 2 句的 stepdef 已移除；測試可編譯；失敗僅限尚未實作之產品行為（chrome 未渲染、dashboard 未移除、debounce／insert 未實作、resize 未接線）。

---

## Phase 4A: MODIFY Feature File - cli/chat/prompting-with-suggestions.feature

**Goal**: 讓 `prompting-with-suggestions.feature` 全綠 — 額外涵蓋「接受建議會插入編輯器（last-token 規則）」與「>3 行建議不提供」，維持既有三來源建議。
...
- [ ] T019 [BDD-GREEN] 讓 Test Scope 全綠（並使 T017 的刷新 `[UNIT]` 轉綠）
- [ ] T020 [BDD-REFACTOR] 在綠燈下整理建議插入與 debounce／cancel 路徑

## Phase 4B: ADD Feature File - cli/chat/presenting-the-interactive-prompt.feature

**Goal**: 讓 `presenting-the-interactive-prompt.feature` 全綠 — `-i` 提示呈現 reference chrome（有框線編輯器 + 其下建議清單 + 單一 `>` cursor + 無 metrics 標頭 + placeholder 提示鍵 + 保留所打文字 + frame 在終端寬度內）。

**Shared Must Read**:
- `specs/truth/features/cli/chat/presenting-the-interactive-prompt.feature` -> `Feature: Presenting the interactive prompt`
- `specs/truth/features/cli/chat/dsl.md` -> `the interactive prompt is framed around a multi-line editor`、`the interactive prompt lists suggestions beneath the editor`、`the interactive prompt marks one suggestion as the current choice`、`the interactive prompt shows no session metrics header`、`the interactive prompt advertises the submit and abort keys`、`the interactive prompt keeps the typed text "…"`、`the interactive prompt frames the editor within the terminal width`
...
- [ ] T021 [BDD-GREEN] 讓 Test Scope 全綠（並使 T016 的 chrome `[UNIT]` 轉綠）
- [ ] T022 [BDD-REFACTOR] 在綠燈下整理 chrome tokens 與 resize 路徑

## Phase 4C: DELETE Feature / DSL Truth - the `-i` session dashboard header

**Goal**: 移除 `-i` 提示面的 session dashboard 標頭（strict parity），並確認 `using-the-interactive-prompt.feature` 在無 dashboard 下仍成立。
...
- [ ] T023 [CODE-REMOVE] 移除 dashboard 標頭的產品分支與 `Dashboard` 注入路徑
- [ ] T024 [REGRESSION] 跑 Test Scope，確認新版 truth 成立（無 dashboard）且 submit／abort 不受影響
...
- [ ] T025 [REGRESSION] 執行全域回歸 + falsifiability witness
  ...
  - **可偽性見證 (a)（chrome／dashboard）**：暫時讓 `model.go` 的 View 再次渲染 dashboard 標頭列，確認 `presenting-the-interactive-prompt.feature` 的 `the interactive prompt shows no session metrics header` 失敗；觀察到失敗即還原。
...
|:---|:---|:---:|
| `specs/truth/techstack.md` -> CLI Application（Interactive TUI prompt (`-i`) — strict-parity chrome） | T003、T016、T021、T022 | PASS |
| `specs/truth/techstack.md` -> CLI Application（Session dashboard — removed from `-i`) | T004、T005、T009、T016、T023 | PASS |
| `specs/truth/techstack.md` -> CLI Application（Prompt suggestion engine — debounce/cancel/drop/insert） | T003、T014、T017、T019 | PASS |
| `specs/truth/techstack.md` -> Testing & Verification（Interactive TUI prompt harness — style/state assertions） | T002、T016、T017 | PASS |
| `truth-delta.md` -> `/axb-technical-research` MODIFY（`specs/truth/techstack.md`） | T003、T016、T017、T019–T023 | PASS |
| `truth-delta.md` -> `/axb-api-plan` NOOP（`specs/truth/contracts/**`） | 豁免（NOOP 不建任務；T025 驗證詞彙維持 11） | PASS |
| `truth-delta.md` -> `/axb-data-plan` NOOP（`specs/truth/data/**`） | 豁免（NOOP 不建任務） | PASS |
| `truth-delta.md` -> `/axb-dsl-refine` DELETE（`chat/using-the-interactive-prompt.feature` dashboard rule） | T004、T005、T023、T024 | PASS |
| `truth-delta.md` -> `/axb-dsl-refine` ADD（`chat/presenting-the-interactive-prompt.feature`） | T006–T012、T021、T022 | PASS |
| `truth-delta.md` -> `/axb-dsl-refine` MODIFY（`chat/prompting-with-suggestions.feature`） | T013、T014、T015、T019、T020 | PASS |
| `truth-delta.md` -> `/axb-dsl-refine` MODIFY（`chat/dsl.md` — +10 rows / −2 rows） | T001、T004–T015、T018 | PASS |
| `truth-delta.md` -> `/axb-dsl-refine` NOOP（root `cli/dsl.md`） | 豁免（NOOP 不建任務；T025 驗證詞彙維持 11） | PASS |
| `research.md` -> Decision 1（reproduce chrome on the existing family; no new dep） | T003、T016、T021、T022、T025 | PASS |
| `research.md` -> Decision 2（debounced/async/cancelable refresh + >3-line drop） | T003、T014、T017、T019 | PASS |
| `research.md` -> Decision 3（`Tab`/`Shift+Tab` insert; last-token heuristic） | T011、T013、T015、T016、T019 | PASS |
| `research.md` -> Decision 4（remove the dashboard header — strict parity） | T009、T016、T023 | PASS |
| `research.md` -> Decision 5（terminal-width handling + graceful narrow） | T003、T012、T016、T022 | PASS |
| `research.md` -> Decision 6（hermetic no-pty verification; style/state assertions） | T002、T016、T017、T018 | PASS |
| `research.md` -> Decision 7（no new dependency; POSIX-only; keybindings unchanged） | T003（Boundary）、T022（Boundary）、T025 | PASS |
| `research.md` -> Decision 8（must-ask questions unchanged） | T025（不改 runner／端） | PASS |
| `spec.md` -> US1–US3、`FR-001`–`FR-011`、`NFR-001`–`NFR-004` | T004–T017（對齊）、T019–T024（交付）、T025 | PASS |
| `plan.md` -> Source-code structure（`internal/ui/tui/prompt/**`、`internal/cli/cli.go`；no new pkg/module） | T003、T016、T017、T019–T023 | PASS |
| `plan.md` -> Scope notes（api NOOP；data NOOP；UI reviewed；CLI end → `/axb-dsl-refine`） | T018、T025 | PASS |
| `ui/ui-plan.md` + `ui/screens/*.txt`（terminal-mode PM artifact） | T003、T021、T022 | PASS |
| Round-016 locked decision (#39): strict parity / interaction parity / responsive / no-regression | chrome → T003/T016/T021/T022/T023；interaction → T003/T011–T017/T019/T020；responsive → T003/T012/T016/T022；no-regression → T022/T023/T025 | PASS |
