...
| ADD | `specs/truth/features/cli/chat/presenting-the-interactive-prompt.feature` | New atomic rules: the prompt is framed around a bordered editor and lists suggestions beneath; it shows no session metrics header; it advertises the submit/abort keys and keeps the typed text; the editor frame stays within the terminal width. | Round 016 — carry the chrome parity into executable truth (`FR-001`–`FR-004`). |
