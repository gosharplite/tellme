...
The accessor is surfaced as a **returned value**, not a `stderr` diagnostic (S-3 → the returned-value form): a package-level `UnpairedCallIDs(prior []llm.Message) []string` builds the round(s) and returns the accumulated `dropped` ids. **Why not a diagnostic:** the adapter owns no logging/stderr seam today (the CLI owns the chrome), and a `[Tool …]`-class note or a `stderr` write from infrastructure would be a new side-effect that could pollute the E2E stdout/stderr assertions — it is deferred (RF-067-1) and the returned-value accessor is the honest, hermetic observability. **Scope of the claim (F-067-2):** the accessor has **no live consumer** today, so at runtime the boundary drop is **exactly as silent as before round 067**; the property this round lands is that the drop is **accountable in code** through a single owner — not that it is surfaced to the operator. **Why exported (`UnpairedCallIDs`) and not test-only:** it is a documented, single-owned seam a future diagnostic/consumer can adopt, and exporting keeps it out of the `unused` lint class — the `internal/…` package boundary already keeps it non-public to the world. It shares **one build pass** with `buildContents` (`buildRound`), so the account and the emitted body cannot disagree by construction (F-067-3).
...
The round-066 residual was that `TestRequestBody_ShortRound_DropsUnpairedNames` (`N=2 M=1`) **cannot kill a partial-drop mutant** — the emitted turn is the same single `functionResponse` part whether the adapter drops **one** or **both** unpaired calls (inherent `M == N/2` arithmetic equivalence). The accounting now makes the drop **observable**, so a pin can assert the **identity** of the unpaired call. **What actually retires RF-066-8 is the CROSS-ROUND account**, not the short-round pin: a mutant that reinstates the pre-fold *conditional partial drop* (retaining a stale `pending` slice instead of the unconditional `b.pending = nil`) leaves the short-round pin **green** (at `N=2 M=1` the two forms are arithmetically equivalent) but reds `TestUnpairedCallIDs_MultiRound` — the retained stale calls are re-accounted on the next flush (`[call_r1b call_r1b call_r2a]` ≠ `[call_r1b call_r2a]`). *(Attribution corrected at the round-067 architect review F-067-1: the carrier is `TestUnpairedCallIDs_MultiRound`, witnessed and recorded as `tasks.md` T009(d).)* **RF-066-8 is therefore retired** (SC-007) by the cross-round identity pin.
