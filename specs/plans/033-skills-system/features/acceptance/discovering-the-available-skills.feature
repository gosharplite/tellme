Feature: Discovering the skills available in the workspace

  # Acceptance only (business journeys): when the operator's workspace holds
  ...
  # skill's supporting material is not a skill of its own; a workspace with none
  # is reported honestly). A skill the operator points tellme to can then be
  # opened and followed on demand.

  Rule: tellme reports the skills present in the workspace

    Example: An operator asks what skills are available
      Given the workspace holds a skill "golang-patterns" and a skill "golang-testing"
  ...
  Rule: A skill's supporting material is not reported as a skill of its own
    ...
    Example: A skill that also ships reference material
      Given the workspace holds a skill "golang-patterns" that also ships its own reference material
      When the operator asks tellme to list its skills
  ...
  Rule: A workspace with no skills is reported honestly

    Example: An operator who has no skills installed
      Given the workspace holds no skills
      When the operator asks tellme to list its skills
  ...
  Rule: A listed skill can be opened and followed on demand
    ...
    Example: The assistant follows a skill the operator points it to
      Given the workspace holds a skill "golang-testing" that explains how to write tests
