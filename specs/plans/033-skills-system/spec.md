...
- **What counts as a skill**: a skill is a Markdown file that declares YAML frontmatter carrying a `name` and a `description` (the reference's rule), discovered recursively. [NEEDS CLARIFICATION: whether the acceptance rule is "any `.md` with valid `name`+`description` frontmatter" or strictly files named `SKILL.md` — low risk, does not block planning.]
- **Tool conventions**: `list_skills` follows tellme's existing agent-tool conventions (registered in the agent tool set, offered on prompt-bearing turns, bounded per the round-024 resource contract, with the schema-style `reason` the other tools carry); the exact schema, field names, and list ordering are RD decisions owned by `/axb-technical-research` / `/axb-tasks`, not spec requirements. [NEEDS CLARIFICATION: the listed field detail (is the path shown? what ordering?) — low risk, does not block planning.]
