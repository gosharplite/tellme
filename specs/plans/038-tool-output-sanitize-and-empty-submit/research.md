...
| Layer | Witness |
| --- | --- |
| Unit (`internal/ui`) | `TestFormatToolOutputLineStripsControlSequences` (hostile fixture: SGR set/no-reset, SGR set+reset preserving text, OSC title BEL + ST, cursor-hide, stray BEL, ESC charset, TAB preserved, UTF-8 preserved, **ESC before a multi-byte rune** (review B1), **unterminated CSI** (review RF-2)); `TestToolOutputWriterStripsSequenceSplitAcrossWrites` (split-across-writes via the writer); `TestToolOutputWriterEndRestoresNeutralState` + `TestToolOutputWriterStartFailureRestoresNeutralState` (a restore precedes the closing separator on the normal and the start-failure paths). |
| Unit (`internal/ui/tui/prompt`) | `TestModelEmptySubmitDoesNotQuit` (the `Update` command is nil for an empty `Ctrl+S`/`Alt+Enter`); `TestModelWhitespaceOnlySubmitDoesNotQuit`; `TestModelNonEmptySubmitQuits`; `TestModelAbortStillQuits`; the tightened `TestModelEmptySubmitKeepsFrame` (round-023 FR-004, now also asserting the nil command — review RF-3). |
| E2E (`tests/e2e`) | `the run streamed the command's output free of terminal control sequences` (content lines carry no ESC/control byte **and are valid UTF-8** — review B1); `the terminal is left in its default state` (a restore on the block's close line — review N-3); the empty-then-real-submit Example (exactly one request ⇒ the empty submit did not quit). |
