...
**Status**: Draft — produced by `/axb-specify` from operator tasking. **Clarify round 1 CLOSED (3 questions, one at a time): Q1 → Option 2** (only the **estimated pre-flight** line gains the delta and drops the budget; the **measured** line keeps `<tokens>/<budget>`) · **Q2 → Option 1** (the baseline is the last estimate emitted **this session**, held **in memory**; no persistence) · **Q3 → Option 1** (`-t`/`turns.log` stays **content-equal but plain** — it follows the new estimated-line shape and the new cap, carries no colour, and keeps its current line set). **Two low-risk boundary cases are exposed as assumptions below (A7, A8 — operator-vetoable) rather than a 4th question (the round's 1–3 question budget).** No open `NEEDS CLARIFICATION`. No `specs/truth/**` file is written by this skill.
...
| **Q2** | **What is "the previous payload" — and is it held in memory or persisted?** Options: **(A)** the last payload value **emitted in this session**, held in memory by the CLI (zero persistence; resets on `--new`/restart — a resumed session starts with no predecessor); **(B)** the previous **measured** `prompt_tokens` **persisted** on the turn record (a round-027-`Calls`-style `Entry` field; survives `--new`/restart; a data-model change under `data/**`); **(C)** the previous call **within the current turn** rather than the previous turn. | ✅ **LOCKED → Option 1** (operator, 2026-09-19): the previous estimated payload is the last estimate **emitted this session**, held **in memory** by the CLI — zero persistence, no data-model change. It resets on `--new`/restart, so a resumed session's first estimate has no predecessor (the no-predecessor form is Q3 → (b)). |
| **Q3** | **How exact must `-t`/`turns.log` be with respect to the terminal?** Options: **(A)** content-parity, plain (the file keeps today's line set, colour-free, but follows the new content — the estimated `+<delta>` form and the 500-rune cap); **(B)** byte-parity including colour (reverses round-054 B-54-1); **(C)** full parity — also add the stderr-only lines into the file (reverses round-053 F-53-3). | ✅ **LOCKED → Option 1 (A)** (operator, 2026-09-19): *"`-t` needs to still be the same as what is shown on the terminal"* = the file carries the **same content** as the terminal (so it follows the new estimated-line shape + the cap), but stays **plain** (no colour escapes) and keeps its current line set (the stderr-only `[Tool …]`/`[Tool Output]` lines stay out). No `turns.log`-mechanism change. |
...
2. **Given** a first payload of a session (no predecessor), **When** the line is rendered, **Then** the delta renders as `+0` (A7 — a uniform, always-parseable segment).
3. **Given** a current payload **smaller** than the previous one, **When** the line is rendered, **Then** the delta renders as the true signed value (e.g. `-1234`, A8) — not clamped.
...
- **FR-007**: the delta MUST resolve to the last **estimated** payload emitted **this session** (in-memory, session-scoped — Q2 → 1), and MUST render its two boundary cases per A7/A8 (no predecessor ⇒ `+0`; a smaller payload ⇒ the true signed delta).
- **FR-008**: the estimated line's remaining content MUST be unchanged (timestamp, `Payload:`, `tokens`, ` - <mode> - <model>` — I-1); the round-054 **green** accent on the `MODE` token is preserved, and the delta's own colour is a recorded open item (the delta is a new token type; default = plain).
- **FR-009**: the delta MUST be display-only — no persistence, no change to the round-024 effective budget, the round-026 tool-usage accounting, or any exit code (I-3/I-4).
- **FR-013**: `turns.log`/`-t` MUST stay **content-equal but plain** (Q3 → 1): the file follows the new estimated-line shape (`+<delta> ~<tokens>`) and the new 500-rune action cap, carries **no** colour escapes, and keeps its current line set (the stderr-only `[Tool …]`/`[Tool Output]` lines are **not** added).
...
- **A7**: *No predecessor* (the first estimated payload of a session, or right after `--new`/restart) renders the delta as **`+0`** — the token segment keeps the uniform, always-parseable `+<delta> ~<tokens>` shape. *(Operator-vetoable; exposed as an assumption because the round's clarify budget (1–3 questions) is spent and this is a boundary form, not a workflow decision.)*
- **A8**: A current payload **smaller** than the previous renders the **true signed delta** (e.g. `-1234`) — not clamped to `+0`. A shrinking payload is real signal (compaction / summarised history), and the operator's "+100 is the increment" reads as a signed difference. *(Operator-vetoable; same rationale as A7.)*
- **A9**: The delta token carries **no** new colour (plain) in this round; the round-054 green on the `MODE` token stays. *(If the operator wants the delta coloured, that is a small follow-on.)*
