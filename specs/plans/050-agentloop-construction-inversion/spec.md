...
> **All four questions are CLOSED** (Q1 → A · Q2 → (i) · Q3 → (a) · Q4 → A — recorded below). Per the `/axb-specify` → `/axb-clarify` gate, they were asked **one at a time**.
...
| **Q4** | **The CLI test surface** (surfaced during grounding, after Q1–Q3). RULE-E evaluates the **merged** (production + in-process test) import graph (ADR 0016 D4; `tools/arch/arch_test.go` — *"test imports ARE governed"*, review N-3). So a `_test.go` import of `internal/agent` in `internal/cli` would keep the `→ agent` edge **alive** and the baseline line would **not** go stale → **the DoD cannot be met**. Today the CLI's `runTurn` unit tests (`turn_test.go`, `persistence_invariant_test.go`, `tui_submit_chrome_test.go`) exercise the **real** `AgentLoop` (they drive tool rounds through the fake *gateway*, asserting `Calls`, the tool-loop log ordering, the header count). Options: **(A)** the CLI tests adopt an in-package **fake `agentport.Loop`** (the fixture's `LoopFactory` returns a fake) — real-loop coverage stays in `internal/agent` unit tests + the godog E2E; the loop-dependent `runTurn` tests are reworked to the fake (asserting CLI orchestration, not loop internals) · **(B)** keep the real loop in the CLI tests via a `_test.go` `internal/agent` import — **the edge survives, the baseline stays 2, the DoD fails** (stated for completeness; rejected) · **(C)** amend the gate (ADR 0016 D4) so RULE-E evaluates the **production-only** graph, keeping the real loop in the CLI tests — a **rule change** contradicting ADR 0016/review N-3, out of this round's scope. | ✅ **A** |
...
### Q4 → A (LOCKED) — the CLI tests adopt an in-package fake `agentport.Loop`

RULE-E evaluates the **merged** (production + in-process test) graph, so the CLI test surface must also leave `internal/agent`. The injected `LoopFactory` in the CLI's test fixture (`defaultTestDeps`) returns an **in-package fake `agentport.Loop`**; the real-loop coverage stays in **`internal/agent` unit tests + the godog E2E**. The three loop-dependent `runTurn` test files are reworked to assert **CLI orchestration** (what `runTurn` does with the `Result`/error — persistence, exit codes, stream ordering) rather than loop internals; where a test needs a specific loop outcome it is scripted by the fake. Recorded cost: **honest test churn** on `turn_test.go`/`persistence_invariant_test.go`/`tui_submit_chrome_test.go` (a test-only adaptation; no production behaviour change). Rejected: **B** (a `_test.go` `internal/agent` import keeps the edge alive -> the DoD fails); **C** (amending RULE-E to production-only — a rule change contradicting ADR 0016/review N-3, out of scope).

**Clarify round 1 — CLOSED** (Q1 → A · Q2 → (i) · Q3 → (a) · Q4 → A); **no `NEEDS CLARIFICATION` remains**.
...
### Q2 → (i) (LOCKED) — domain interface + func-typed factory; adapter in `internal/agent`

The port is a domain **interface** `agentport.Loop` (`Run(ctx context.Context, prompt string, prior []history.Entry) (Result, error)`) fed by a domain **`LoopSpec`** value struct carrying the loop's 9 construction inputs (all already domain/stdlib-typed: `Gateway llm.Gateway`, `Registry tools.Registry`, `MaxLoops int`, `Stderr io.Writer`, `Now func() time.Time`, `EffectiveBudget int`, `ToolUsage history.ToolUsageSink`, `Lines agentport.ToolLineRenderer`, `Observer agentport.LoopObserver`). The adapter is a new exported constructor **`agent.NewLoop(spec agentport.LoopSpec) agentport.Loop`** in `internal/agent` (tier 4; imports `internal/domain/**` **downward** — RULE-A-clean), bound at the composition root. `deps.Dependencies` gains a **func-typed** factory field (`agentport.LoopFactory func(LoopSpec) Loop`), so the existing `Validate()` predicate (`Kind()==reflect.Func`) already covers it — **no** explicit interface-seam assertion is required (FR-009 stays latent; ADR 0017 §Forward caveat not triggered). Rejected: **(ii)** a domain struct-of-funcs (the F-8 smell, minted fresh); **(iii)** a `cmd/tellme` closure (hides the loop's field contract in `main` and grows root field-wiring knowledge). **RULE-C purity**: `LoopSpec`/`Loop` reference only stdlib + domain types; the `Gateway`/`ToolUsageSink`/`LoopObserver`/`ToolLineRenderer` types are already domain-owned, so no new domain cross-edge.
...
- **Test-import reach (Q4)** — RULE-E evaluates the **merged** (production + in-process test) graph, so a `_test.go` `internal/agent` import in `internal/cli` keeps the edge alive and the DoD fails; the CLI test surface must therefore also leave `internal/agent` (Q4).
- **Build-tag scope (RULE-F only)** — the **RULE-F** coupling-surface scan covers **production** sources only (ADR 0018 F-3); **RULE-E** is merged-graph (above).
