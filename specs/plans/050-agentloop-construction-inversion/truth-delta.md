...
| MODIFY | `specs/truth/techstack.md` — **CLI Application / Agent tool loop** row (fold) | **Round-050 fold TD-2(iii) + RF-1**: records that (a) the real loop's **rendering/ordering** witness is now owned in layers — loop schedule/hook ordering → `internal/agent` unit tests; the bytes → `internal/ui` (`toolrenderer_test.go`, `toolcall_reason_test.go`, `toolcall_sanitize_test.go`); the CLI call renderer's tail/blank rules in isolation → **`internal/cli/call_renderer_reason_test.go`** (direct, not via the fake loop); end-to-end → the godog E2E (the `internal/cli` turn tests own only its orchestration, after the Q4 → A fake-loop move; pointer corrected by fold **F-1**); (b) `LoopSpec` carries the loop **struct's** 9 fields and stays a **capability contract** (no implementation-only knobs); (c) the loop's `Registry`-identity / `BindToolOutput`-before-`Run` standing invariant. | round-050 fold TD-2(iii)/RF-1/TD-3. |
