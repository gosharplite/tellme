...
- [x] 每個使用者故事都可被獨立驗證
- [x] 每個使用者故事都包含驗收情境
- [x] 可歸屬單一故事的 FR 已直接掛在故事底下（US1: FR-001…FR-004；US2: FR-005…FR-007，條件式）
- [x] 全域需求只保留跨故事或無法合理歸戶的條目（無獨立全域條目；I-1…I-5 為不變式）
- [x] 正式需求沒有在故事區與全域需求區重複列出

## 缺口與澄清策略

- [ ] 只有高影響缺口才升級到 `/axb-clarify` — **Q1 待答**：範圍僅限候選池（A），或同時納入 reference 其餘來源行為（空輸入顯示前 5、合併 session 最後一則提示詞）（B）。此題改變需求集（US2 是否成立）與 CLI「啟動不讀磁碟」性質，屬高影響。
- [x] 本輪 clarify 題數控制在 1 至 3 題／session 上限 5 題 — **1 題（Q1）**，一題一問
- [x] 低風險未定細節已用假設揭露（A1–A6；深度值 50 為可單點調整的常數）
- [ ] 仍保留的 `NEEDS CLARIFICATION` 已標示是否阻塞後續規劃 — **Q1 阻塞 US2 切分與驗收標準，須先拍板才能進入 `/axb-spec-by-example` 的完整化**
...
**備註**: 需求主體（US1：候選池 10 → 50，上限不變 10）已完備且可獨立驗收；**Q1 為進入完整 `/axb-spec-by-example` 前唯一阻塞項**（決定 US2 是否成立）。Q1 一拍板即可折入 spec 並續行。
