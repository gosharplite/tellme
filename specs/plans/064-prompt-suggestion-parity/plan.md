...
Witnesses (reproduced then reverted): (a) restoring the depth constant to **10** turns the unit pin **RED** with the **literal pin's** diagnostic — `promptPoolDepth = 10, want the reference's 50 (ADR 0034)` (`service_test.go:97`; the literal pin precedes the relation check, so its message fires first — review R-5) — and the E2E depth Example RED. The `depth > cap` relation check is now reachable only from a **cap** mutation `≥ 50`; it stays a cheap cross-constant consistency guard. (b) raising the **surface cap** (`maxSuggestions = 20`) turns the **E2E cap Example RED** (`The ten nearest matches are offered and the surplus is dropped`, `267/268`) while the unit pin stays GREEN — the unit pin's `len(got) == maxSuggestions` clause is self-referential (it moves with the mutation), so the **E2E cap Example is the value-binding cap carrier** (review R-4).
