...
| **S-7** | **The projection is silent — (operator 2026-09-19).** `ADR 0031` **documents** the rule; the run emits **no** notice for a projected-away keyword. Rationale: the operator's report was a *failure*, not a visibility gap; the only existing channel (MCP discovery warnings) runs earlier than the floor's effect and cannot faithfully count the adapter-side drops, so an always-on line would be noise and half-true. A future "aggregated projection notice" is a recorded, non-blocking forward item. |
