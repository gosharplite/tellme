...
| **RES-093-FV-1** (fold verification) the ADR 0063 **index row** (`docs/decisions/README.md`) kept the unqualified `599/960 → 0/960` figure the fold had swept past | Qualified it (~62 % → 0, authoring host; the direction is the claim, **F-093-2**) — the F-090-1/F-091-2 index-row lineage. Re-sweep predicate **with its exclusion** (the round-089/091 lesson): no **live** unqualified magnitude survives; the sole `grep` hit is *this ledger row quoting the fixed defect*. Final verdict: **`FOLDS VERIFIED — LOOP CLOSED (no residuals)`**. |
