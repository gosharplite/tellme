...
4. **`TestFileStore_Rollback_PreservesSurvivorBytesOnToolWrittenPath`** (EC-004, the grill-flagged sub-clause) — append three entries through the store, snapshot the file, roll back one, assert the surviving bytes are **byte-identical** to the first two original lines (the tool-written path, not a hand-filled file).
