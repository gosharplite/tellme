...
| --- | --- | --- |
| FR-1 | `techstack.md`'s `search_files` row carries no bare ordinal | `specs/truth/techstack.md:36` reads *"the in-file content search tool (`search_files`) — the missing half of the reader trio …"*, no "ninth" |
...
- **Out**: product code; `.feature`/step/row-semantics changes; **editing an Accepted ADR body**
  (0043 + the `0051`/`0052`/`0055`/`0058` counts stay verbatim); rewriting frozen `specs/plans/NNN-*/**`;
