...
**Fold 2 (residuals)** (`<pending>`): folded RES-091-FV-1/2/3 (record-only): the ADR enumeration completed to eight on `plan.md` §4 + `tasks.md` T009/CLM-003 (and the fold ledger's surface list corrected); the §3 change-surface table gained the two comment-only `.go` edits (N-091-FV-1); the W-A predicate restated in `checklists/requirements.md` FR-2 + `tasks.md` T008 (with the N-091-4 exclusion); the checklist `NFR-1` + the boundaries restated **behaviour-scoped** to name the two comment-only `.go` edits. RES-091-FV-4 → homed on live issue [#191](https://github.com/gosharplite/tellme/issues/191).
