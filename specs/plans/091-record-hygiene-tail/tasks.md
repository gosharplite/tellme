...
**Fold 1** (`<pending>`): F-091-1 → dropped the ordinal from the `search.go` comment + the self-referential literal from `STATUS.md` + restated W-A as a predicate with an explicit exclusion list (frozen history · the immutable ADR 0043 body · a self-referential defect mention); F-091-2 → restated the ADR enumeration to all **eight** (`research.md` D3/D7, `spec.md`, `checklists/requirements.md`, `truth-delta.md`) + recorded R-091-1; F-091-3 → corrected the FR-1 expected string + `spec.md` §2 to the shipped text; TD-091-1 → dropped the stale "five tools" count from the `filesystem_test.go` comment; N-091-1/2/3/4 folded (research D1 softened; the "eleven-survives" note added; CLM-006 behaviour-scoped; the exclusion list names the self-referential shape).
