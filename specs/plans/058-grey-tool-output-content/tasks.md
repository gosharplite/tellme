...
**Fold-verification (5739547989) fold-back.** **TF-058-1**: the fold's own ADR-0027 `Status` line was a **double paste** (`assumption A3 … superseded by ADR 0028` twice on line 3) — fixed to one occurrence. **PM follow-up recorded** (non-blocking, the R-056-c class): the plan-side acceptance Rule *"The block is plain when the output is not a terminal or raw output is asked for"* still carries its two original block-less Examples, so the PM-owned journey is weaker than its now-non-vacuous interface carrier; mirror the anti-vacuity Example into the acceptance journey at the next PM pass (recorded in `STATUS.md`, not edited by RD).
