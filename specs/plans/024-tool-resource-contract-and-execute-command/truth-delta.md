...
| MODIFY | `specs/truth/techstack.md` | **Review fold (PR #54, head `7bdeede`):** T3 — pinned that the round-009 payload status line displays the **effective budget** (equal to `MAX_HISTORY_TOKENS` when no window is configured); T4 — added the readers' **timeout-as-nil-error-result** clause to the Read-only filesystem tools row (the former `ctx.Err()` → `error: ` path is retired; FR-018); R3 — noted the tool's own marker-reserving bound is operative and the loop clamp is deliberately larger; forward — clarified the "write tools deferred" line (`execute_command` is already an arbitrary-write path). | The environment architecture review on `7bdeede` (fold before `/axb-tasks`). |
...
| MODIFY | `specs/truth/features/cli/chat/listing-a-directory.feature`, `surveying-a-folder-tree.feature` | **Grill Q5:** each gains a Rule + Example witnessing the truncation marker (FR-011) via a small `max_output_tokens`. The tools' **output shape** is unchanged (an internal distinction, not a ledger action). | The earlier "a loop concern, no new step" rationale was **false** (the bound is set by the tool, not the loop); FR-011 must be falsifiable. |
...
| MODIFY | `specs/truth/features/cli/chat/dsl.md` | **Review fold (T2):** pinned one **deterministic** process-tree fixture (`(sleep 60; echo late > child.sentinel) & echo $! > child.pid; wait`) — the Given now **requires** the pid file + the sentinel, and the Then confirms the descendant via a **bounded poll**. | The two ends of the Q6 witness described different fixtures (a possible silent fall-through). |
