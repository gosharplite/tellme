...
- [x] 驗收情境足以驗證主要成功路徑（US1 以「每個 call/result part 帶 id、response id = call id」為紅→綠見證；US2 以「亂序結果仍配到自己的 call」為紅→綠見證，另以 replay FIFO fallback 保真）
...
- **修法（issue 已載明）**：wire 上帶 id；`buildContents` 依 id 配對（FIFO fallback）；採用參考的 id 紀律；exact unmatched 記帳。
