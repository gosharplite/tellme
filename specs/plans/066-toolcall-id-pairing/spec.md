...
3. **Given** a round that yields **fewer** results than calls (`M < N`), **When** the body is built, **Then** the batched turn carries the `M` parts the round produced (an unpaired call contributes no part — the boundary drop is unchanged) and no result is silently mispaired. *(Corrected at fold-verification F-066-5: the earlier "accounted by identity" wording was retracted — the unpaired calls are not surfaced.)*
...
- **A4**: **Truth impact expected** — `specs/truth/techstack.md` (the *Vertex/Gemini adapter* row: the id-linked pairing rule; the `ToolCallID` clause updated from "optional" to "carried when present; the FIFO name pairing is the **defensive fallback** (replay is id-primary)"), and a **new ADR** (the id-keyed pairing decision; it **extends ADR 0035** and annotates its D2 "FIFO is sound for the sequential loop" note + its §Forward RF-065-1; the ADR-0033/0035 forward-annotation precedent). `/axb-api-plan` **NOOP**; `/axb-data-plan` **NOOP**.
