...
- [x] **T010** [RECORD] Keep the `TestRequestBody_ShortRound_DropsUnpairedNames` `N=2 M=1` residual note (the boundary drop is **unchanged**; the id-keyed pairing did not reach it — F-066-1) · topology audit **5 pre-existing, none new** (no feature/DSL edit) · STATUS updated · commit + PR (human merge only).
...
- **T010** — the `TestRequestBody_ShortRound_DropsUnpairedNames` residual is **unchanged** (the boundary drop still contributes no part for an unpaired call; the id-keyed pairing did not reach it — the earlier retracted accounting note is recorded at F-066-1, the residual re-homed as RF-066-7/RF-066-8). No feature/DSL edit ⇒ the topology audit is unchanged (**5 pre-existing, none new**; no `specs/truth/features/**` change). `go.mod`/`go.sum` unchanged.
