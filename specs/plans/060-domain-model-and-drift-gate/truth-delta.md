...
| MODIFY | `docs/decisions/0011-layer-discipline-gate.md` — **Status** line (+ its `docs/decisions/README.md` index row) | Annotates the frozen ADR's **Status** (and its index row) with a **forward pointer** to ADR 0030, so a reader landing on 0011 learns its D10 "no modelith toolchain" clause was amended. Body never rewritten (round-058 F-058-2 precedent). | round 060 TD-060-4 (PR #126 review). |
| MODIFY | `SESSION-BOOTSTRAP.md` — Step 1 + Agent Rule 10 | Step 1 now reads tellme's three domain models **right after `README.md`** (a Step-1 detail section + Agent Rule 10 recording the descriptive-docs/truth-wins boundary + the generated-`.md` discipline). A companion change that MUST land with this round. | round 060 FR-011; TD-060-3 (PR #126 review) — traceability. |
