...
- **FR-004**: The round MUST **adopt the modelith toolchain** (the `gosharplite/modelith` fork) and add `make` targets: **`modelith-lint`** (validate every `*.modelith.yaml`), **`modelith-render`** (regenerate the `*.modelith.md` from the YAML), and **`modelith-check`** (fail if any committed `.md` is stale). `modelith-check` MUST be a **zero-tolerance member of the aggregate `make verify`** (Q3 = 1): **drift ⇒ fail**, and an **absent `modelith` binary ⇒ fail**, naming the **install route** — the **clone + pinned-build** route single-sourced in `docs/domain-model/README.md` (the fork declares the upstream module path, so it is **not** `go install`-able from its GitHub path). The gate MUST NEVER silently skip. `modelith` is a **dev-tool binary** prerequisite, **not** a `go.mod` dependency; the **immutable commit pin** (`b4153541cee8`) is recorded in the README.
...
- **modelith binary absent** on a host running `make verify` → per **Q3 = 1** the gate **fails**, naming the install route (the **clone + pinned-build** route single-sourced in `docs/domain-model/README.md`; see FR-004); the gate never silently skips. (Consequence recorded: `make verify` now requires the modelith dev tool.)
