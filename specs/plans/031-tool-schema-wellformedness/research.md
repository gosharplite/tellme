...
## Decision 2: The well-formedness gate is a **unit test over the production registry**

- **Decision**: add a unit check that iterates the **production tool registry** (`newToolRegistry()`, `internal/cli`) and asserts, for **every** registered tool, that each name in the schema's `required` is declared under `properties` **and** that the schema decodes as a JSON object. It sits beside the existing `TestNewToolRegistryOffersAgentTools` (the "tool-registry surface" the issue names), so a future tool cannot regress silently. The existing blind-spot test (`TestToolSchemasRequireReason`, which asserts only that `required` *contains* `reason`, over the three readers) is strengthened/replaced by this registry-wide invariant.
- **Rationale**: the registry is the authoritative "every tool the model is offered" surface — the same surface that already asserts the offered-tool set. Running the invariant there covers all six tools (including the write pair and `execute_command`, which today's blind-spot test never touches) and couples directly to what the transport actually sends, so it is the check that would have caught #64.
- **Alternatives considered**:
  - **A test in `internal/infrastructure/tools`** over `NewFilesystemTools()` + the write pair + the command tool — rejected as the *primary* gate: it enumerates the tools by hand rather than reading the one registry the CLI wires, so a newly added tool could be omitted from the list; the registry test cannot miss one.
...
- **Decision**: the automated verification is **hermetic** (offline): the round relies on the unit gate (Decision 2) plus the existing E2E/unit suites; `make verify` stays network-free. A **real Vertex/Gemini call** — a plain prompt and a tool-using prompt, no `400` — is verified **manually at closeout** (round-013 style, Q1 → 1), not inside the gate.
