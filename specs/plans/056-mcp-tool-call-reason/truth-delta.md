...
| Action | Truth Spec | Change Summary | Reason |
| --- | --- | --- | --- |
| MODIFY (expected) | `specs/truth/features/cli/chat/using-tools-from-a-remote-mcp-server.feature` (+ `chat/dsl.md`) | **Round 056**: a new Rule/Example — an MCP tool call renders a `[Tool Reason]` row, and the server receives only its payload; an envelope-less MCP call is refused. | `spec.md` US1/US2 / FR-001…FR-006 |
