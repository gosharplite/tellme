...
- **FR-009**: the reason-required rule MUST have **one owner** — the loop must **reuse** the round-046 single-owned reason-presence predicate (the injected `agentport.ToolLineRenderer.ReasonLine` port it already holds; `internal/agent` cannot import `internal/ui`), asking it at the gate for every call; **no** second reason-presence predicate may be defined, and the MCP adapter MUST NOT re-implement the reason half (it adds **only** the MCP-envelope check). The rule MUST NOT be split/duplicated across layers.
