...
| --- | --- | --- | --- |
| MODIFY | `specs/truth/techstack.md` | Added a **Write filesystem tools** row to the CLI-application agent-tool surface: the `write_file` **create-only + atomic** contract (**atomic create-only** via `os.Link`/`EEXIST`; created file mode **`0644`**), the `replace_text` **strict-unique + atomic** contract (temp + `rename`; a no-op replace short-circuits), and the **`replace_text` whole-file-read hazard**; no security/consent and no backup/undo; the deferred/non-goal split (`append_text` / `undo_file_change` deferred; `delete_path` / `create_directory` non-goals). Rewrote the *Not Introduced Yet* write-tools bullet so `write_file` / `replace_text` are no longer listed as deferred. | Round-029 D1–D8: the write pair becomes part of tellme's current tool surface; the truth must reflect the current system (`truth-current`). |
