...
**Independent verification**: run a turn with standard output redirected; confirm the answer is the only thing on standard output, ends with exactly one newline, carries no presentation escape sequences, that any failure phrase goes to standard error only, and that the run never blocks waiting for terminal input.

**Acceptance Scenarios**:

1. **Given** a successful turn whose standard output is not a terminal, **When** the turn completes, **Then** the answer text appears on standard output alone with exactly one trailing newline and no presentation escape sequences, and the process exits with the success code.
...
- **FR-006**: The system MUST write the answer text to standard output only, with exactly one trailing newline; the answer MUST NOT be written to standard error.
...
- **NFR-003**: For a given provider answer, the bytes written to standard output MUST be identical across repeated runs and MUST NOT depend on terminal detection beyond the suppression rule in FR-007.
...
- **SC-002**: In the acceptance set, 100% of redirected-standard-output runs place the answer on standard output only, with exactly one trailing newline and no escape sequences, exiting with code `0`.
