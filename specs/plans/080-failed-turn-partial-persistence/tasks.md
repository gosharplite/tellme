...
- **N-2 — hole #2's behavioural E2E is declined; the predicate is unit-witnessed.** A `Ctrl+C` landing inside a real 1–3 s retry wait is timing-sensitive. The broadened predicate is witnessed by a **direct `failTurn`-seam unit pin** (`TestFailTurn_CancelledContextWinsOverAFailure`: a cancelled ctx + a transport error → the interrupted path), falsifiable by deleting the `ctx.Err()` term (round-080 fold F-080-1). The round-079 interrupted tests exercise only the `errors.Is` term. Recorded (RF-080-6).
...
- **F-080-1..3 + TD-080-1/2 + N-080-1..4** (architect review `#issuecomment-5773659836`, APPROVE WITH REQUIRED FOLDS) — folded: **F-080-1** the hole-#2 fix now has a direct `failTurn`-seam unit pin (mutation: delete the `ctx.Err()` term ⇒ red); the record corrected here + ADR RF-080-6. **F-080-2** the persisted-`calls` E2E carrier added (`the failed turn was recorded with {count} completed provider call`; mutation `Calls: 0` ⇒ red). **F-080-3** the interruption-over-failure precedence recorded (ADR D5 + the `techstack.md` row). **TD-080-1** the failure-answer Then is parameterised (`the failed turn was closed with the answer "{answer}"`) so a class swap reddens. **TD-080-2** the rejected-outright Example gained the answer/keep-line/stdout Thens. **N-080-1** dsl typo (`close`→`closed`); **N-080-2** the ADR consequence reworded; **N-080-3** one shared `readSessionEntries` helper; **N-080-4** the author/reviewer `F-080-N` collision recorded. RF-080-A/RF-080-B added to ADR §Forward.
