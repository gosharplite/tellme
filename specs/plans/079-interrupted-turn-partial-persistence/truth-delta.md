...
| MODIFY | `specs/truth/features/cli/chat/dsl.md` | **+2** Given rows (the fake runs a command then answers; an arranged interrupted exchange) + **+5** Then rows (the persisted step count; the persisted `calls`; the synthetic close; the interruption line; the role-alternation close) + the class-phrase-absence row and the round-079 note | `spec.md` US1/US2; `research.md` D9 |
