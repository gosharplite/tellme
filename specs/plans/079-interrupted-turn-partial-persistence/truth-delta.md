...
| MODIFY | `specs/truth/features/cli/chat/dsl.md` | **+2** Given rows (the fake runs a command then answers; an arranged interrupted exchange) + **+4** Then rows (the persisted interrupted turn's step count; the persisted `calls`; the synthetic close; the interruption line) + **+2** more Then rows added at fold F-079-2/TD-079-1 (the persisted `calls` carrier; the class-phrase absence) + the role-alternation close + the round-079 note | `spec.md` US1/US2; `research.md` D9 |
