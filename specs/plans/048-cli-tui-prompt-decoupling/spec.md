...
**Status**: Draft — plan package created by `/axb-specify`. Anchor issue [#101](https://github.com/gosharplite/tellme/issues/101) (**R5** of [#92](https://github.com/gosharplite/tellme/issues/92)). This round is **R5.2** — the **first de-coupling slice** (round 047 = **R5.1**, the **RULE-E** gate + baseline). Clarify round 1 **Q1 → B LOCKED** (slice = the **TUI prompt** edge `internal/cli → internal/ui/tui/prompt`). **Q2 (port home)** and **Q3 (F-4 ride-along) remain open** — see §Locked decisions / §Open decisions.
...
## Open decisions (to be locked one at a time)

| # | Decision |
| --- | --- |
| **Q2 — port home** | Does the injected prompt port live in **`internal/domain/**`** (a domain-owned `Prompter`/`Source` seam over stdlib `io`/`context`/`time` types — RULE-C-pure; the TUI package satisfies it structurally) or in **`internal/app/**`** (an application-tier seam)? Constraint: RULE-A forbids any tier below 5 from importing `internal/ui/**`, so the *adapter* that calls `tuiprompt.Run` stays inside `internal/ui/**`; only the *port declaration* home is in question. **Recommended: domain** (a port is a domain-facing contract; #101 open-decision 3). |
| **Q3 — F-4 ride-along** | The PR #104 deferral **F-4** (`cli.Options.RunTUIPrompt` is an exported field of an unexported type `tuiPromptRunner`) becomes moot once the widened func-type seam is replaced by the port — should **F-4 be folded in this round** (unexport/delete the field) or stay deferred? **Recommended: fold in** (the change is in the same code being rewritten; leaving it creates a contradictory interim state). |
...
### Tier constraint that shapes the port (the RD problem)

ADR 0011 **D1** ranks `domain` 0 · `config`/`home` 1 · `app` 2 · `infrastructure` 3 · `agent` 4 · `ui`/`ui/tui/**` 5 · `cli` 6 (higher = more upward). **RULE-A** forbids an *upward* import. Therefore **only tiers ≥ 5 may import `internal/ui/**`** — so the adapter that actually calls `tuiprompt.Run` must remain **inside `internal/ui/**`** (or `internal/ui/tui/prompt` itself). The de-coupling is: declare the **port** in a permitted tier, have the TUI package (or a thin `internal/ui` adapter) **satisfy** it, and **wire** it at the composition root (`cmd/tellme`, exempt). `internal/cli` then imports only the port's home tier. This is why "port home" (Q2) — not "adapter placement" — is the live decision.
...
- **`internal/domain/**` stays 100 % pure** if the port lands there (only stdlib + domain types; `io`/`context`/`time` are stdlib); no `ui` type crosses the boundary (RULE-C).
- **Tests stay hermetic** (no mutable package globals; round-029/030 precedent); the existing `tuiPromptRunner` test seam (`tui_dispatch_test.go`, `tui_submit_chrome_test.go`) is adapted, not bypassed.
...
### User Story 1 - the TUI prompt is reached through an injected port, so `internal/cli` no longer imports `internal/ui/tui/prompt` (Priority: P1)

As a maintainer, I want the interactive TUI prompt to be invoked through an **injected port** wired at the composition root, so `internal/cli` no longer imports `internal/ui/tui/prompt` and the RULE-E baseline can drop that edge — with **no** behavioural change to `tellme -i`.
...
4. **Given** the new port, **When** it is declared, **Then** it lives in a permitted tier (Q2) and the adapter that calls `tuiprompt.Run` stays inside `internal/ui/**`; the port is wired at `cmd/tellme`.

**Functional Requirements**:

- **FR-001**: The **`internal/cli → internal/ui/tui/prompt`** import MUST be removed from the production **and test** import graph of `internal/cli`; the TUI prompt MUST be reached through an **injected port** (a small interface or function value) declared in a permitted tier and wired at the composition root.
- **FR-002**: The port MUST NOT drag an unsanctioned type across its boundary: if declared in `internal/domain/**` it MUST reference only stdlib + domain types (RULE-C purity — no `ui` type); the TUI implementation satisfies it **structurally** from inside `internal/ui/**`. If the seam proves inexpressible without leaking a `ui` type, the port home is `internal/app/**` (Q2). The chosen home is recorded in `research.md` + the round's ADR.
...
- **FR-004**: `tools/arch/baseline.txt` MUST be regenerated from the gate (`make verify-architecture-update`) after the de-coupling, dropping exactly `internal/cli -> internal/ui/tui/prompt`; no RULE-A/B/C entry appears; the gate is **green on `dev` at delivery**.
- **FR-005**: The round MUST NOT add any baseline entry, weaken RULE-E, or exempt the removed edge; the sanctioned set (ADR 0016) is unchanged.
...
1. **Given** the round lands, **Then** a new **ADR** records the port, its home tier, and the ratchet movement (3 → 2), citing ADR **0011/0016/0013**.
...
- **FR-006**: A new **ADR** (next free number, confirmed in research; `docs/decisions/NNNN-<slug>.md` + the `docs/decisions/README.md` index row) MUST record the de-coupling: the port, its home tier, the behaviour-preservation claim, the baseline movement **3 → 2**, and its relation to ADR 0011/0016/0013 — including **what the change is *not*** (no user-facing change; no new normative source; the other two edges untouched).
- **FR-007**: `specs/truth/techstack.md` (**Build & Tooling**) MUST be **MODIFY**d: the Layer-discipline gate row records the new RULE-E baseline figure (**2**) and cites the ADR; any architectural row that names the inverted seam (e.g. CLI Application *Interactive TUI prompt* / *Composition root*) MUST be updated to the injected-port reality; the `verify` aggregate member is unchanged.
- **FR-008**: The round MUST leave the de-coupling of the **other two** residual edges (`→ agent`, `→ ui`) and **F-6/F-7/F-8** **out of scope** (later slices, tracked on [#101](https://github.com/gosharplite/tellme/issues/101)), and MUST state the round's slice boundary in `research.md`/`plan.md`.
...
- **A domain port that would leak a `ui` type** → the port MUST move to `internal/app/**` (Q2); a `ui`-typed domain seam fails RULE-C.
- **The adapter that calls `tuiprompt.Run`** → it MUST live in a tier ≥ 5 (`internal/ui/**`), because RULE-A forbids a lower tier importing `internal/ui/**`; a `cmd/tellme`-side adapter is fine (exempt) but the port must not require a lower tier to import the TUI package.
...
- **The `tuiPromptRunner` test seam** (`tui_dispatch_test.go`, `tui_submit_chrome_test.go`) → it MUST be adapted to the port (a test double implementing the port), not deleted silently; the nil-fallback that live-imports the TUI package must be removed.
...
- **FR-009**: The round MUST preserve **zero behavioural change** (FR-003) and MUST ship the de-coupling + baseline regeneration + truth/ADR records as **one PR** (NFR-002); it MUST NOT touch the other two residual edges, any flag/exit-code/stream contract, or any domain business logic.
- **FR-010**: The round MUST include **falsifiability witnesses** reproduced then reverted (ADR 0010): (a) re-introduce `internal/cli → internal/ui/tui/prompt` ⇒ RULE-E reds; (b) leave the now-fixed TUI edge in the baseline ⇒ the gate reds (stale); (c) a missing/false port injection ⇒ the TUI dispatch unit seam (or the build) fails loudly.
- **FR-011**: The round MUST NOT modify a delivered `specs/plans/NNN-*` package and MUST record the remaining slices + **F-4** (if not folded, Q3) + F-6/F-7/F-8 on the live issue [#101](https://github.com/gosharplite/tellme/issues/101).
...
- **The injected port** — the domain- or application-tier seam carrying `Run(ctx, in io.Reader, out io.Writer, src Source, debounce) (string, bool, error)` semantics (Q2).
- **The adapter** — the TUI-side value (inside `internal/ui/**`) that satisfies the port; wired at `cmd/tellme`.
...
- **SC-001**: `go list`/`grep` shows **no** `internal/cli` (production + test) import of `internal/ui/tui/prompt`; the RULE-E baseline lists exactly the two remaining residuals; RULE-A/B/C stay **0**; cycles **0**. (covers FR-001, FR-004)
...
- **SC-003**: A deliberately re-introduced `internal/cli → internal/ui/tui/prompt` import makes RULE-E **fail** and names the edge; a stale baseline entry makes the gate **fail** — both reproduced as falsifiability witnesses, then reverted. (covers FR-005, FR-010)
- **SC-004**: The port lives in a **permitted tier** with no `ui` type crossing into `internal/domain/**` (RULE-C preserved); the adapter stays inside `internal/ui/**`; a missing injection fails loudly. (covers FR-002, FR-010, RULE-A/C)
- **SC-005**: `specs/truth/techstack.md` (Build & Tooling) records the baseline figure **2** (citing the new ADR); the ADR + its index row are present; **no production behaviour** changes; `go.mod`/`go.sum` unchanged. (covers FR-006, FR-007, NFR-003, NFR-005)
- **SC-006**: The two remaining residual edges + F-4 (if not folded) / F-6/F-7/F-8 are recorded on [#101](https://github.com/gosharplite/tellme/issues/101); no frozen plan package is touched. (covers FR-008, FR-011)
...
- **A3 (port design is RD)** — the port's exact shape, name, and home tier are **RD** decisions (`research.md` D-x), subject to FR-002 / RULE-A·C.
...
- **A7 (scope guard)** — the round touches `internal/cli/**`, the port's home tier, `internal/ui/**` (the adapter, if any), `cmd/tellme` (wiring), `tools/arch/baseline.txt`, `docs/decisions/**`, `specs/truth/techstack.md`, the plan package, and `STATUS.md` + the day's `session-summary.md`.
...
- **F-6/F-7/F-8** (the PR #104 deferrals: narrow seams ≤2 fields; named `Discovery{Closer}`; `OutputSink` → interface) — later slices. **F-4** rides along only if Q3 locks it.
