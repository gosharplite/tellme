...
| --- | --- | --- |
| **FR-001 / W1** a warm fresh cache dials nothing | E2E `tellme never contacted the MCP server "shop"` (2 Examples: reuse + `--new`) + unit `TestDiscoverCached_WarmFreshMakesNoDial` (1 pin) + `TestLazyClient_NoConnectWhenUnused` | Force every key cold in `DiscoverCached` ⇒ **3** unit pins redden (`WarmFreshMakesNoDial` reports `dialed [u-shop]`, and forging every key cold also removes the stale entry's cache hit, so `StaleServesThenRefreshes` and `FailedRefreshKeepsPrior` red); the 2 `never contacted` Examples + (when the server is closed/failing) 2 more Examples at `the request offered the tool …` redden (**4 Examples** total). |
