...
| --- | --- | --- |
| **FR-001 / W1** a warm fresh cache dials nothing | E2E `tellme never contacted the MCP server "shop"` (2 Examples: reuse + `--new`) + unit `TestDiscoverCached_WarmFreshMakesNoDial` (1 pin; W1 reddens 3 pins in total — see the mutation column) + `TestLazyClient_NoConnectWhenUnused` | Force every key cold in `DiscoverCached` ⇒ **3** unit pins redden (`WarmFreshMakesNoDial` reports `dialed [u-shop]`, and forging every key cold also removes the stale entry's cache hit, so `StaleServesThenRefreshes` and `FailedRefreshKeepsPrior` red); the 2 `never contacted` Examples + (when the server is closed/failing) 2 more Examples at `the request offered the tool …` redden (**4 Examples** total). |
...
- **W1** every key cold ⇒ **unit 3 pins** (`TestDiscoverCached_WarmFreshMakesNoDial` reports `dialed [u-shop]`; `TestDiscoverCached_StaleServesThenRefreshes` + `TestDiscoverCached_FailedRefreshKeepsPrior` red — forging every key cold also removes the stale entry's cache hit); **E2E 4 Examples** redden — 2 at `tellme never contacted the MCP server "shop"` and 2 at `the request offered the tool "lookup_price" … alongside the agent tools` (the cold dial against the closed/failing server drops the tool).
