...
- **A5**: **Acceptance/DSL are expected NOOP-with-recorded-narrowing** — despite the diagnostic being user-visible in principle, there is **no hermetic producer** of `M < N` through the shipped binary (the loop always appends one result per call — the honest note above), so no godog Example can drive it. Its carriers live at the **unit / CLI tiers** (a fake-sink adapter pin + a `ui` formatter pin + a CLI wiring pin) — the **round-059 documented-narrowing** class (its once-per-second cadence is likewise unobservable from a flat capture). `/axb-spec-by-example` and `/axb-dsl-refine` record the narrowing (never an Example-less Rule — RF-063-10 retired).
