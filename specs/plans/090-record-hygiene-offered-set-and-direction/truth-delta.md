...
| ADD | `cmd/tellme/*_test.go` (the offered-set doc-consistency carrier) | A **test-only** check: parse the `chat/dsl.md` offered-set cell and assert set-equality with `agentTools()`. | FR-001 — the bound-to-the-code carrier that reddens on drift (W-A). |
