...
- **本輪有產品碼變更**：`internal/infrastructure/tools`（重塑 `list_files`／多檔 `read_files`、新增 `get_tree`）、`internal/infrastructure/tools/summarize.go`（刪除）、`internal/cli/cli.go`（registry factory 只提供三個工具）、`internal/agent/agentloop.go`（`reason` echo）。維持「先對齊測試層（Phase 3），再在 Feature phase 補產品碼（GREEN）」。
...
- [ ] T001 建立 21 個新句 stepdef 獨立檔骨架
  - Read:
    - `specs/truth/features/cli/chat/dsl.md`（本輪 21 個新句）
    - `tests/e2e/steps/register.go`、`tests/e2e/steps/scenario_context.go`
  - 只做：建立 21 個獨立 stepdef 檔（命名 `tests/e2e/steps/step_r021_t0NN_<slug>.go`，各自 `init()` 自我註冊空白 registrar），對到 T007–T027。
...
**Goal**: 把本輪 Feature 用到的 DSL 自動化測試對齊最新版 truth：先移除已退場工具的步驟、對齊既有 `read_files` 句到多檔 `filepaths`，再為 21 個新句建立失敗訊號；最後補非 DSL 的 `[UNIT]`。不寫產品行為。
...
**Parallel Hint**:
- T003–T030 各派一個獨立 subagent（依 `ParallelHint平行Subagent與同檔調度判準.md` 調度；`[BDD-REMOVE]`／`[BDD-ALIGN]` 觸及的既有檔若同檔則序列化）；T031 等全部回來再啟動 subagent 執行 review。
...
- [ ] T028 [P] [UNIT] 三個工具的 schema 與輸出格式
  - Read:
    - `specs/plans/021-tool-surface-parity/research.md` -> Decision 2, 3, 5
  ...
  - 撰寫：`list_files` 輸出 `Contents of <path>:` + `[d]/[f]`（含預設 path）；`read_files` 多檔 framing／request 順序／100000-byte 截斷／binary／directory `ERROR:`／≤50 上限／空 args 錯誤；`get_tree` connector 輸出／預設 `max_depth` 2／不遞迴 `.git`；三者 schema 皆含 required `reason`。
  - 落點：`internal/infrastructure/tools/filesystem_test.go`、`internal/infrastructure/tools/get_tree_test.go`。

- [ ] T029 [P] [UNIT] `reason` 被 echo 進 tool-loop log 行
...
- [ ] T030 [P] [UNIT] registry factory 只提供三個檔案工具
...
- [ ] T031 subagent review (phase quality gate)
  ...
  - 檢驗：無 undefined step；3 個移除句已消失、21 個新句 stepdef + 3 個 `[UNIT]` 皆已註冊；測試可編譯；失敗僅限尚未實作之產品行為。
...
- [ ] T032 [BDD-GREEN] 讓 Test Scope 全綠（並使 T029 的 `[UNIT]` 轉綠）
- [ ] T033 [BDD-REFACTOR] 在綠燈下整理 `logStep` 的 reason 擷取

## Phase 4B: ADD Feature File - cli/chat/reading-several-files.feature

**Goal**: 讓 `reading-several-files.feature` 全綠 — `read_files` 改為多檔 `filepaths`，含 `--- File: … ---` framing 與截斷／binary／directory／≤50 邊界。

**Shared Must Read**:
- `specs/truth/features/cli/chat/reading-several-files.feature`
- `specs/truth/features/cli/chat/dsl.md` -> 本輪 `read_files` 相關 Given/Then（T005/T006/T007–T013/T017–T020）
...
- `specs/plans/021-tool-surface-parity/research.md` -> Decision 1, 5

**Boundary**:
- 產品碼：`internal/infrastructure/tools/filesystem.go` 重塑 `read_files`（`filepaths`、framing、100000 cap、binary/dir/≤50、inline `ERROR:`）。
- `stdout` 契約不變；class-phrase 詞彙不變；`go mod` 不變。

**Test Scope**:
- `specs/truth/features/cli/chat/reading-several-files.feature`

- [ ] T034 [BDD-GREEN] 讓 Test Scope 全綠（並使 T028 的 read 部分 `[UNIT]` 轉綠）
- [ ] T035 [BDD-REFACTOR] 在綠燈下整理 `read_files` 的 framing 與邊界處理

## Phase 4C: ADD Feature File - cli/chat/listing-a-directory.feature

**Goal**: 讓 `listing-a-directory.feature` 全綠 — `list_files` 輸出 `Contents of <path>:` + `[d]/[f]`，`path` 可省略預設 `.`。
...
**Boundary**:
- 產品碼：`internal/infrastructure/tools/filesystem.go` 重塑 `list_files`（輸出格式 + 預設 path + required `reason`）。
- `stdout` 契約不變。

**Test Scope**:
- `specs/truth/features/cli/chat/listing-a-directory.feature`

- [ ] T036 [BDD-GREEN] 讓 Test Scope 全綠（並使 T028 的 list 部分 `[UNIT]` 轉綠）
- [ ] T037 [BDD-REFACTOR] 在綠燈下整理 `list_files` 的輸出構造

## Phase 4D: ADD Feature File - cli/chat/surveying-a-folder-tree.feature

**Goal**: 讓 `surveying-a-folder-tree.feature` 全綠 — 新增 `get_tree` 工具（connector tree，預設 `max_depth` 2，不遞迴 `.git`）。

**Shared Must Read**:
- `specs/truth/features/cli/chat/surveying-a-folder-tree.feature`
- `specs/truth/features/cli/chat/dsl.md` -> `a configured provider "…" whose endpoint shows the folder tree and then answers with "…"`、`tellme showed the folder tree using its get_tree tool`、`the tree shows "…"`、`the working directory contains a sub-folder "…"`
...
**Boundary**:
- 產品碼：新增 `internal/infrastructure/tools/get_tree.go`（connector tree、預設 path/depth、`.git` 不遞迴）。
- `stdout` 契約不變；`go mod` 不變。

**Test Scope**:
- `specs/truth/features/cli/chat/surveying-a-folder-tree.feature`

- [ ] T038 [BDD-GREEN] 讓 Test Scope 全綠（並使 T028 的 tree 部分 `[UNIT]` 轉綠）
- [ ] T039 [BDD-REFACTOR] 在綠燈下整理 `get_tree` 的樹狀構造
...
- [ ] T040 [BDD-GREEN] 讓 Test Scope 全綠（並使 T030 的 `[UNIT]` 轉綠）
- [ ] T041 [BDD-REFACTOR] 在綠燈下整理 registry factory
...
- [ ] T042 [CODE-REMOVE] 移除 `summarize_history` 產品碼與其註冊
...
- [ ] T043 [REGRESSION] 執行全域回歸 + falsifiability witness
  ...
  - **可偽性見證**（各觀察失敗後還原）：
    ...
    3. 讓 `get_tree` 遞迴 `.git` 或忽略 `max_depth` → `chat/surveying-a-folder-tree.feature` 失敗。
...
|:---|:---|:---:|
| `truth-delta.md` -> `/axb-technical-research` MODIFY（`specs/truth/techstack.md`） | T007–T013、T028、T034、T038、T040、T042 | PASS |
| `truth-delta.md` -> `/axb-api-plan` NOOP | 豁免（NOOP 不建任務；T043 驗證詞彙維持 11） | PASS |
| `truth-delta.md` -> `/axb-data-plan` NOOP | 豁免（NOOP 不建任務；T043 驗證 record 形狀不變） | PASS |
| `truth-delta.md` -> `/axb-dsl-refine` ADD（4 個新 feature） | T007–T027、T034–T041 | PASS |
| `truth-delta.md` -> `/axb-dsl-refine` MODIFY（`watching-the-tool-loop` + `sending-the-configured-persona` + `chat/dsl.md`） | T013、T027、T032、T033 | PASS |
| `truth-delta.md` -> `/axb-dsl-refine` DELETE（`summarising-the-conversation` + 3 rows） | T003、T004、T042、T043 | PASS |
| `research.md` -> Decision 1（multi-file `filepaths`） | T005、T011、T017、T028、T034、T035 | PASS |
| `research.md` -> Decision 2（`list_files` shape + default path） | T014、T021–T023、T028、T036、T037 | PASS |
| `research.md` -> Decision 3（add `get_tree`） | T002、T015、T024、T025、T028、T038、T039 | PASS |
| `research.md` -> Decision 4（`reason` required + echoed） | T013、T027、T029、T032、T033 | PASS |
| `research.md` -> Decision 5（cap/binary/dir/≤50） | T008–T010、T012、T018–T020、T028、T034 | PASS |
| `research.md` -> Decision 6（remove `summarize_history`） | T003、T004、T016、T026、T030、T040、T042 | PASS |
| `research.md` -> Decision 7（testing strategy；no pty） | T001、T028–T030、T043 | PASS |
| `research.md` -> Decision 8（no new dependency；no security layer） | T028、T043（`go mod tidy` graph 不變；無 `SafePath`） | PASS |
| `spec.md` -> US1/US2/US3/US4、`FR-001`–`FR-016`、`NFR-001`、`SC-001`–`SC-006` | T007–T030（對齊）、T032–T042（交付）、T043 | PASS |
| `plan.md` -> Source-code structure（`internal/infrastructure/tools`、`internal/cli/cli.go`、`internal/agent/agentloop.go`） | T002、T032–T042 | PASS |
| `plan.md` -> Scope notes（api NOOP；data NOOP；CLI end → /axb-dsl-refine） | T002、T043 | PASS |
