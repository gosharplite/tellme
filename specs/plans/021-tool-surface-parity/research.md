...
- **Decision**: `read_files` per-file behaviour matches the reference:
  ...
  - **Aggregate result cap (tellme-specific, B2)**: the **whole** `read_files` result is capped at **1 MiB** — a block is appended only while it fits; on the first block that would exceed it the tool appends `... (truncated at the read budget)` and stops (the omitted file gets no header); `list_files` and `get_tree` results are likewise capped at **1 MiB** (appending `... (truncated)`). This preserves the round-008 "cannot exhaust the context window" property: per-file 100000 × ≤50 ≈ 5 MB ≈ 6× a 200 k-token window, and tellme has no in-session overflow recovery (no pruning/summarisation).
- **Rationale**: D3 — "functionality as close as possible." The inline `ERROR:` handling keeps a bad path in one file from failing the whole read, matching the reference. The 100000 cap supersedes the round-008 1 MiB **per-file** placeholder (a tellme-local number, not an external contract), and the **aggregate 1 MiB cap (D3a)** restores the round-008 NFR-001 property that the per-file/≤50 bound alone would have lost.
...
- **Context growth**: the per-file/≤50 limits (`50 × 100000` bytes) would alone be ~6× a 200 k-token window, so the round adds an **aggregate 1 MiB cap** across `read_files` (and caps `list_files`/`get_tree`) — a recorded divergence from the reference, which has no aggregate cap and no in-session recovery. The finite cap still raises per-turn input cost (the loop re-sends the growing conversation each iteration, pruning excluded) — a known characteristic, bounded by `MAX_TOOL_LOOP`.
- **Context-bound caveat**: the 1 MiB aggregate cap matches the round-008 constant's **intent**, not a proof against every model — 1 MiB ≈ ~250 k tokens can still exceed a 200 k-token window. Tying the reader cap to the resolved `MAX_HISTORY_TOKENS` budget is a recorded **forward item**; D3a records the divergence and this caveat so the restored claim is read as precedent, not a fresh universal proof.
