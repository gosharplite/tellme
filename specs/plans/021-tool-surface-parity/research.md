...
- **Decision**: `read_files` per-file behaviour matches the reference:
  ...
  - **Aggregate result cap (tellme-specific, B2)**: the **whole** `read_files` result is capped at **1 MiB** — once the accumulated result reaches the cap it is truncated with a marker; `list_files` and `get_tree` results are likewise capped at **1 MiB**. This preserves the round-008 "cannot exhaust the context window" property: per-file 100000 × ≤50 ≈ 5 MB ≈ 6× a 200 k-token window, and tellme has no in-session overflow recovery (no pruning/summarisation).
