...
- **Decision**: sample **machine-wide** host CPU and memory by hand, POSIX-only — CPU = the Δ of (`Σcpu − idle`) across the host's `/proc/stat` ticks (the `idle` column **only**, matching the reference); memory = host used/total percent; macOS via `sysctl` for memory, with the CPU leg **pending a cgo `mach` `host_statistics` sampler** (currently `0.0%`). Put the sampler behind a small domain **port**, with the OS adapters under `internal/infrastructure/telemetry/` (build-tagged `_linux` / `_darwin`), so unit tests inject fixed readings; render the ` [CPU: <c>% | MEM: <m>%]` segment only on the tool-execution spinner, `%.1f%%`.
