...
- **本輪沒有新增第三方相依**（the spinner is hand-written in `internal/ui`; the CPU/memory sampling is stdlib POSIX; the standard-output probe reuses `golang.org/x/term`, already a direct dependency since round 012; `go.mod`/`go.sum` 不動）。依 SOP **省略 Phase 1 `Setup`**；不得把 helper、fixture 或落點骨架塞進 Setup。
...
- **本輪有產品碼變更**：`internal/ui/spinner.go`（the presenter + the frame set + the label/elapsed formatter）、`internal/ui/metrics_proc_linux.go` + `internal/ui/metrics_proc_darwin.go`（the dependency-free POSIX CPU/memory sampler）、`internal/cli/cli.go`（own the spinner lifecycle around each waiting phase；wire the standard-output probe + the `TELL_ME_FORCE_STDOUT_TTY` seam）。相位順序仍為「先對齊測試層（Phase 3），再於 Feature phase 補產品碼（GREEN）」。**不動** `stdout` bytes、the round-009 payload line、the round-017 turn chrome、the round-018 post-turn lines、the class-phrase vocabulary（仍 11）、the `-i` TUI surface。
...
- **[RESOURCES]** ` [CPU: <c>% | MEM: <m>%]` (one decimal each) — dependency-free POSIX sampling (Linux `/proc/self/stat` + `/proc/meminfo`; macOS `sysctl`/mach), behind an injected seam; **only** on the tool-execution state. 見 T002/T010/T014（research D5）。
- **[GATE]** drawn **only** when `isatty(stdout) && !-r` — wiring the standard-output probe round 006 deferred (closing round-006 / PR #16 **Obs 1**); a diagnostic `TELL_ME_FORCE_STDOUT_TTY` seam drives the branch without a pty. 見 T003/T015/T017（research D6）。
...
- **[SURFACES]** positional / piped / the round-012 reader; **not** the `-i` TUI; **not** the non-prompt paths; `stderr` only; `stdout` byte-exact; POSIX-only; no new dependency. 見 T012/T017/T019（research D6/D9）。
...
- [ ] T002 落點產品碼骨架與 `[UNIT]` 落點檔骨架
  ...
  - 只做：
    - 產品碼骨架（簽名 stub，空輸出/未接線）：`internal/ui/spinner.go`（the spinner presenter 簽名 + the frame set 常數 + the label/elapsed formatter 簽名）、`internal/ui/metrics_proc_linux.go`、`internal/ui/metrics_proc_darwin.go`（the CPU/memory sampler 簽名，behind an injected seam）、`internal/cli/cli.go`（the spinner start/stop seam + the standard-output probe hook + the `TELL_ME_FORCE_STDOUT_TTY` read）。
    - 測試落點骨架：`internal/ui/spinner_test.go`、`internal/ui/metrics_proc_test.go`（stdlib `testing` 空殼）。
...
- `internal/ui/turn.go`、`internal/ui/spinner.go`、`internal/ui/metrics_proc_linux.go`、`internal/ui/metrics_proc_darwin.go`（T002 落點）

**Boundary**:
- 一條 DSL 一個 task；落入獨立單一檔案（檔案 `init()` 自我註冊）。
- 只改該句的 stepdef／assertion／直接依賴的 helper。
- `[UNIT]` 落點：`internal/ui/spinner_test.go`、`internal/ui/metrics_proc_test.go`。
- 不寫產品碼（除 T002 已預留的骨架）。
...
- [ ] T003 [P] [BDD-RED] `Given: the operator is watching a terminal`
  ...
  - 語意：把 `TELL_ME_FORCE_STDOUT_TTY=1` 設進 subprocess 環境，使 tellme 的 **standard-output** terminal probe 回報該 stream 為 terminal（round-019 seam）。
...
- [ ] T012 [P] [BDD-RED] `Then: the run shows no progress spinner`
  ...
  - 語意：`stdout` 與 `stderr` 皆**不**帶 spinner frame。用於 piped（stdout 非 terminal）、`-r`、`--version`、`-l`、prompt-less `--new`、`-i` submit path。
...
- [ ] T014 [P] [UNIT] the CPU/memory samplers + the resource segment
  - Read:
    - `specs/plans/019-turn-spinner/research.md` -> Decision 5
    - `specs/truth/techstack.md` -> CLI Application（Turn progress spinner (operator)）
  - 撰寫：以注入的 readings 驅動 sampler parser（Linux `/proc/self/stat` + `/proc/meminfo` 的固定取樣；macOS `sysctl`/mach 的固定取樣）→ CPU 百分比與 MEM 百分比（各 `%.1f`），以及 the resource segment 的字串；macOS 專屬檔以 build tag 隔離。
  - 落點：`internal/ui/metrics_proc_test.go`。

- [ ] T015 [P] [UNIT] the spinner gate resolution (forced stdout seam + `-r`)
  - Read:
    - `specs/plans/019-turn-spinner/research.md` -> Decision 6
    - `specs/truth/techstack.md` -> CLI Terminal detection（the standard-output probe）
    - `internal/cli/cli.go`（the existing stdin probe seam — mirror it）
  - 撰寫：斷言 gate 的解析 — `isatty(stdout) && !-r`；`TELL_ME_FORCE_STDOUT_TTY` 覆寫生效；無 terminal 或 `-r` 時 spinner 不啟。
...
- [ ] T016 subagent review (phase quality gate)
  - Read:
    ...
    - `internal/ui/spinner.go`、`internal/ui/metrics_proc_*.go`、`internal/cli/cli.go`
...
**Goal**: 讓 `presenting-the-progress-spinner.feature` 全綠 — 在非 TUI prompt turn 的每個 waiting phase，於 `stderr` 發出 the live spinner（braille frame + phase label + elapsed；tool-execution 另帶 CPU/MEM），只在 `isatty(stdout) && !-r` 時畫，於 answer 前讓位且無殘留；`stdout` byte-exact。
...
**Boundary**:
- 產品碼：`internal/ui/spinner.go`（the presenter + the frame set + the label/elapsed formatter）、`internal/ui/metrics_proc_linux.go` + `internal/ui/metrics_proc_darwin.go`（the dependency-free POSIX CPU/memory sampler）、`internal/cli/cli.go`（own the lifecycle：每個 waiting phase 啟動、interleaved output 前停止、deferred stop；接線 the standard-output probe + `TELL_ME_FORCE_STDOUT_TTY`）。
- 依 `research.md` Decisions 1–9；labels 具 leading space；elapsed 為 whole seconds；resources `%.1f`；gate `isatty(stdout) && !-r`。
...
- [ ] T019 [REGRESSION] 執行全域回歸 + falsifiability witness
  - 執行 `make verify`（`gofmt`、`go vet`、`staticcheck`、`golangci-lint`、`govulncheck`）與 `go test -count=1 ./...`（含 godog）。
  - 亦確認新的 no-spinner carriers：`diagnostics/version-and-setup-diagnostic.feature`（`--version`）、`history/inspecting-the-session-history.feature`（`-l`）、`history/starting-a-fresh-session.feature`（prompt-less `--new`）、`chat/presenting-the-turn.feature`（`-i`）皆斷言 `the run shows no progress spinner`。
  ...
  - **可偽性見證 (b)（gate）**：暫時讓 the spinner 在 non-terminal `stdout`（piped）時仍畫出，確認 `presenting-the-progress-spinner.feature` 的 `the run shows no progress spinner` 失敗；觀察到失敗即還原。
...
| `specs/truth/techstack.md` -> CLI Application（Terminal detection — the wired standard-output probe + the forced seam） | T002、T015、T017 | PASS |
...
| `plan.md` -> Source-code structure（`internal/ui/spinner.go`、`metrics_proc_*.go`、`internal/cli/cli.go`；no new module） | T002、T017、T018 | PASS |
...
| operator 拍板（stop/resume A；`-i` out of scope；POSIX-only） | A → T011/T017；scope → T012/T019；POSIX → T014/T019 | PASS |
