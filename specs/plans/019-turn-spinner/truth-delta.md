...
| MODIFY | `specs/truth/features/cli/dsl.md` | Add **1 root Given row** `the operator is watching a terminal`（the round-019 `stderr` terminal seam）and **1 root Then row** `the run shows no progress spinner` — both are cross-module (the Given is used by the `chat`, `diagnostics`, and `history` modules; the negative by `chat` (`-i`), `diagnostics`, and `history`), so they live at the interface root (single authority). | Round 019 — the gate Given and the negative boundary are cross-module; no new class phrase (still 11). |
