...
| MODIFY | `specs/truth/techstack.md` | **Configuration** section: reworded the `Variable expansion` row to record that the environment lookup is injected via an `EnvLookupFunc` port defaulting to `os.LookupEnv`; reworded `Effective-value resolution & validation` to record **expand-then-validate** ordering (a mandatory field resolving to empty is rejected) and that the resolved provider is carried on the resolution outcome. | PR [#11](https://github.com/gosharplite/tellme/pull/11) architectural review findings #1 (dependency-invert the OS lookup for parallel-safe tests) and #2 (validate the post-expansion state). In-round correction to round-003's own truth output (round not yet delivered). |
...
| MODIFY | `specs/truth/features/cli/configuration/starting-with-a-configuration.feature` | Added 1 Example to the Rule `A run stops when the selected provider entry is malformed or missing mandatory fields`: a mandatory field (`URL: "${EMPTY_URL_VAR:-}"`) that resolves to empty after `${VAR:-default}` expansion is rejected as `the provider configuration is invalid`. Uses existing DSL rows only — no `dsl.md` change; topology audit re-run PASSED. | PR [#11](https://github.com/gosharplite/tellme/pull/11) architectural review finding #2: proves the non-empty mandatory-field invariant is enforced on the post-expansion state (expand-then-validate). In-round correction to round-003's own truth output (round not yet delivered). |
