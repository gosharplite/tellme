...
- The **grill-round fold** (PR #102; fixes 1–8 in `spec.md` *Grill-round fold*) adds these task directives: (1) `deps.NewTUIRegistry` (3-reader) + widen the runner seam (`. RunTUIPrompt func(ctx, res resolution, env runtimeEnv, dp deps.Dependencies) (string, bool, error)` — **TD-1**: the widened form is `dp deps.Dependencies`, **not** `opts Options`, which would be self-referential) + thread the value through `Run`/`run`/`runTUIPrompt`; (2) delete `newRenderer`, inline `ui.NewRenderer()`; (3) `domaintools.OutputSink.Enabled()` + the `internal/infrastructure/tools/**` rename; (4) thread `dp` through the 8 signatures **+ `newTurnSpinner`/`augmentRegistryWithMCP`** (metrics read after `spinnerGate`); (5) argument-taking `NewToolUsageStore`/`NewPromptTracker` signatures; (6) migrate `turn_test.go` (8) + `prompt_multiline_test.go` (5) (archive-path doubles) and rewire `cli_test.go` to a failing store double (**no** relocation); (7) rename `options`→`flags`; (8) the `cmd/tellme` test is assembler-gate + deps-smoke only (**no stream assertions**), and no `internal/cli` test file imports `internal/infrastructure/*`.
- **PR #102 review `5243337430` directives** (`/axb-tasks` must carry): **TD-1** — the `RunTUIPrompt` widened signature (above); **RF-1** — a **Foundational** task that builds the shared in-memory `defaultTestDeps(modifiers …)` fixture in `internal/cli` **before** any test migration; **RF-2** — `cmd/tellme` wires `UserHomeDir: os.UserHomeDir` **unconditionally** and every test double supplies a **non-nil** resolver.
