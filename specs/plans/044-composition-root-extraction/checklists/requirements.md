...
- **Q2↔Q6 coupling recorded**: because `internal/app/deps` is tier 2 and **cannot** import `ui` (tier 5), the **one** `ui`-typed presentation seam (`RunTUIPrompt` — **widened by fix-1** to `func(ctx, res resolution, env runtimeEnv, dp deps.Dependencies) (string, bool, error)`) lives in a cli-local `cli.Options`, **not** in `Dependencies`; the `newRenderer` var is deleted (inlined). This is *forced by the R1 tier table*, not a preference — recorded in the spec Edge Cases.
