...
- **Q2↔Q6 coupling recorded**: because `internal/app/deps` is tier 2 and **cannot** import `ui` (tier 5), the two presentation seams (`newRenderer`, `newTUIPromptRunner`) live in a cli-local `cli.Options`, **not** in `Dependencies`. This is *forced by the R1 tier table*, not a preference — recorded in the spec Edge Cases.
