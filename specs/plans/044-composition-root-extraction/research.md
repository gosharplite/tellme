...
- **`cli.Options`** (defined in `internal/cli`, which may legally import `ui`) carries `deps.Dependencies` + the **one** presentation seam `RunTUIPrompt` (type **`func(ctx context.Context, res resolution, env runtimeEnv, dp deps.Dependencies) (string, bool, error)`** — **widened by grill-fold fix-1** so `defaultRunTUIPrompt` can reach `dp.NewPromptTracker`/`dp.NewTUIRegistry`; references unexported cli types, so `cmd` cannot populate the field; it is **nil-defaulted inside `cli`**). The `newRenderer` var is **deleted** (inline `ui.NewRenderer()`; the renderer's injectable seam stays `runtimeEnv.renderer`). **Why `dp deps.Dependencies` and not `opts Options`:** `Options` contains `RunTUIPrompt`, so a runner typed on `Options` would be self-referential (TD-1).
...
**RF-1 (review directive, PR #102 review `5243337430`) — shared in-memory test fixture.** Because `deps.Dependencies` is a ~12-field bag and a partial stub with a nil func field panics on call, the round adds **one** `internal/cli` test helper — an in-package `_test.go` factory `defaultTestDeps(modifiers ...func(*deps.Dependencies)) deps.Dependencies` returning a fully-populated, **in-memory, no-op** bag (a fake gateway, `fakeStore`/`capturingUsageStore`/`noopUsageStore`/`noopPromptTracker`, empty `domaintools.NewRegistry()` for both registry funcs, no-op binders, nil metrics provider, a no-op `MCPDiscoverer`, and a non-nil `UserHomeDir`), with `modifiers` for per-test overrides. It must be built **first** (Foundational) so every test migration (the D5 table) depends on it rather than hand-stubbing — and it must import **no** `internal/infrastructure/*` (NFR-003). It is plan-side test scaffolding, so the E2E suite is untouched.

**RF-2 (review directive) — `UserHomeDir` must be non-nil.** `internal/infrastructure/history/global_prompt_tracker.go`'s `destPath()` calls `t.userHome()` **unguarded**, whereas `tool_usage_store.go:48` guards `homeDir == nil`. The round does **not** change adapter behaviour (D11), so the obligation is on the **wiring**: `cmd/tellme` MUST set `UserHomeDir: os.UserHomeDir` **unconditionally**, and every test double (RF-1's `defaultTestDeps`) MUST supply a **non-nil** resolver so `GlobalPromptTracker.destPath()` cannot panic on an uninitialised call. *(The adapter-side symmetry is a recorded non-goal — a change to `global_prompt_tracker.go` is out of scope for R2.)*
...
| A nil `UserHomeDir` panics `GlobalPromptTracker.destPath()` (RF-2) | `cmd/tellme` wires `os.UserHomeDir` unconditionally; RF-1's fixture supplies a non-nil resolver (D5) |
