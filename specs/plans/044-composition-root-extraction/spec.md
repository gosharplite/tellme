...
1. **TUI assembly (fix-1).** The `-i` suggestion source consumes a **3-reader** tool set (`cli.go:229`), **not** the 7-tool `NewToolRegistry`; the `tuiPromptRunner` seam's type (`cli.go:206`, unexported `resolution`/`runtimeEnv`) cannot receive deps. → add **`deps.NewTUIRegistry func() domaintools.Registry`**, **widen the runner seam** to receive **`dp deps.Dependencies`** (the widened type `func(ctx, res resolution, env runtimeEnv, dp deps.Dependencies) (string, bool, error)` — **not** `opts Options`, which would be self-referential), and **thread the value** through `Run` → `run` → `runTUIPrompt`. Without this, the `infratools`/`infrhistory` edges survive.
