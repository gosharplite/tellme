...
- **FR-019**: Before a server's tool is offered to the model, its argument schema MUST be **normalized and verified** to be well-formed — a JSON object declaring its offered arguments (`properties`) and its mandatory arguments (`required`) with `required ⊆ properties` (the round-031 / issue #64 invariant). An **absent / empty / `null`** schema degrades to a **freeform** object (an object accepting free-form arguments — safe by construction). A **non-object, unparseable, or otherwise unsafe** schema — including one whose `required` names an undeclared property — MUST be **skipped with a warning** (the tool is not offered and cannot 400 the turn); tellme MUST NOT offer a third-party schema verbatim when it can fail a strict provider.
...
- **FR-022**: tellme MUST expand `${VAR}` / `${VAR:-default}` expressions in the `MCP_SERVERS` string fields (`URL`, `TOKEN`, `USERNAME`, and `COMMAND` for surface consistency) **before** the entry is used — mirroring the provider-entry expansion — so an entry such as `TOKEN: "${GITHUB_TOKEN}"` authenticates (the reference expands `${VAR}` across `MCP_SERVERS` at config load; a literal token was sent and the server was warn+skipped as a misleading "could not be reached"). The expansion is **best-effort and NON-FATAL**: a variable that is unset without a default (or a malformed expression) leaves the field's **original text in place**, so an unresolved `${VAR}` neither fails the load nor turns a would-be warn+skip into a startup failure. Expansion runs before FR-002 validation (so the validator sees resolved values); it introduces **no new class phrase or exit code**, and the resolved values MUST NOT be logged (FR-017). A field whose expansion fails MUST additionally emit a **non-fatal diagnostic warning** naming the server, field, and variable, so an unresolved credential is **self-diagnosing** rather than surfacing only as an opaque skip. *(Added from the SC-002 live check; issue #67.)*
