...
- [ ] T014 [P] [BDD-RED] stepdef: `a remote MCP server "{server}" that advertises a tool "{tool}" with a malformed schema`
  - 做：啟動 fake 使其對 `{tool}` 廣告一個 **malformed** schema（非 object，或 `required` 無對應 `properties`）。
- [ ] T015 [P] [BDD-RED] stepdef: `a remote MCP server "{server}" whose tool "{tool}" reports a tool error`
  - 做：啟動 fake 使其對 `{tool}` 回 **MCP-level tool error**（`isError: true`）。
- [ ] T016 [P] [BDD-RED] stepdef: `a remote MCP server "{server}" that fails the tool call`
  - 做：啟動 fake，於 tool call 時在 **transport** 層失敗。
- [ ] T017 [P] [BDD-RED] stepdef: `the request offered the tool "{tool}" from the MCP server "{server}" alongside the agent tools`
  - 做：斷言 fake provider 記錄的 offered 定義含 `mcp_{server}_{tool}` **且** 含 native agent tools。
- [ ] T018 [P] [BDD-RED] stepdef: `the request offered no tool from the MCP server "{server}"`
  - 做：斷言無 offered 名稱以 `mcp_{server}_` 起頭。
- [ ] T019 [P] [BDD-RED] stepdef: `tellme called the tool "{tool}" on the MCP server "{server}"`
  - 做：斷言 fake MCP server 記錄了對 `{tool}` 的呼叫。
- [ ] T020 [P] [BDD-RED] stepdef: `tellme reported on stderr that the MCP server "{server}" could not be reached`
  - 做：斷言 `stderr` 含單一來源的 skip warning（點名 `{server}`）；且非 frozen class phrase。
- [ ] T021 [P] [BDD-RED] stepdef: `tellme never contacted the MCP server "{server}"`
  - 做：斷言 recording fake 記錄 **零** 連線。
- [ ] T022 [P] [BDD-RED] stepdef: `the MCP server "{server}" received the token "{token}"`
  - 做：斷言 fake 記錄 `Authorization: Bearer {token}`。
- [ ] T023 [P] [BDD-RED] stepdef: `the run continued past the failed MCP tool call`
  - 做：斷言失敗的 MCP tool call 以 **recoverable tool result** 回饋、loop 續行至最終答案（run 不中止；**非** `the tool request failed`）。
- [ ] T024 [P] [UNIT] Config validation: `MCP_SERVERS` rules
...
- [ ] T025 [P] [UNIT] Auth resolution + token-not-logged
...
- [ ] T026 [P] [UNIT] Deterministic naming (namespacing + 64-byte hash truncation)
...
- [ ] T027 [P] [UNIT] Discovery fast-fail bound + skip + sorted registration
...
- [ ] T028 [P] [UNIT] Fold additions: schema normalization (B1), tool-call failure semantics (TD1), bounded token resolution (B3), name byte-budget (TD3), timeout clamp (TD5), nil-args normalization (TD2/R2)
...
- [ ] T029 subagent review (phase quality gate)
...
- [ ] T030 [BDD-GREEN] Implement the MCP client + discovery + registration to pass the ADD'ed feature
...
- [ ] T031 [BDD-REFACTOR] Refactor under green (naming/validation/discovery seams)
...
- [ ] T032 [REGRESSION] Full regression + falsifiability witnesses + `make verify` + topology audit
