...
- **本輪是「新增能力」輪**：`/axb-api-plan` 與 `/axb-data-plan` 為 **NOOP**；`/axb-technical-research` **MODIFY** `techstack.md`（已交付）；`/axb-dsl-refine` **ADD** `specs/truth/features/cli/chat/**`（已交付）。因此：
  ...
  - **Phase 3 `Test Alignment`** 把 11 條新 DSL row 各落一條 `[BDD-RED]` stepdef，並把純函式（config validation、auth resolution、naming、discovery bound）落 `[UNIT]`。
...
> 來自 `research.md` Decisions 1–7 與 spec US1/US2/US3（FR-001..FR-017、NFR-001..NFR-005）與 Q1–Q5。

- **[TRANSPORT]** Remote **Streamable HTTP** only；`COMMAND`（stdio）entry 由 config validation **確定性拒絕**（FR-013；Q1）。
...
- **[AUTH]** 依 auth mode 解析（`auto`/`gh`/`bearer`/`basic`/`none`）：explicit `TOKEN` 優先；GitHub-hosted 主機 fallback 至 token 來源（`gh auth token` → `GITHUB_TOKEN`，memoized），否則匿名；token **不得被記錄**（FR-006/017；Q4）。
...
- [ ] T001 [SETUP] Add the MCP Go SDK dependency + smoke test
...
- [ ] T002 [FOUNDATION] Domain port `tools.MCPClient`
  ...
  - 做：新增 `internal/domain/tools/mcp_client.go` —— `MCPToolDefinition{Name, Description, InputSchema json.RawMessage}` 與 `MCPClient` interface（`ListTools(ctx) ([]MCPToolDefinition, error)`、`CallTool(ctx, name string, args map[string]interface{}) (ToolResult, error)`、`Close() error`）。**TD2：** 型別固定為 `InputSchema json.RawMessage` 與 `args map[string]interface{}`，並在 port 註解寫明 **nil args → `{}`** 的不變式（MCP wire 不得攜帶 `"arguments": null`，strict server 會拒）。**R3：** port 註解並寫明 **call-time 失敗（tool-level error 與 transport failure 皆是）一律以一個 `ToolResult`（**nil error**）回傳**，使 loop 走 **recoverable** 路徑；`CallTool` **不因 call-time 失敗回非 nil error**——非 nil error 僅保留給真正 terminal、且非 call-time 的情況（例如 `Close` 後對已關閉 client 呼叫）。network-free、protocol-free。
...
- [ ] T004 [FOUNDATION] Typed `MCP_SERVERS` config + validation skeleton
  ...
  - 只做：型別 + decode + stub。不做：validation 邏輯（T020 unit）。
...
**DSL 參照**: `specs/truth/features/cli/chat/dsl.md` -> `## Given (round 032)`（5 rows）與 `## Then (round 032)`（6 rows）。讀取每列的 **StepDef 實作語意** 欄以取得該 step 的落地契約。

**Markers**: `[P]` `[BDD-RED]`（每條新 DSL row 一條）；`[P]` `[UNIT]`（純函式）。**只動測試層，不寫產品碼。**

**Shared Must Read**: `specs/truth/features/cli/chat/using-tools-from-a-remote-mcp-server.feature`；`specs/truth/features/cli/chat/dsl.md` -> `## Given/Then (round 032)`；`specs/plans/032-mcp-client/research.md` -> `Decision 3`–`Decision 5`。
...
- [ ] T009 [P] [BDD-RED] stepdef: `a remote MCP server "{server}" that offers a tool "{tool}" answering "{result}"`
  - Read: `chat/dsl.md` -> 對應 row（StepDef 實作語意）
  - 做：實作 stepdef：寫入 `MCP_SERVERS.{server}`（URL = fake）並啟動 fake MCP server（list `{tool}`；call 回 `{result}`）。不寫產品碼。
- [ ] T010 [P] [BDD-RED] stepdef: `a remote MCP server "{server}" that never answers"`
  - 做：寫入 `MCP_SERVERS.{server}` 並啟動 never-answers fake。
- [ ] T011 [P] [BDD-RED] stepdef: `a remote MCP server "{server}" that is marked off"`
  - 做：寫入 `MCP_SERVERS.{server}` with `ENABLED: false` + recording fake。
- [ ] T012 [P] [BDD-RED] stepdef: `a remote MCP server "{server}" that requires the token "{token}" and offers a tool "{tool}" answering "{result}"`
  - 做：寫入 `MCP_SERVERS.{server}` with `TOKEN`；啟動要求 `Authorization: Bearer {token}` 的 fake。
- [ ] T013 [P] [BDD-RED] stepdef: `a configured provider "{provider}" whose endpoint asks tellme to use the MCP tool "{tool}" from the server "{server}" and then answers with "{answer}"`
  - 做：script fake provider 回一個 tool-call（名稱 `mcp_{server}_{tool}`），再一次回 `{answer}`。
- [ ] T014 [P] [BDD-RED] stepdef: `the request offered the tool "{tool}" from the MCP server "{server}"`
  - 做：斷言 fake provider 記錄的 offered 定義含 `mcp_{server}_{tool}`。
- [ ] T015 [P] [BDD-RED] stepdef: `the request offered no tool from the MCP server "{server}"`
  - 做：斷言無 offered 名稱以 `mcp_{server}_` 起頭。
- [ ] T016 [P] [BDD-RED] stepdef: `tellme called the tool "{tool}" on the MCP server "{server}"`
  - 做：斷言 fake MCP server 記錄了對 `{tool}` 的呼叫。
- [ ] T017 [P] [BDD-RED] stepdef: `tellme reported on stderr that the MCP server "{server}" could not be reached`
  - 做：斷言 `stderr` 含單一來源的 skip warning（點名 `{server}`）；且非 frozen class phrase。
- [ ] T018 [P] [BDD-RED] stepdef: `tellme never contacted the MCP server "{server}"`
  - 做：斷言 recording fake 記錄 **零** 連線。
- [ ] T019 [P] [BDD-RED] stepdef: `the MCP server "{server}" received the token "{token}"`
...
- [ ] T022 [P] [UNIT] Deterministic naming (namespacing + 64-byte hash truncation)
  - Read: `research.md` -> `Decision 5`；T005 helper
  - 做：unit —— `mcp_<server>_<tool>`；超長 → rune 截斷 + 8-hex SHA-256；兩個同名 tool 跨 server 不碰撞。
...
- [ ] T024 [P] [UNIT] Fold additions: schema normalization (B1), tool-call failure semantics (TD1), bounded token resolution (B3), name byte-budget (TD3), timeout clamp (TD5), nil-args normalization (TD2/R2)
...
**Shared Must Read**: `specs/truth/features/cli/chat/using-tools-from-a-remote-mcp-server.feature`；`chat/dsl.md` -> `## Given/Then (round 032)`；`specs/truth/techstack.md` -> MCP Client rows；`research.md` -> `Decision 1`–`Decision 6`。
