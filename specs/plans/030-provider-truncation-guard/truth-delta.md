...
| Action | Truth Spec | Change Summary | Reason |
| --- | --- | --- | --- |
| ADD | `specs/truth/features/cli/chat/refusing-a-cut-off-reply.feature` | New interface feature (chat module) with 2 atomic Rules — (1) a reply cut off while asking tellme to change a file is refused, and the file is untouched (a cut-off create writes nothing; a cut-off edit leaves the file as it was); (2) a reply cut off before the answer is finished is refused, not shown as complete (a cut-off answer is not printed — default and Gemini families). | `acceptance-coverage` for the round-030 acceptance rule `refusing-a-cut-off-reply.feature`. |
| MODIFY | `specs/truth/features/cli/chat/dsl.md` | **Added** a round-030 section: 4 Given rows (a provider cut off at the output limit while **creating** / **editing** a file; a default provider cut off before finishing its answer; a **Gemini** provider cut off before finishing its answer) + 2 Then rows (`tellme creates no file "{name}"`; `tellme prints no answer`). | The new guard steps must be executable and uniquely owned (`dsl-exact-one-match`); the module note records the round. |
