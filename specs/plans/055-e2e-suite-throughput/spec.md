...
- **FR-005**: The subset MUST be selected **without editing `specs/truth/**`** — via a **`Makefile` target `test-fast`** that passes a `godog.paths` selection into the suite (Q2 → A). A `SUBSET — NOT THE GATE` banner MUST name the selection, and the selection MUST be **refused when it resolves to the full `features/cli` root** — enforced on **resolved paths** (the set of `.feature` files), so a traversal (`…/cli/../cli`) or an all-modules list is caught, and enforced in the **harness** so that **both** entry points (`make test-fast` and a hand-typed `go test -args -godog.paths`) obey it. The default selection is the **non-`chat` modules** (43 Examples, ≈2 s); **`E2E_FAST_MODULES=<comma list>`** overrides it (round-055 review TD-055-3: the shipped name, not `MODULES`).
