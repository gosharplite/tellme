...
- [X] **T010 `[REGRESSION]`** — `make verify` **green** with `tools/arch/baseline.txt` **byte-identical** (RULE-E **0 new / 0 stale**, baseline **2**; RULE-A/B/C **0**; **0** cycles; **RULE-F** surface `internal/cli -> internal/agent == {AgentLoop}` (0 new / 0 stale); lint 0; govulncheck clean; cross-compile 4/4) · `go test -count=1 ./...` green (incl. the godog E2E) · the Gherkin/DSL topology audit **unchanged** · `gofmt`/`go vet` clean. **RULE-F (TD-1, the machine carrier for the DoD)**: the round's *"CLI names exactly `AgentLoop` from `internal/agent`"* claim is now enforced by the guard, not a human grep; `internal/agent` declares **none** of the three contracts (no alias). **Falsifiability witnesses** reproduced then reverted (ADR 0010): (a) **compile-level** — delete a domain declaration ⇒ the loop/CLI seam **fails to compile**; (b) a **flipped `Result` field name** ⇒ compile break; (c) **count-level (RULE-F)** — add a *surviving* `internal/agent` identifier (e.g. `var _ = agent.DefaultToolTimeout`) ⇒ RULE-F **FAILS** (*"NEW coupling-surface identifier(s): [DefaultToolTimeout]"*) **with the baseline still green** (the exact evidence RULE-E alone cannot produce). **Boundary**: verification only.
...
- **Gate**: `make verify` **OK** — `verify-architecture` reports RULE-E **0 new / 0 stale** with the baseline **byte-identical** at **2** (`internal/cli -> internal/agent`, `internal/cli -> internal/ui` — **unchanged**, as designed); RULE-A/B/C **0**; **0** cycles; **RULE-F** surface `internal/cli -> internal/agent == {AgentLoop}` (0 new / 0 stale); lint 0 issues; govulncheck clean; cross-compile **4/4**.
...
- **Falsifiability witnesses (reproduced then reverted, ADR 0010)**: (a) **compile-level** — deleting a domain declaration breaks the loop/CLI seam at compile time; (b) a **flipped `Result` field name** (`Answer`→`AnswerX`) breaks the same seam at compile time; (c) **count-level (RULE-F / TD-1)** — adding a surviving `internal/agent` identifier (`var _ = agent.DefaultToolTimeout`) makes **RULE-F FAIL** (*"NEW coupling-surface identifier(s): [DefaultToolTimeout]"*) while the baseline stays green — the machine evidence that RULE-E's edge granularity alone could not produce.
