...
## Decision 7: The DoD is **the gate + the identifier-count check + unit seams** — not the E2E suite

- **Decision**: prove the round by (i) the gate (RULE-E reports the baseline **2**, 0 new / 0 stale; RULE-A/B/C **0**; cycles **0**) and (ii) an **identifier count** — `internal/cli` production references exactly **one** `internal/agent` identifier (`agent.AgentLoop`) — and (iii) the existing turn unit pins + the godog E2E run green as **regression**, **plus** three falsifiability witnesses reproduced then reverted (ADR 0010): (a) leave one extracted contract referenced from `internal/agent` in `internal/cli` ⇒ the identifier count is **2+**, not 1 (incomplete extraction); (b) flip a domain contract field ⇒ the CLI seam **fails to compile**; (c) a partial extraction leaving a second coupling ⇒ rejected by the identifier-count check.
- **Rationale**: #92 **AC5** — the witness is the gate + unit seams; a green E2E suite says nothing about which package the CLI imports (the round-009 trap). The identifier count is the machine-checkable expression of "the surviving coupling is edge-sized".
- **Alternatives considered**: **rely on the green E2E suite** — rejected; **author a new plan-side acceptance Feature** — no user-facing behaviour to express (the round-042 A3/A6 precedent) — rejected.
