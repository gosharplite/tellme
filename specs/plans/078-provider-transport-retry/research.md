...
**D3 — Schedule and bound: named constants, no config.** `retryDelays = {1s, 3s}` → **at most 2 retries / 3 attempts**, then the frozen phrase + exit 6. Fixed constants (mirroring round-076's `maxUnknownToolFolds`), pinned by a **literal** unit test (the round-064/076 literal-pin precedent). A hermetic seam `TELL_ME_FORCE_RETRY_DELAY_MS` (milliseconds; unset/invalid = the real delays) mirrors `TELL_ME_FORCE_TOOLOUTPUT_IDLE_MS` (`internal/cli/cli.go:1001`, `internal/ui/coordinator.go:44`) so the E2E asserts **count/order**, not wall-clock. *Rejected:* config keys (the operator asked for "simple"; a knob is a forward item); exponential/jitter (the reference's policy — not adopted).
...
**D5 — The retry diagnostic: one plain, non-class line on `stderr` only.** `tellme: retrying the provider request in <delay> (attempt <n> of <max>): <folded detail>` — deliberately **NOT** the frozen class phrase (so `the provider request failed` appears **exactly once**, on final failure — protecting the E2E assertion and operator comprehension). Control-free, best-effort, never on `stdout`, never in `turns.log`. Emitted through the CLI's existing terminal-safe line/yield seam (ADR 0014/0015) so it cannot tear the spinner frame; TTY-accent-free. *Rejected:* reusing the class phrase (collides with the failure assertion, confuses operators); silence (a 4 s stall with no explanation).
