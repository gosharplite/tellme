...
| **S-7** | **The hermetic test seam** — how the E2E proves the retry count without paying the real 1 s/3 s (an injected sleeper / clock port vs. accepting the delay), and how the fake provider scripts a transport drop / fail-once-then-succeed. | research decision (D-x) |
