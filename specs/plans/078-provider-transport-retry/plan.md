...
- **N-1 — the retry line must NOT carry the `tellme: ` prefix.** The round-017 *failed provider request* carrier asserts **exactly one** `tellme: `-prefixed line (the class phrase). A retry line prefixed `tellme: ` would break it, so the retry line is **chrome-styled** (`[HH:MM:SS] retrying …`) — consistent with the turn chrome (which carries no `tellme:` prefix) and with the class-phrase vocabulary (`dsl.md`). Pinned by the unit test (`TestRetryNotifier_*`) **and** the E2E (the `drop once` Example now carries `tellme announces on stderr that it is retrying the provider request`, so the retry line — and its no-prefix shape — is observed on the failed-request surface).
- **N-2 — the retry line must YIELD/RESTORE the spinner frame.** The round-019 *no progress spinner residue* carrier would otherwise see an un-cleared frame (the retry line written while the frame is live). The notifier clears the indicator (ADR 0014's yield route) before the line and restores it after, so the frame never survives into the line. Pinned by a **unit** pin on the `[yield, write, restore]` order (through the narrow `retryYield` seam, so no full `render.Indicator` is needed — fold F-078-3); the E2E carries no-residue on the non-retry path.
