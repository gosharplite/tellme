...
- [X] T001 建立 stepdef 落點骨架（獨立檔、`init()` 自我註冊）
  - Read:
...
- [X] T003 [P] [BDD-ALIGN] `Then: the pre-flight payload line is separated from the answer by a single blank line`
...
- [X] T004 [P] [BDD-RED] `Then: the submitted prompt "{prompt}" is echoed on the diagnostic output`
...
- [X] T005 [P] [BDD-RED] `Then: the prompt is not echoed on the diagnostic output`
  - Read: `specs/truth/features/cli/chat/dsl.md` -> 該 Then
...
- [X] T006 [P] [BDD-RED] `Then: the interactive prompt is cleared`
...
- [X] T007 [P] [UNIT] 模型於 submit/abort 清除 frame（`internal/ui/tui/prompt`）
  - Read:
...
- [X] T008 [P] [UNIT] `-i` submit 的 echo/chrome wiring（`internal/cli`）
  - Read:
...
- [X] T010 [BDD-GREEN] 讓 Test Scope 全綠（並使 T006–T008 轉綠）
- [X] T011 [BDD-REFACTOR] 在綠燈下整理 `-i` submit 的 teardown／echo／chrome 落點
...
- [X] T012 [BDD-GREEN] 讓 Test Scope 全綠
- [X] T013 [BDD-REFACTOR] 整理 feature header note 與 Rule 群
...
- [X] T014 [BDD-GREEN] 讓 Test Scope 全綠
- [X] T015 [BDD-REFACTOR] 整理重錨後的註解與停用字詞
