...
| MODIFY | `specs/truth/features/cli/chat/dsl.md` | ADD `the interactive prompt is cleared` · `the submitted prompt "{prompt}" is echoed on the diagnostic output` · `the prompt is not echoed on the diagnostic output` · `the pre-flight payload line is separated from the answer by a single blank line`; DELETE `the tool loop added no blank line before the answer`; round-019/rond-022 notes updated + a round-023 note. | FR-003/FR-007/FR-008; the round-022 re-anchor. |
