...
- **Review fold (PR #88, reviewed `c88ee46` → fold `b5197a0` + ledger `48b5bbc`) — APPROVE WITH REQUIRED FOLDS, all landed:** **R-1** (required, pre-merge) the round mis-identified the hardcoded 2 s budget as *the production fast-fail constant* (production is `mcpDiscoveryBound = 3 s`; the 2 s literal mirrors the sibling `ghWaitDelay`) → the rule was reworded to *"a test must not hardcode a tight wall-clock budget that is not its subject"* across ADR 0010 (+ the provenance lesson), the decisions index, `spec.md`, `research.md`, `plan.md`, this file, `truth-delta.md`, `techstack.md`, `STATUS.md` · **R-2** the F-1 exit-code fold swept from its 8 remaining `elapsed >= bound` sites (ADR Consequences, `research.md` ×5, `plan.md` ×2) · **R-3** the ADR residual recorded (ceiling validated in the `-count=20 <pkg>`-under-e2e config; whole-suite package parallelism not covered; the ceiling adds no discriminating power under D3) · **R-4** the bound-headroom forward item filed as [#89](https://github.com/gosharplite/tellme/issues/89) + referenced from the ADR · **R-5** nits (`ee.ExitCode()` quote, "three"→"four" sub-rules, the ceiling-is-a-harness-value sentence, the STATUS head-block regeneration). No product or harness behaviour change.
- **Fold re-review (PR #88, reviewed `3cb8ec8` → fold: *"docs(041): fold PR #88 re-review — R-6 (#87 body), R-7/R-8 (ADR 0010), R-9 convention"*) — CERTIFIED; R-6/R-7/R-8 landed (R-9 = a closeout-log convention):** **R-6** (merge gate) issue [#87](https://github.com/gosharplite/tellme/issues/87)'s **body** rewritten to the measured cause (hardcoded 2 s literal; production `mcpDiscoveryBound = 3 s`; PATH not the cause) + the shipped D1–D4 + rejected alternatives + the sharpened acceptance (`-count=20 -timeout 30m ./...` under contention) + the shipped *Harness* text; the 13:06 status comment marked **Superseded** · **R-7** ADR 0010 **D2** forward pointer to its own Residual · **R-8** the provenance lesson re-attributed to the inherited #87 comment (`5713960582`, ratified unverified) — *"a reviewer's claim is not evidence either"* · **R-9** convention: a ledger row names the reviewed head + the **content-fold head**; the ledger commit identifies itself by content, never by its own SHA.
