...
| --- | --- | --- | --- |
| MODIFY | `specs/truth/techstack.md` — **Turn progress spinner** row | The elapsed display becomes a **dual timer** `({total}s {call}s)` (both **unlabelled**): the **total** since prompt capture (turn-scoped; **never reset** — round-019 D4 preserved) **plus** the **current model call's** elapsed, **reset at each AI-endpoint call** (stamped in `OnInferenceStart`; an **amendment** to round-019 D4; *a turn = the whole prompt exchange, a model call = one AI-endpoint call*). And, during a `[Tool Output]` block, the spinner is **no longer hidden for the whole block**: after an **idle gap** (default **3 s**; seam `TELL_ME_FORCE_TOOLOUTPUT_IDLE_MS`, ms, `0` = admit immediately, unset/invalid = the default; the watcher polls on the ~200 ms cadence) the indicator **resumes**, and the next complete output line is preceded by a **synchronous clear** — the invariant is **mutual exclusion + join** (admit under the block-writer mutex; clear = the goroutine-joined `deactivate()`; lock order block-mutex → spinner-mutex; the redraw goroutine draws the first frame) — a **supersession of round-034's whole-block pause** (ADR 0005 D7). Presentation-only. | `spec.md` FR-001..FR-008; `research.md` D1–D5. |
| ADD | `docs/decisions/0009-spinner-dual-timer-and-streaming-liveness.md` (+ the `docs/decisions/README.md` index row) | Records the dual-timer policy (per-AI-call reset; the round-019-D4 amendment) and the idle-gap liveness (**mutual exclusion + join** — the presenter's goroutine-joined clear; the resume-only admission path); **supersedes ADR 0005 D7** and carries the **three** recorded reference divergences (per-phase figure; no reference liveness; **unlabelled** figures). | `spec.md` FR-001/FR-007; `research.md` D1–D5/D8. |
...
| MODIFY | `specs/truth/features/cli/chat/presenting-the-progress-spinner.feature` — new `Rule: The spinner shows the total time and the current model call's time` (+ the two tool-phase Examples amended) | ADD a Rule with two Examples asserting the two-figure shape (single-call and tool-using); the existing tool-phase Examples gain the times Then. The per-call reset arithmetic is a unit pin. | `spec.md` FR-005–FR-008; `research.md` D1–D2. |
| MODIFY | `specs/truth/features/cli/chat/dsl.md` — `the progress spinner does not appear while the command's output streams` → `the run shows the progress spinner again while the command stays quiet`; + a `## Given (round 040)` (the idle-gap state Given `the command stays quiet for longer than the spinner's idle gap`, pinning the seam `TELL_ME_FORCE_TOOLOUTPUT_IDLE_MS`, + a quiet-command provider Given) + a `## Then (round 040)` (the dual-timer Then `the progress spinner shows the time since the prompt and the current model call's time`) + a round-040 note | Rows matching exactly one step each (`dsl-exact-one-match`); `dsl-single-authority` preserved (the streaming row is modified in place, the others are new). The no-interleave/no-residue and the per-call-reset facets are recorded as unit pins in the rows. | `spec.md` FR-001–FR-008; `research.md` D4/D6. |
| MODIFY | `specs/truth/features/cli/chat/dsl.md` — `the run shows the progress spinner while it waits` + `the progress spinner shows how long it has waited` (rows updated **in place** to the dual `({total}s {call}s)` shape) + the round-019 note's round-040 pointer | The two general spinner rows' stepdefs were widened to the dual shape with the implementation; the rows followed (PR **#86** review **R-40-2** — an implementation-outran-the-truth fold). | `spec.md` FR-005/FR-006 (the dual figure); `research.md` D1–D2. |

## Governance (ADR)

| Action | Artifact | Change Summary | Reason |
| --- | --- | --- | --- |
| ADD | `docs/decisions/0009-spinner-dual-timer-and-streaming-liveness.md` (+ the `docs/decisions/README.md` index row) | Records the dual-timer policy (per-AI-call reset; the round-019-D4 amendment) and the idle-gap liveness (**mutual exclusion + join**; goroutine-joined clear; the resume-only admission path); supersedes ADR 0005 D7; carries the **three** recorded reference divergences. | The change to two `Accepted` decisions needs a durable, citable home (not a frozen plan package). |
