Feature: Timing the current turn
  ...
  # the current turn has been running — so a slow single call can be told apart from a slow series of
  # calls. The first figure keeps counting from the prompt; the second restarts for each model call.

  Rule: The indicator shows the total wait and the current turn's time
    ...
    Example: A single-call answer shows both times
      ...
      Then the progress indicator shows the time since the prompt and the current turn's time
      And tellme exits successfully

    Example: A tool-using answer restarts the turn time for each model call
      Given the operator has a runnable tellme installation
      And the runtime home is "ait-tmg"
      And the diagnostics are shown at a terminal
      ...
      Then the progress indicator shows the time since the prompt and the current turn's time
      And the current turn's time restarts when a new model call begins
      And tellme exits successfully
