...
- **SC-005**: The change has **falsifiability witnesses**: removing the idle-gap resume fails the WS-A Example; **freezing the second figure (no per-call reset) fails the *unit* pin** (T006's second-`OnInferenceStart` sub-assertion — the WS-B E2E Example asserts the **shape** only, per SC-003); widening the line to two 3+-digit figures without the row-aware clear strands a residue frame — each reproduced then reverted. *(The reset-witness attribution is reworded by round-040 review **R-14** to the unit layer — an accuracy fix, PM-owned.)*
