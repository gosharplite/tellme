...
| E2E (`tests/e2e`) | a scripted quiet-then-resuming command with a **real** idle gap — a small **nonzero** forced threshold (e.g. 50 ms) plus a **child** `sleep` (not a Go `time.Sleep`, so `verify-no-test-sleep` is untouched) — shows a spinner frame **inside** the block window on the captured `stderr` (SC-002). |
