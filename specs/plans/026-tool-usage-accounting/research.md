...
- **Decision**: persist one JSON object per line — `{"timestamp":"<RFC3339>","tool":"<wire name>","outcome":"<ok|error|timeout>"}` — appended to `~/.tellme/tools-count.jsonl` (the user home resolved via `os.UserHomeDir()`), opened `O_APPEND|O_CREATE|O_WRONLY` (0o644). The file is **global** (shared across repos, envs, and personas) and **never reset, archived, or truncated by `--new`**. There is **no** summary companion — the report reads the log once and sums.
...
- **Rationale**: mirrors the established offline reporting commands (`-l`, `-d`) so the prompt path's `stdout` stays byte-exact; a global file naturally yields a machine-wide view; listing every registered tool makes **zero-use** tools visible (the prune signal); a fixed registry order makes the output **deterministic** independent of the log's line order (FR-009).
- **Alternatives considered**:
