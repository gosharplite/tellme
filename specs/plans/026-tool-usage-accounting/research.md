...
- **Tie-break (required fold, review finding 3)**: in `execute_command`'s `runCaptured` the byte budget ("trimmed") and the deadline ("stopped") can both fire; the tool then returns a **truncation** result while the loop's deadline signal is true — FR-002 and the structural signal can disagree. **DECISION: the accounting follows the loop's deadline signal** (records `timeout`), and the tool's trim-vs-stop guard remains a *result-shape* concern (the result text stays the bounded success). The boundary is **unit-pinned** in T015.
...
- **Decision**: the loop records each executed invocation through an injected sink — a small domain port `internal/domain/history.ToolUsageSink` with **`Record(tool string, outcome ToolOutcome) error`** (the shipped signature — reconciled post-implementation, implementation review B), **nil = no-op**. The **adapter** stamps the timestamp and builds the persisted `ToolUsageRecord` (the log's on-disk shape, used by the store's marshal/`Load`), so the loop never fabricates a timestamp. The CLI constructs the file-backed adapter (`~/.tellme/tools-count.jsonl`) and injects it, mirroring `gatewayFactory` / `historyStoreFactory` / `newUsageStore`. A sink error is **swallowed** (best-effort); `internal/agent` stays free of filesystem/`os.UserHomeDir` coupling. The classification (Decision 1) is computed in the loop; the sink only writes.
...
- **Decision**: add a new boolean flag — the **`--tool-usage`** flag (the name is **pinned** by this round's truth) — that, when set, prints the per-tool roll-up to **`stdout`** and exits. It is **strictly offline** (no provider request, no stdin read) and is evaluated in the dispatch precedence **before** any prompt/stdin access. **Footprint**: it is **`--version`-class**, *not* `-l`-class (see the Footprint bullet below) — it requires **no** `-c`, **no** `TELL_ME_HOME`, and **no** workspace. The report lists **every live-registry tool** in the registry's offer order (today: `list_files`, `read_files`, `get_tree`, `execute_command`) with, per tool, total invocations and the `ok` / `error` / `timeout` breakdown, aggregated over the **whole** global log in a **single streaming pass** (O(tools) memory — NFR-001). Only the report's *line wording* remains a CLI-contract-owner (`/axb-dsl-refine`) detail.
...
- **Recoverable inline failures count as `ok`** (forward note, implementation review). Per the operator-locked Q1 (structural signals only), `error` = a non-nil Go error; but `read_files` renders a missing/unreadable file as an inline `ERROR: …` result with a **nil error** (the round-024 contract), so it is recorded `ok`. Correct *by contract*, but a blind spot for the slice's purpose ("which tools fail — fix candidates"): a file-not-found is logged as a success. A future `error` axis could admit a tool-declared recoverable-failure signal.
- **Pruning erases the prune signal** (forward note). The report enumerates the **live registry**, so a tool removed in a later round silently drops its historical counts — exactly when the measurement is acting on that evidence. A future "no longer registered" line for log-only tool names would preserve it.
