...
│                                  #   (os.UserHomeDir; lazy-create on first Record; O_APPEND|O_CREATE; append-only JSONL; streaming aggregate)
├── ui/
...
internal/domain/history/tool_usage_test.go   # NEW — the record/port types + the store round-trip + streaming aggregation
internal/agent/tool_usage_test.go            # NEW — the loop outcome classification (incl. the trim-vs-deadline tie-break)
internal/ui/toolusage_test.go                # NEW — the report formatter
...
2. `Global tool-usage log (local-state interface)`
   ...
   - Primary interface: the **append-only JSONL record** `{"timestamp":"<RFC3339>","tool":"<wire name>","outcome":"ok|error|timeout"}` at `~/.tellme/tools-count.jsonl` — one record per executed invocation; global (shared across repos/envs/modes); never reset by `--new`; **streaming-aggregated** by the report (never read whole into memory).
...
*(none — the operator locked the outcome taxonomy (Q1 → 1), the storage shape/location (Q2 → Others: `~/.tellme/tools-count.jsonl`), and the surfacing channel (Q3 → 1: a dedicated offline flag); `research.md` Decisions 1–6 settled the classification signals, the counting seam, the report shape, and the hermetic verification split. **Open (non-blocking):** the report's exact line *wording* (the flag name `--tool-usage` is **pinned**), the CLI module home (`/axb-dsl-refine`), and the record's DBML prose (`/axb-data-plan`). None gate this round.)*
