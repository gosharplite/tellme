...
| `tellme exits with the environment error code` | 無 | 不支援 | `碼值`: `4` (**pinned**, round 002 / FR-005) — a non-zero code distinct from success (0) and from every other error class (usage 2, configuration 3, diagnostic 5). | `必查`: `呈現結果`: the exit code **equals the pinned environment error code `4`**. `不該發生`: it must not collapse to the success code or to any other error class. |
