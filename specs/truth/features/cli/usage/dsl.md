...
| `the help is reported as a success` | 無 | 不支援 | `通道`: standard output + standard error. `來源`: the round-074 help path. | `必查`: `呈現結果`: the exit code equals the success code (0) and the captured **standard error is empty** — no `tellme: …` class phrase. `不該發生`: a help request must not be reported as a failure (no `tellme: ` line, no non-zero exit code). (The "went to standard **output**" half is carried by the paired `tellme prints its flag list` Then — the two are asserted together.) |
