...
| `tellme prints its flag list` | 無 | 不支援 | `通道`: standard output. `來源`: the round-074 help path. | `必查`: `呈現結果`: the captured standard output carries the flag list — the new `-h`/`--help` and `-v`/`--version` flags appear, alongside the existing `-c`, `-d`, `-i`, `-l`, `-r`, `-t`, `--new`, `--tool-usage`. `不該發生`: the help must not be empty, and it must not omit the new `-h`/`--help` or `-v`/`--version` flags. |
| `the help is reported as a success` | 無 | 不支援 | `通道`: standard output + standard error. `來源`: the round-074 help path. | `必查`: `呈現結果`: the exit code equals the success code (0), the help block went to standard **output**, and the captured **standard error is empty** — no `tellme: …` class phrase. `不該發生`: a help request must not be reported as a failure (no `tellme: ` line, no non-zero exit code). |
