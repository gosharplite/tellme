...
| `the run shows no progress spinner` | 無 | 不支援 | `通道`: standard output + standard error. `來源`: the turn spinner — a braille frame followed by a phase status. | `必查`: `呈現結果`: neither the captured standard output nor standard error carries a spinner frame (a braille frame followed by a phase status such as `Thinking …` or `Executing …`). `不該發生`: the spinner must not appear when the diagnostic stream (`stderr`) is not a terminal, under `-r`, on any non-prompt path (`--version`, `-l`, a prompt-less `--new`), or on the `-i` interactive-prompt submit path. |
