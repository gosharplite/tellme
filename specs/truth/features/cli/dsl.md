...
| `the session history already holds the exchanges:` | 無 | 支援（表頭 `prompt`, `answer`）。 | `工作區`: writes into `$TELL_ME_HOME/output/<mode>/history.jsonl` (the effective mode's session workspace). | `怎麼做`: create the per-mode session workspace directory if absent, then write one JSON line per table row (`prompt`, `answer`) into `history.jsonl`, in table order. `權威狀態落地`: the session history file holds the arranged exchanges. `回寫`: the session history file. |
| `the diagnostics are shown at a terminal` | 無 | 不支援 | `來源`: arranged via the diagnostic environment seam `TELL_ME_FORCE_STDERR_TTY`. | `怎麼做`: set `TELL_ME_FORCE_STDERR_TTY` to `1` in the subprocess environment so tellme's **standard-error (`stderr`)** terminal probe reports the stream as a terminal (the round-019 interface-root row, shared by the `chat`, `diagnostics`, and `history` modules; mirroring the round-012 stdin seam). `權威狀態落地`: the effective `stderr` terminal probe reports a terminal. `回寫`: none. |
...
| `tellme explains on stderr that "{reason}"` | `reason`: string; the **frozen class phrase** for a failure class (verbatim phrase). | 不支援 | `片語詞彙`: the eleven frozen class phrases — `the configuration could not be found`, `no configuration could be found`, `the configuration could not be parsed`, `the configuration is invalid` (a well-formed file whose resolved values fail validation — e.g. a negative rendered width, round 006), `the selected provider is not in the registry`, `the provider configuration is invalid`, `the provider request failed`, `the tool request failed` (a prompt run whose tool loop cannot complete — round 008), `the workspace path is not a directory`, `the runtime home is not usable` (one class for the *not-usable*, *unset*, **and** *standard-input-unreadable* paths), `the command-line usage is invalid`. | `必查`: `呈現結果`: exactly **one** stderr line begins with `tellme: `, and that line starts with `tellme: {reason}`; any class-specific trailing detail (a path, a provider name, an OS error string) is **not part of the frozen contract**. `不該發生`: the failure reason must not be swallowed or emitted only on stdout, and no second `tellme: `-prefixed line may exist. |
| `the run shows no turn chrome` | 無 | 不支援 | `通道`: standard output + standard error. `來源`: the turn chrome — the input-capture acknowledgement, the 80-column `─` rule, and the `╭─⠿ Turn …` header. | `必查`: `呈現結果`: neither the captured standard output nor standard error carries the input-capture acknowledgement, the 80-column `─` rule, or a `╭─⠿ Turn …` header line. `不該發生`: the turn chrome must not appear on the `-i` interactive-prompt submit path, on a `--version` run, on a prompt-less `--new`, or on any other non-prompt path. |
| `the run reports no post-turn status` | 無 | 不支援 | `通道`: standard output + standard error. `來源`: the post-turn status — the metrics line and the `╰─⠿ Ready` summary. | `必查`: `呈現結果`: neither the captured standard output nor standard error carries a metrics line (`[HH:MM:SS] [<provider>] M: … H: … C: … Th: …`) or a `╰─⠿ Ready` summary line. `不該發生`: the post-turn status must not appear when the provider reports no usage, nor on any non-prompt path (`--version`, `-l`, or a prompt-less `--new`). |
| `the run shows no progress spinner` | 無 | 不支援 | `通道`: standard output + standard error. `來源`: the turn spinner — a braille frame followed by a phase status. `讀法`: read each stream as a terminal would (each `\n`-delimited line reduced to the text after its last carriage-return, with the erase-to-end-of-line applied). | `必查`: `呈現結果`: neither the captured standard output nor standard error shows a spinner **residue** — read as a terminal, the surviving text carries no braille frame followed by a phase status such as `Thinking …` or `Executing …`. (A failed provider turn draws one frame and then synchronously clears it, so it leaves no residue.) `不該發生`: the spinner must not appear when the diagnostic stream (`stderr`) is not a terminal, under `-r`, on any non-prompt path (`--version`, `-l`, a prompt-less `--new`), or on the `-i` interactive-prompt submit path. |
