Feature: Presenting the turn
  ...
  # the positional/piped prompt turn and the round-012 plain reader only; the `-i` interactive prompt and
  # the non-prompt paths show no chrome. `stdout` stays byte-exact. Acceptance journeys:
  ...
  Rule: The turn chrome appears only on the non-interactive prompt turn
