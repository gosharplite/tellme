...
| `the captured standard output is wrapped so that no line is wider than {width} columns` | `width`: integer; the maximum rendered line width. | 不支援 | 無 | `必查`: `呈現結果`: every line of the captured standard output is at most `{width}` display columns wide. `不該發生`: no rendered line exceeds `{width}` columns. |
