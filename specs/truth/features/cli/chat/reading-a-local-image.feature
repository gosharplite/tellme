Feature: Reading a local image with the agent
  ...
  # An image past 32 MiB, or a file that is not a supported picture, fails LOUDLY
  # and no image reaches the request.
  # Acceptance journey: features/acceptance/reading-a-local-image-with-the-agent.feature.
