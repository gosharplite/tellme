...
| `the session's turn log holds the turn progress` | 無 | 不支援 | `來源`: the resolved session's `output/<mode>/turns.log` (the round-053 writer). | `必查`: `權威狀態`: the active session's `turns.log` holds the rendered turn chrome — the `╭─⠿ Turn …` header and a `Payload: …` status line. `不該發生`: a prompt turn must leave a turn log (the write side of US2). |
