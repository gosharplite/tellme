...
| Terminal detection | Go stdlib `os.File` + `os.ModeCharDevice` | Detect whether a stream is a terminal, behind an injected seam (dependency-free — no `golang.org/x/term`). The seam is a general stream probe: wired to **stdin** (gating whether stdin is read) and, since round 006, to **stdout** (gating tellme's own presentation — `UseColor = isTTY && !raw`); the answer's *rendering* is gated by `-r` only |
