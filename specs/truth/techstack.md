...
| No-network verification | No-egress sandbox (privileged netns **or** unprivileged hostile-env fallback) + build-graph capability guard | Prove SC-004 (zero network on `-d`, incl. the failure path): the sandbox is a differential witness (assert identical output with egress blocked — `unshare -n` where permitted, else a hostile DNS/proxy env; the privileged netns is **not** available on the local dev host); the build-graph guard (`go list -deps` / `go tool nm`) asserts no `net` / `net/http` / provider SDK in the diagnostic binary's closure |
