...
| Request assembly | Hand-written mapping from the resolved `config.Provider` | Builds endpoint (`<URL>/chat/completions`), auth (`API_KEY` → `Authorization: Bearer`), `MODEL`, `MAX_TOKENS`, merged `HEADERS`, `reasoning_effort` from the resolved provider entry, and the `messages` array (resumed history + current user prompt) |
| Response normalization | Hand-written extraction of `choices[0].message.content` | Reduces the provider response to the minimal answer text printed to stdout and persisted to the session history (full provider-agnostic `Thought` model deferred) |
...
- **History summarisation and token-budget pruning** — deferred until the **agent-tools** round (round 007 records the plain, full-length append-only history); history **pinning**, the undo/retry flags (`-b`/`--retry`), and `SafePath` are **out of tellme scope** (not merely deferred)
