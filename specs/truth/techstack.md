...
| Terminal detection | Go stdlib `os.File` + `os.ModeCharDevice` | Detect whether a stream is a terminal, behind an injected seam (dependency-free — no `golang.org/x/term`). The seam is a general stream probe: wired to **stdin** (gating whether stdin is read). The **stdout** side uses the same probe and is ready to gate tellme's **own** presentation (`UseColor = isTTY && !raw`), but no such chrome ships yet, so it gates nothing observable today (a named pin, not a verified gate); the answer's *rendering* is gated by `-r` only (round 006) |
