...
| No-network verification | `unshare -n` sandbox + build-graph capability guard | Prove SC-004 (zero network on `-d`, incl. the failure path): the sandbox is a differential witness (assert exit 0 + identical output under an empty netns); the build-graph guard (`go list -deps` / `go tool nm`) asserts no `net` / `net/http` / provider SDK in the diagnostic binary's closure |
