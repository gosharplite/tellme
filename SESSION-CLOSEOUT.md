...
9. **No liveness contradiction.** The **delivered-rounds index** records *delivery*, not *liveness* — it must **never** assert liveness (never write "no round is in flight" / "051 is the most recently delivered round"): a live statement there is born-stale the moment the next round opens. The **`future slices`** roadmap row must **not** list an issue named by the current-round row (a candidate is *not yet opened*; the in-flight round's issues move out of the candidate list). *(Three occurrences of this class — round-045 F-7, round-051 R-51-5, round-052 F-52-2 — are why it is now a closeout rule, not an editorial fix.)*
10. **Curate the open-items index — do not let it render deferrals as work.** `STATUS.md`'s **Open items (non-blocking)** section is an **index of disclosures, not a work queue**. Record each deferred item as a **pointer to its `ADR 00NN §Forward`** with an **explicit trigger**; mark any trigger-gated item **⚠ trigger-gated — do not re-raise**; and **compact a round's forward batch to a single pointer row once the round is two or more rounds old** (the `ADR` stays the authority). A forward item is a *disclosure*, never open tasking — see closeout **Rule 17**. *(The `RF-063-10`/`RF-068-1` recurrence: each was re-litigated at every bootstrap until annotated; the recurrence, not the item, was the bug.)*
...
14. **No liveness contradictions in `STATUS.md`** — the delivered-rounds index states **delivery only** (never "no round is in flight"), and the `future slices` row never names an issue that the current-round row carries. *(Class found 3×: round-045 F-7 · round-051 R-51-5 · round-052 F-52-2.)*
...
17. **Curate open items — a disclosure is not a task** — `STATUS.md`'s **Open items (non-blocking)** section is an **index of disclosures**, not a work queue. Record a deferred item as a **pointer** to its `ADR 00NN §Forward` with an **explicit trigger**; mark a trigger-gated item **⚠ trigger-gated — do not re-raise**; and **compact** a round's forward batch to one pointer row once the round is **two or more rounds old** (the `ADR` remains the authority). Never let a bootstrap-read surface render a deferral as open work — that is how `RF-063-10` and `RF-068-1` each got re-litigated at every session until annotated. **Two addenda (session 52):** (a) **a forward-item cluster must never generate a round theme** — a theme comes from operator value or a live issue, never from "this would clear the backlog"; (b) **a settled / declined decision is never reopened — even to fire a trigger — without explicit operator intent**; a trigger that is settled *off* **retires** its gated items (annotate each in its `ADR §Forward`). *(Session-46 / session-51 / session-52 lesson: the **recurrence / the process**, not the item, is the bug — session 52 opened a round off a forward-item cluster in violation of this rule and had to retract it.)*
