...
**Last updated**: 2026-09-16 (day close, session 16: **round 032 `032-mcp-client` — PLAN + TRUTH HALF CERTIFIED READY**; PR [#66](https://github.com/gosharplite/tellme/pull/66) **open, awaiting human merge**). tellme gains a **remote (Streamable HTTP) MCP client**. The plan+truth half is complete and certified; the implementation half (`/axb-implement`, T001–T032) is **not started**. Round-031 detail lives in the archive (Rule 12); rounds 001–030 live in the archives. Open issues: **#60** (dogfooding-enablement umbrella) · **#13** (coverage tooling).
**Session mode**: `butler` (working directly with the user — no `pm`/`rd` delegation this phase)
**Active branch**: `032-mcp-client` (off `dev`; PR [#66](https://github.com/gosharplite/tellme/pull/66) open — **human-only merge**; next: `/axb-implement` T001–T032)
...
## Round 032 — `032-mcp-client` (PLAN + TRUTH HALF CERTIFIED READY; PR #66 open)

**Status**: ⏳ **PLAN + TRUTH HALF CERTIFIED READY** — branch `032-mcp-client` (off `dev`); PR [#66](https://github.com/gosharplite/tellme/pull/66) open. A **new-capability** round (operator request *"Let tellme support MCP"*) motivated by the reference's startup stall: `tell-me-go` discovers MCP tools on **every** CLI invocation and `wg.Wait()`s under a 30 s cap, so one offline server (`hf`) delayed every run. **No product code yet** — the implementation half (`/axb-implement`, T001–T032) is the next deliverable.

**Scope**: a **remote (Streamable HTTP) MCP client** — a typed `MCP_SERVERS` registry, a `tools.MCPClient` domain port, an SDK adapter confined to `internal/infrastructure/mcp/`, credential resolution, **non-stall** discovery (a small fixed fast-fail bound + per-server `ENABLED`), and deterministic `mcp_<server>_<tool>` names offered **alongside** the native tools. Stdio transport, cross-invocation caching, and MEMORY/PLUR are **deferred** (recorded forward items).

**Pipeline**: specify ✅ · spec-by-example ✅ (3 journeys) · research ✅ (`research.md` D1–D11) · system-analysis ✅ (1 CLI interface; api/data NOOP) · dsl-refine ✅ (ADD `chat/using-tools-from-a-remote-mcp-server.feature` + 15 `chat/dsl.md` rows) · tasks ✅ (T001–T032) · **implement ⏳ (next)** · **merged ⏳** · **propagated ⏳ (pending)**.

**Decisions locked (round 032)**: operator Q1–Q5 — **Q1 → 1** remote HTTP only; **Q2 → 1+3** fixed fast-fail bound + per-server `ENABLED`; **Q3 → 1** official MCP Go SDK (`v1.7.0`), confined behind the domain port; **Q4 → 1** full auth parity (`auto`/`gh`/`bearer`/`basic`/`none`) + reference naming; **Q5 → 1** MCP client only. Review folds: **B1–B3** (schema normalization; SDK-built fake in `mcptest/`; bounded injectable `tokenResolver`), **TD1–TD8**, **R1–R8** (incl. timeout default **300 s** / **fixed 7200 s** ceiling; `CallTool` never returns a non-nil error for a call-time failure), and the principal-review **tasks.md Phase-3 realignment** (T009–T023 = the 15 DSL rows; units T024–T028; review T029; Phase 4 T030–T032).

**Review trail (PR #66)**: Architectural Review (**NOT APPROVED** — B1/B2/B3 + TD1–TD8 + R1–R4) → fold `912010d` → **BLOCKER-FOLD ACCEPTED** (residuals R1–R4) → fold `313e11f` → **CERTIFIED READY** (nits N1–N3) → fold `0b926c4` → **CERTIFICATION STANDS** → closed-client wording (option a) fold `cb407ed` → **review loop CLOSED** → Principal Architectural Review flag (**tasks.md Phase-3 DSL drift** blocker) → fold `4b4ca46` → **principal review: BLOCKER RESOLVED — CERTIFIED READY FOR IMPLEMENTATION**.

**Commits** (branch `032-mcp-client`): `cd882a2` (spec) · `6c622d2` (acceptance) · `37a12f3` (research + techstack) · `4bd6bad` (system-analysis) · `c63633a` (interface truth) · `c94c5ac` (tasks) · `912010d` (review fold B1–B3/TD/R) · `313e11f` (residual fold R1–R8) · `0b926c4` (nits N1–N3) · `cb407ed` (closed-client wording) · `4b4ca46` (principal fold — tasks realignment). Branch head **`4b4ca46`**; pushed to `origin/032-mcp-client`.

**Propagation**: ⏳ **PENDING** — the round is **not** at a mergeable point (the implementation half is not delivered) and no human merge has occurred. Recorded per Step 7 / Rule 8.
...
Per-round detail lives in the archives (001–002 in [`2026-09-11.md`](docs/archives/status/2026-09-11.md); 003–012 in [`2026-09-13.md`](docs/archives/status/2026-09-13.md); 013–019 in [`2026-09-14.md`](docs/archives/status/2026-09-14.md); 020–026 in [`2026-09-15.md`](docs/archives/status/2026-09-15.md); 027–031 in [`2026-09-16.md`](docs/archives/status/2026-09-16.md)); **031 is the most recent delivered round; 032 is in progress**.
...
| `032-mcp-client` | **in progress** | The current round's working branch off `dev`; PR [#66](https://github.com/gosharplite/tellme/pull/66) open (plan+truth half). |
...
> **Direction (2026-09-15)** — no security · no Windows · **bash-first** · a deliberately small tool surface (see [`README.md`](README.md#-design-intent--direction-operator-declared)). Recent slices: **031** ([#64](https://github.com/gosharplite/tellme/issues/64)) = the tool-schema well-formedness fix + gate; **032** = the remote MCP client (in progress).
...
| **032** | (operator request) | The **remote MCP client** (Streamable HTTP) + non-stall discovery. | 🚧 **In progress** (plan+truth half certified; PR [#66](https://github.com/gosharplite/tellme/pull/66) open) |
...
- **Round 032 (current)** — **implementation half not started**: run `/axb-implement` (T001–T032) on `032-mcp-client`; then the **implementation review**, a **human merge** of PR [#66](https://github.com/gosharplite/tellme/pull/66), and propagation `032 → dev → main`.
- **Round-032 forward items** — (a) **local stdio MCP transport** (`COMMAND`); (b) **cross-invocation tool caching**; (c) **MCP-backed MEMORY/PLUR** integration; (d) **MCP `-d` diagnostic** (non-dialing); (e) `mcptest/` to issue [#13](https://github.com/gosharplite/tellme/issues/13)'s coverage exclusion list; (f) the stale `make help` `verify-no-network` text.
...
- **Issue tracker (2026-09-16 closeout, session 16)** — **[#60](https://github.com/gosharplite/tellme/issues/60)** open (dogfooding-enablement umbrella); **[#13](https://github.com/gosharplite/tellme/issues/13)** open (coverage tooling). No issues closed/revised this closeout (round 032 has no anchor issue; nothing landed to close).
...
- **Host / toolchain**: Go 1.26; `golangci-lint` / `staticcheck` / `govulncheck` present in `$GOPATH/bin` (run with `$GOPATH/bin` on `PATH`); the `tellme` binary is installed at `$(go env GOPATH)/bin/tellme` via `go install ./cmd/tellme` (last refreshed from `06d574f`, the merged round-031 head — **not** refreshed this session: round 032 shipped no product code). Sandbox: the privileged netns (`unshare -n`) is unavailable on the host, so the offline-path guard uses the unprivileged canary + hostile-env differential.
- **MCP client dependency (round 032, planned)**: `github.com/modelcontextprotocol/go-sdk` **v1.7.0**, confined to `internal/infrastructure/mcp/**` behind the `tools.MCPClient` port (a `verify-mcp-sdk-confinement` Makefile gate) — **not yet vendored** (T001 lands it).
