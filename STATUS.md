...
| `006-rendered-output-and-raw-flag` | delivered / frozen (round 006) | Round-006 branch — PR [#18](https://github.com/gosharplite/tellme/pull/18) **merged** into `dev` (`958a050`); propagated `→ main`; frozen history |

> **Branch convention**: each round works on its own `NNN-*` branch off `dev`. Delivered round branches
> (`001`, `002`, `003`, `004`, `005`) remain frozen history and never receive post-round commits.
> Read live heads with `git rev-parse --short main dev HEAD`.
>
> **Propagation status (2026-09-12, session 19):** PR [#16](https://github.com/gosharplite/tellme/pull/16) **merged** into `dev` (`37c0c24`); **propagated `dev → main` (no-ff) — DONE**. Round 005 delivered / frozen.
...
**Status**: ✅ **DELIVERED / FROZEN** (2026-09-12, session 20) — PR [#18](https://github.com/gosharplite/tellme/pull/18) **merged** into `dev` (`958a050`); propagated to `main`; godog **46/46 scenarios · 325/325 steps**; `make verify` OK.
...
**Artifacts**: `spec.md`/`checklists` · acceptance Gherkin (`rendering-the-answer`, `controlling-the-rendered-width`) · `research.md` + `specs/truth/techstack.md` · `plan.md` · `specs/truth/features/cli/**` (ADD ×2, MODIFY ×2 incl. round-005 `piping-the-answer-out`; root vocabulary 9→10) · `tasks.md` (23 tasks; orphan sweep 0) · implementation (23/23 `[X]`; `internal/ui/renderer.go`, `-r`, `EffectiveWrapWidth`; PR [#18](https://github.com/gosharplite/tellme/pull/18) **merged** `958a050`).

**Review**: architectural review ([#5645192048](https://github.com/gosharplite/tellme/pull/18#issuecomment-5645192048)) → blockers **H1** (de-reserved degrade `[WARN]`) + **H2** (Obs 1 re-recorded **OPEN**; composition-root fixed) + **M1–M8** fixed in-round (`393d82a`) → re-check ([#5645234132](https://github.com/gosharplite/tellme/pull/18#issuecomment-5645234132)) **FULL APPROVAL** → nit tidy (`7e67d5e`) → final-head certification ([#5645251002](https://github.com/gosharplite/tellme/pull/18#issuecomment-5645251002)) **CLEARED TO MERGE**.

**Open (non-blocking)**: PR #16 **Obs 1** (stdout TTY probe) stays **OPEN** — tellme ships no own presentation chrome to gate; **Obs 2** (`RuntimeEnv` stream consolidation) remains future work.
...
- **Round 006 delivered** — PR [#18](https://github.com/gosharplite/tellme/pull/18) merged (`958a050`) + propagated `dev → main`. **Next round starts a fresh `007-*` off `dev`**.
