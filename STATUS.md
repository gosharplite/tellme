...
**Status**: ⏳ **IN PROGRESS** — branch `031-tool-schema-wellformedness` (off `dev`). Resolves issue [#64](https://github.com/gosharplite/tellme/issues/64): a tool-schema `required`/`properties` defect from round 024 — 5 of 6 tools list `reason` in `required` but never declare its property, so a strict provider (Vertex/Gemini) 400s **every** request.

**Pipeline**: specify ✅ · research ✅ · spec-by-example (expected skipped — no new journey) · system-analysis (expected NOOP) · dsl-refine (expected NOOP) · tasks ⏳ · implement ⏳.
...
**Commits**: `0180f9c` (plan package + spec).

**Next**: `/axb-spec-by-example` (expect skip) → `/axb-system-analysis` → `/axb-tasks` → `/axb-implement`.
