...
- **Binary refresh (session 25, round 010)**: `go install ./cmd/tellme` rebuilt `$(go env GOPATH)/bin/tellme` from `dev`; `tellme --version` reports `dev` (no `-ldflags` version stamp).
