...
func TestRenderToolUsageDiagnosesReadError(t *testing.T) {
	dp := defaultTestDeps(func(d *deps.Dependencies) {
		...
		d.NewToolRegistry = func(domaintools.OutputSink) domaintools.Registry {
			return domaintools.NewRegistry(noopTool{name: "list_files"})
		}
	})

	var out, errBuf bytes.Buffer
	if code := renderToolUsage(runtimeEnv{stdout: &out, stderr: &errBuf}, dp.NewToolRegistry, dp.NewToolUsageStore, dp.UserHomeDir, dp.NewLines(false)); code != Success {
