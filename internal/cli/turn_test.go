...
func TestRunTurn_PrintsRawAnswerAndPersists(t *testing.T) {
	...
	st := &fakeStore{}
	code := runTurn(resolution{Selected: "p", Mode: "butler", MaxHistoryTokens: 1000000, Provider: config.Provider{Model: "deepseek-v4-flash"}}, st, "ping", true, env(&out, &errOut, &stubRenderer{}), factoryReturning(fg, nil), false)
	if code != Success {
...
func TestRunTurn_CarriesPriorMessages(t *testing.T) {
	...
	code := runTurn(resolution{Selected: "p"}, st, "q2", true, env(&out, &errOut, &stubRenderer{}), factoryReturning(fg, nil), false)
	if code != Success {
...
func TestRunTurn_LoadErrorIsEnvironmentError(t *testing.T) {
	...
	code := runTurn(resolution{Selected: "p"}, st, "ping", true, env(&out, &errOut, &stubRenderer{}), factoryReturning(&fakeGateway{text: "x"}, nil), false)
...
func TestRunTurn_ProviderFailure(t *testing.T) {
	...
	code := runTurn(resolution{Selected: "p"}, &fakeStore{}, "ping", true, env(&out, &errOut, &stubRenderer{}), factoryReturning(fg, nil), false)
...
func TestRunTurn_UnsupportedFamilyIsProviderError(t *testing.T) {
	...
	code := runTurn(resolution{Selected: "p"}, &fakeStore{}, "ping", true, env(&out, &errOut, &stubRenderer{}), factoryReturning(nil, buildErr), false)
	if code != ProviderError {
		t.Fatalf("code = %d, want %d (provider error)", code, ProviderError)
	}
...
func TestRunTurn_PostTurnStatusFollowsAnswer(t *testing.T) {
	...
	e := runtimeEnv{stdout: &buf, stderr: &buf, renderer: &stubRenderer{out: "ANSWER"},
		clock: func() time.Time { return time.Date(2026, 9, 13, 12, 0, 0, 0, time.UTC) }}
	if code := runTurn(res, &fakeStore{}, "ping", true, e, factoryReturning(fg, nil), false); code != Success {
		t.Fatalf("code = %d, want success", code)
	}
	out := buf.String()
