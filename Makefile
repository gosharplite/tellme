...
# module dependency; go.mod/go.sum stay unchanged. Install (fork branch, POSIX):
...
# fails, and an ABSENT binary fails (naming the install command) — it never skips.
MODELITH := $(shell command -v modelith 2>/dev/null)
MODELITH_INSTALL := go install github.com/gosharplite/modelith/cmd/modelith@feat/self-domain-model
