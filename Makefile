...
# DEV-TOOL BINARY (like staticcheck/golangci-lint/govulncheck above) — NOT a Go
...
.PHONY: help build fmt vet staticcheck tidy lint vulncheck test test-fast verify verify-no-test-sleep verify-no-network verify-cross-compile verify-mcp-sdk-confinement verify-architecture verify-architecture-update modelith-lint modelith-render modelith-check
